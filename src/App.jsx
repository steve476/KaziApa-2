      ...
      <div style={{ flex:1, overflowY:"auto", padding:14, background:T.bg }}>
        ...
        {msgs.map(m=>(
          <div key={m.id} style={{ display:"flex", justifyContent:m.sender_phone===currentPhone?"flex-end":"flex-start", marginBottom:8 }}>
            <div style={{ maxWidth:"75%", padding:"9px 13px", borderRadius:14, fontSize:13, background:m.sender_phone===currentPhone?(themeColor||T.primary):T.card, color:m.sender_phone===currentPhone?"#fff":T.text, border:m.sender_phone===currentPhone?"none":`1px solid ${T.border}` }}>
              {m.message}
            </div>
          </div>
        ))}
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#fff", zIndex:200, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:14, background:T.bg }}>
        ...
        {msgs.map(m=>(
          <div key={m.id} style={{ display:"flex", justifyContent:m.sender_phone===currentPhone?"flex-end":"flex-start", marginBottom:8 }}>
            <div style={{ maxWidth:"75%", padding:"9px 13px", borderRadius:14, fontSize:13, background:m.sender_phone===currentPhone?"#7D3C98":T.card, color:m.sender_phone===currentPhone?"#fff":T.text, border:m.sender_phone===currentPhone?"none":`1px solid ${T.border}` }}>
              {m.message}
            </div>
          </div>
        ))}
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#fff", zIndex:200, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:14, background:T.bg }}>
        ...
        {msgs.map(m=>(
          <div key={m.id} style={{ display:"flex", justifyContent:m.sender_phone===currentPhone?"flex-end":"flex-start", marginBottom:8 }}>
            <div style={{ maxWidth:"75%", padding:"9px 13px", borderRadius:14, fontSize:13, background:m.sender_phone===currentPhone?T.primary:T.card, color:m.sender_phone===currentPhone?"#fff":T.text, border:m.sender_phone===currentPhone?"none":`1px solid ${T.border}` }}>
              {m.message}
            </div>
          </div>
        ))}
...
function MarketProfileScreen({ market, phone, userName, showToast, onBack, onAddBusiness, onRequestGuide }) {
  ...
  async function loadTab(t) {
    ...
    if(t==="shop") {
      ...
      const { data } = await sb.from("market_notices").select("*").eq("market_id",market.id).eq("status","active").gt("expires_at",new Date().toISOString()).order("created_at",{ ascending:false }).limit(50);
      setNotices(data||[]);
    } else if(t==="spaces") {
