...
function SmartChat({ listing, currentPhone, currentName, buyerPhone, buyerName, sellerPhone, sellerName, onBack, onCheckout }) {
  ...
  async function send() {
    ...
    if (PHONE_RGX.test(input)||CONTACT_RGX.test(input)) {
      await sb.from("chats").insert({ listing_id:String(listing?.id||""), listing_title:listing?.title, buyer_phone:buyerPhone, buyer_name:buyerName, seller_phone:sellerPhone, seller_name:sellerName, sender_phone:"system", sender_name:"KaziApa", message:"⚠️ Phone numbers can't be shared here for safety." });
      setInput(""); setShowPay(true); loadMsgs(); return;
    }
    await sb.from("chats").insert({ listing_id:String(listing?.id||""), listing_title:listing?.title, buyer_phone:buyerPhone, buyer_name:buyerName, seller_phone:sellerPhone, seller_name:sellerName, sender_phone:currentPhone, sender_name:currentName, message:input });
    setInput("");
    loadMsgs();
  }
        ...
        {isBuyer && msgs.length>=2 && !showPay && (
          <div style={{ display:"flex",gap:6,marginBottom:8,overflowX:"auto" }}>
            {[
              ...
              <div key={q.label} onClick={q.action} style={{ background:q.label.includes("WhatsApp")?T.successSoft:T.surface, color:q.label.includes("WhatsApp")?"#25D366":T.textMid, borderRadius:20,padding:"4px 12px",fontSize:11,fontWeight:700,whiteSpace:"nowrap",cursor:"pointer",border:`1px solid ${q.label.includes("WhatsApp")?"#25D366":T.border}`,flexShrink:0 }}>{q.label}</div>
            ))}
          </div>
        )}
        ...
        {loading
          ...
          : fundis.length === 0
            ? <div style={{ textAlign:"center",padding:"40px 0",color:T.textMuted }}>
                ...
                <div style={{ fontSize:13,marginTop:6 }}>Be the first to post your services!</div>
