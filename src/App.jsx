            ...
            {providers.length===0 ? (
              ...
              <div key={p.phone} style={{ background:T.card, borderRadius:14, padding:14, marginBottom:10, border:`1px solid ${T.border}`, display:"flex", gap:12, alignItems:"center" }}>
                ...
                <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                  <button onClick={()=>{ setTargetProvider(p); setPayNow(false); setFormError(""); setShowForm(true); }} style={{ padding:"9px 10px", background:T.accentSoft, color:T.accentDark, border:"none", borderRadius:10, fontSize:12, fontWeight:800, cursor:"pointer", whiteSpace:"nowrap" }}>👉 Nudge</button>
                  <a href={`tel:${p.phone}`} style={{ padding:9, background:T.successSoft, color:T.success, border:"none", borderRadius:10, fontSize:16, textDecoration:"none" }}>📞</a>
                </div>
              </div>
            ))}
          </>
...
function HoodListScreen({ phone, userName, showToast, onBack, onOpenMarket }) {
  const [view, setView] = useState("root"); // root | county | town
  ...
  async function openCounty(c) {
    setCounty(c); setTown(null); setView("county"); setLoading(true);
