      ...
      {showUpgrade && (
        <div style={{ position:"fixed",inset:0,background:T.overlay,zIndex:150,display:"flex",alignItems:"flex-end",justifyContent:"center" }} onClick={()=>{ if(!upgrading) setShowUpgrade(false); }}>
          <div style={{ background:T.card,borderRadius:"22px 22px 0 0",padding:"28px 20px 36px",width:"100%",maxWidth:430 }} onClick={e=>e.stopPropagation()}>
            ...
            {!upgrading && (
              <>
                {[{ plan:"daily",tier:"daily",type:"chat_pass",label:"Daily Pass",price:"Ksh 20 / day",perks:["Unlimited messages","Priority support"],color:T.primaryLight },
                  { plan:"weekly",tier:"week",type:"chat_pass",label:"Weekly Plan",price:"Ksh 50 / week",perks:["Unlimited messages","List up to 20 items","1 free boost"],color:T.primary }
                ].map(p=>(
            ...
            {activeListing.seller_phone === phone && (
              <div style={{ display:"flex",gap:8,marginBottom:10,flexWrap:"wrap" }}>
                ...
                {!activeListing.featured && !showBoostOptions && (
                  <button onClick={()=>setShowBoostOptions(true)} style={{ flex:1,padding:12,background:T.accent,color:T.primary,border:"none",borderRadius:12,fontSize:13,fontWeight:700,cursor:"pointer",minWidth:100 }}>
                    🚀 Boost Listing
                  </button>
                )}
                {!activeListing.featured && showBoostOptions && (
