...
// ─── JOBS SCREEN ─────────────────────────────────────────────────────────────
function SimpleChatThread({ chatKey, title, otherName, otherPhone, currentPhone, currentName, themeColor, onClose }) {
  const [msgs, setMsgs] = useState([]);
  const [input, setInput] = useState("");
    ...
    <div style={{ minHeight:"100vh", background:T.surface, paddingBottom:90 }}>
      <div style={{ background:`linear-gradient(130deg,${T.primary},${T.primaryMid})`, padding:"16px 16px 14px", color:"#fff" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
          ...
          <button onClick={()=>{ setFormError(""); setShowForm(true); }} style={{ padding:"8px 16px", background:T.accent, color:T.primary, border:"none", borderRadius:20, fontSize:13, fontWeight:800, cursor:"pointer" }}>+ Post</button>
        </div>
        <div style={{ display:"flex", gap:8 }}>
      ...
      {showForm && (
        <div style={{ position:"fixed", inset:0, background:T.overlay, zIndex:150, display:"flex", alignItems:"flex-end", justifyContent:"center" }} onClick={()=>setShowForm(false)}>
          <div style={{ background:T.card, borderRadius:"22px 22px 0 0", padding:"24px 20px 32px", width:"100%", maxWidth:430, maxHeight:"85vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            ...
            <button onClick={()=>{ setShowForm(false); setFormError(""); }} style={{ width:"100%", padding:11, marginTop:8, background:"transparent", border:"none", color:T.textMuted, fontSize:13, cursor:"pointer" }}>Cancel</button>
          </div>
        </div>
      )}
