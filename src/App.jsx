        ...
        {tenancy && (
          <>
            <div style={{ background:T.primarySoft, borderRadius:14, padding:16, marginBottom:16 }}>
              ...
              <div onClick={endTenancy} style={{ color:T.coral, fontSize:12, fontWeight:700, marginTop:10, cursor:"pointer" }}>End Tenancy</div>
            </div>
            ...
            {ledger.map(entry => (
              <div key={entry.id} style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:12, padding:14, marginBottom:10 }}>
                ...
                <div style={{ fontSize:12, color:T.textMuted, marginTop:4 }}>M-Pesa: {entry.mpesa_code} · {new Date(entry.submitted_at).toLocaleDateString()}</div>
...
function TenantRentView({ phone, userName, showToast }) {
  ...
  const totalConfirmed = ledger.filter(l=>l.status==="confirmed").reduce((s,l)=>s+Number(l.amount),0);
  ...
  const estBalance = Math.max(0, expectedSoFar - totalConfirmed);
      ...
      {tab === "pay" && (
        <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:14, padding:16 }}>
          ...
          <label style={{ display:"block", marginBottom:12 }}>
            ...
            <input type="file" accept="image/*" onChange={e=>e.target.files[0] && uploadShot(e.target.files[0], setScreenshot)} style={{ fontSize:12 }} />
        ...
        <>
          <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:14, padding:16, marginBottom:16 }}>
            ...
            <label style={{ display:"block", marginBottom:12 }}>
              ...
              <input type="file" accept="image/*" onChange={e=>e.target.files[0] && uploadShot(e.target.files[0], setMPhoto)} style={{ fontSize:12 }} />
