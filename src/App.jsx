      ...
      <div style={{ padding:"10px 16px 20px",background:T.card,borderTop:`1px solid ${T.border}` }}>
        {isBuyer && msgs.length>=2 && !showPay && (
          <div style={{ display:"flex",gap:6,marginBottom:8,overflowX:"auto" }}>
            {[{ label:"💳 Pay Now",action:()=>setShowPay(true) },{ label:"🏍️ Delivery",action:()=>setShowDel(true) },{ label:"💬 Final price?",action:()=>setInput("What is your final price?") },{ label:"📱 Share My Number",action:async()=>{ await sb.from("chats").insert({ listing_id:String(listing?.id||""), listing_title:listing?.title, buyer_phone:buyerPhone, buyer_name:buyerName, seller_phone:sellerPhone, seller_name:sellerName, sender_phone:"system", sender_name:"KaziApa", message:`📱 ${currentName} has shared their number: ${currentPhone}` }); loadMsgs(); } }].map(q=>(
