...
function MarketProfileScreen({ market, phone, userName, showToast, onBack, onAddBusiness }) {
  ...
  async function load() {
    ...
    const { data:mem } = await sb.from("market_members").select("id").eq("phone",phone).eq("market_id",market.id).maybeSingle();
    setJoined(!!mem);
    setLoading(false);
  }

  async function joinMarket() {
    await sb.from("market_members").upsert({ phone, market_id:market.id, roles:["buyer"] }, { onConflict:"phone,market_id" });
    setJoined(true);
    showToast(`Joined ${market.name} 🏬`);
  }

  async function addMyBusiness() {
    const { data: existingKiosk } = await sb.from("kiosks").select("phone").eq("phone",phone).maybeSingle();
    if(existingKiosk) {
    ...
    <div>
      ...
      <div style={{ padding:16 }}>
        <div style={{ background:T.card,borderRadius:13,padding:14,border:`1px solid ${T.border}`,marginBottom:16 }}>
          ...
          {!joined && <button onClick={joinMarket} style={{ marginTop:12,width:"100%",padding:11,background:T.primary,color:"#fff",border:"none",borderRadius:11,fontWeight:700,fontSize:13,cursor:"pointer" }}>+ Join This Market</button>}
        </div>
