...
export default function KaziApa() {
  ...
  const SAFE_SCREENS = ["home","browse","requests","post","profile","chats","kiosk","guidelines","privacy","terms","about","admin","housing","orders"];
...
function KioskScreen({ phone, userName, town, showToast, onBack, onAddItem }) {
  ...
  async function save() {
    ...
    if(kiosk) {
      await sb.from("kiosks").update({ name, description:desc, location, category, logo_url, banner_url, updated_at:new Date().toISOString() }).eq("phone",phone);
    } else {
      await sb.from("kiosks").insert({ phone, seller_name:userName, name, description:desc, location, category, logo_url, banner_url });
    }
          ...
          <>
            ...
            {userResults.map(u=>(
              <div key={u.phone} style={{ background:"#fff",borderRadius:12,padding:14,marginBottom:10,border:`1px solid ${T2.border}` }}>
                <div style={{ fontWeight:700,color:T2.text,fontSize:14 }}>{u.name} {u.is_verified && "✓"} {u.banned && <span style={{ color:T2.coral }}>🚫 BANNED</span>}</div>
                ...
                <button onClick={()=>toggleBan(u)} style={{ marginTop:10,width:"100%",padding:9,background:u.banned?T2.primary:T2.coral,color:"#fff",border:"none",borderRadius:8,fontSize:12,fontWeight:700,cursor:"pointer" }}>
