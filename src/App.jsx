...
function MarketPicker({ marketId, marketName, onPick, label="Tag to a Market (optional)" }) {
  ...
  return (
    <div style={{ marginBottom:14,position:"relative" }}>
      <label style={{ fontSize:12,fontWeight:700,color:T.text,display:"block",marginBottom:7 }}>{label}</label>
      <input value={q} onChange={e=>search(e.target.value)} onFocus={()=>{ if(results.length) setOpen(true); }} placeholder="e.g. Gikomba, Kibuye..."
        style={{ width:"100%",padding:"11px 13px",borderRadius:11,border:`1px solid ${T.border}`,fontSize:14,color:T.text,background:T.card }} />
      {marketId && <div style={{ fontSize:11,color:T.success,marginTop:4 }}>📍 Tagged to {marketName}</div>}
...
export default function KaziApa() {
  ...
  async function postListing() {
    ...
    const { error } = await sb.from("listings").insert({
      ...
      lat:postLat, lng:postLng, seller_avatar_url:userAvatar||null, market_id:postMarketId,
    });
...
function RequestBoard({ userName,phone,town,requests,setRequests,showToast }) {
  ...
  async function postRequest() {
    ...
    const { data } = await sb.from("requests").insert({ buyer_name:userName,buyer_phone:phone,item:form.item,budget:parseInt(form.budget),category:form.category,town:form.town||town,area:form.area,market_id:form.market_id||null }).select();
