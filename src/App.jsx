...
export default function KaziApa() {
  ...
  async function postListing() {
    ...
    const { error } = await sb.from("listings").insert({
      ...
      lat:postLat, lng:postLng, seller_avatar_url:userAvatar||null,
    });
    ...
    <div>
      ...
      <div style={{ padding:16 }}>
        ...
        <div style={{ display:"flex",gap:8,marginBottom:16 }}>
          {[{k:"directory",l:"🏪 Directory"},{k:"info",l:"ℹ️ Info"}].map(t=>(
            <div key={t.k} onClick={()=>setTab(t.k)} style={{ flex:1,textAlign:"center",padding:"9px 0",borderRadius:11,background:tab===t.k?T.primary:T.card,color:tab===t.k?"#fff":T.textMid,fontWeight:700,fontSize:12,border:`1px solid ${tab===t.k?T.primary:T.border}`,cursor:"pointer" }}>{t.l}</div>
          ))}
        </div>
        ...
        {tab==="info" && (
          <div style={{ fontSize:13,color:T.textMid,lineHeight:1.7 }}>
            ...
            <div style={{ marginTop:12,fontSize:11,color:T.textMuted }}>Jobs, Services, Transport, Buyer Requests and Notice Board for this market are coming in the next update.</div>
          </div>
        )}
      </div>
