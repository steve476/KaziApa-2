...
const TOWNS = [
  "Athi River","Awendo","Bungoma","Busia","Butula","Bomet","Chuka","Diani","Eldama Ravine","Eldoret",
  "Embu","Garissa","Gatundu","Gilgil","Githunguri","Homabay","Isiolo","Iten","Kakamega","Kaloleni",
  "Kangundo","Kapenguria","Kapsowar","Karatina","Kabarnet","Keroka","Kerugoya","Kericho","Kiambu",
  "Kikuyu","Kilifi","Kisii","Kisumu","Kitale","Kitui","Kwale","Lamu","Limuru","Lodwar","Lugari",
  "Machakos","Makindu","Malaba","Malindi","Mandera","Maralal","Mariakani","Marsabit","Meru",
  "Migori","Molo","Mombasa","Moyale","Mumias","Muranga","Mwingi","Nairobi","Naivasha","Nakuru",
  "Nanyuki","Narok","Ndhiwa","Nkubu","Nyahururu","Nyamira","Nyeri","Ogembo","Ol Kalou","Othaya",
  "Oyugis","Rongo","Ruiru","Rumuruti","Sagana","Siaya","Shimba Hills","Suneka","Suba","Tala",
  "Taveta","Thika","Turbo","Ugunja","Ukunda","Vihiga","Voi","Wajir","Webuye","Wote",
].sort();
  ...
  if (authStep==="profile") return (
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", background:T.primary, minHeight:"100vh", maxWidth:430, margin:"0 auto", padding:32, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ marginBottom:32 }}>
        ...
        <input value={userTown} onChange={e=>setUserTown(e.target.value)} placeholder="e.g. Butula, Nakuru CBD, Kondele, Gikomba..."
          style={{ width:"100%", padding:"14px 16px", borderRadius:12, border:"2px solid rgba(255,255,255,0.2)", background:"rgba(255,255,255,0.08)", color:"#fff", fontSize:16, fontWeight:600, boxSizing:"border-box", outline:"none" }} />
        <div style={{ fontSize:11, color:"rgba(255,255,255,0.5)", marginTop:6 }}>Type any town, market, estate or area in Kenya</div>
      </div>
          ...
          <div style={{ padding:16 }}>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <select value={postForm.cat} onChange={e=>setPostForm({...postForm,cat:e.target.value})} style={{ width:"100%", padding:"11px 13px", borderRadius:11, border:`1px solid ${T.border}`, fontSize:14, color:T.text, background:T.card }}>
                {CATEGORIES.filter(c=>c.id!=="nyumba").map(c=><option key={c.id} value={c.id}>{c.icon} {lang==="sw"?c.sw:c.en}</option>)}
              </select>
            </div>
            {/* Town — free text so anyone anywhere can post */}
            <div style={{ marginBottom:6 }}>
              <label style={{ fontSize:12, fontWeight:700, color:T.text, display:"block", marginBottom:7 }}>
                Your Town / Area
              </label>
              <input value={postForm.town} onChange={e=>setPostForm({...postForm,town:e.target.value})}
                placeholder="Type your town e.g. Butula, Kondele, Gikomba, Ol Joro Orok..."
                style={{ width:"100%", padding:"11px 13px", borderRadius:11, border:`1px solid ${T.border}`, fontSize:14, color:T.text, boxSizing:"border-box", background:T.card }} />
            </div>
            <div style={{ fontSize:11, color:T.textMuted, marginBottom:14, paddingLeft:2 }}>
              📍 Be specific — include your stage, market or estate so buyers can find you easily.
            </div>
            <div style={{ background:T.accentSoft, borderRadius:14, padding:16, marginBottom:18, border:`1px solid ${T.accent}` }}>
