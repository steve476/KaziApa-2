        ...
        <div>
          ...
          <div style={{ padding:16 }}>
            ...
            {[
              ...
              { icon:"🚪",label:"Log Out",            action:()=>{ localStorage.removeItem("kaziapa_user"); sessionStorage.removeItem("kaziapa_screen"); sessionStorage.removeItem("kaziapa_nav"); navHistoryRef.current=[]; setAuthStep("phone"); setUserName(""); setPhone(""); setOtp(["","","","","",""]); } },
            ].map((item,i)=>(
...
function rtEntryDetail(entry) {
  if (entry.entry_type === "landlord_payment" || entry.entry_type === "payment_submission") {
...
function LandlordRentView({ phone, userName, showToast, onDataChange }) {
  ...
  async function addUnit() {
    ...
    setSaving(true);
    await sb.from("rt_units").insert({ property_id: activeProperty.id, landlord_phone: phone, unit_label: nuLabel, rent_amount: parseFloat(nuRent), due_day: parseInt(nuDueDay) || 5, status: "vacant" });
  ...
  async function endTenancy() {
    if (!confirm2("End this tenancy? The unit will be marked vacant.")) return;
    await sb.from("rt_tenancies").update({ status:"ended", ended_at: new Date().toISOString() }).eq("id", tenancy.id);
  ...
  const UA_LABELS = { payment:"Record Payment", credit:"Add Credit", discount:"Add Discount", charge:"Add Charge", correction:"Correction" };
  const UA_ENTRY_TYPE = { payment:"landlord_payment", credit:"credit", discount:"discount", charge:"additional_charge", correction:"correction" };
  ...
  async function submitUpdateAccount() {
    ...
    const { error } = await sb.from("rt_ledger").insert({
      ...
      payment_method: uaAction === "payment" ? uaMethod : null,
      bank_name: uaAction === "payment" && uaMethod === "bank" ? uaBankName : null,
      reference: uaAction === "payment" && uaMethod !== "mpesa" ? uaReference : null,
      mpesa_code: uaAction === "payment" && uaMethod === "mpesa" && uaReference.trim() ? uaReference.trim().toUpperCase() : null,
      note: uaNote || null,
          ...
          <>
            <div style={{ background:T.primarySoft, borderRadius:14, padding:16, marginBottom:16 }}>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:10 }}>
                ...
                <div onClick={endTenancy} style={{ color:T.coral, fontSize:12, fontWeight:700, cursor:"pointer" }}>End Tenancy</div>
              </div>
            </div>
  ...
  if (view === "inbox") {
    return (
      <div style={{ padding:"0 16px 30px" }}>
        ...
        <div style={{ fontWeight:800, fontSize:17, color:T.text, marginBottom:16 }}>Payment Inbox</div>
        {pendingInbox.length === 0 && <div style={{ color:T.textMuted, fontSize:13, textAlign:"center", padding:30 }}>No payments awaiting confirmation</div>}
        {pendingInbox.map(entry => (
          <div key={entry.id} style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:12, padding:14, marginBottom:10 }}>
            <div style={{ fontWeight:700, color:T.text }}>Ksh {entry.amount}</div>
            <div style={{ fontSize:12, color:T.textMuted, marginTop:2 }}>From {entry.tenant_phone} · M-Pesa: {entry.mpesa_code}</div>
            <div style={{ fontSize:11, color:T.textMuted, marginTop:2 }}>{new Date(entry.submitted_at).toLocaleString()}</div>
            {entry.screenshot_url && <img src={entry.screenshot_url} style={{ width:"100%", maxWidth:200, borderRadius:8, marginTop:8 }} />}
            <div style={{ display:"flex", gap:8, marginTop:10 }}>
              <button onClick={()=>respondToPayment(entry,"confirm")} style={{ flex:1, padding:"8px 0", background:T.success, color:"#fff", border:"none", borderRadius:8, fontSize:12, fontWeight:800, cursor:"pointer" }}>✓ Confirm</button>
              <button onClick={()=>respondToPayment(entry,"dispute")} style={{ flex:1, padding:"8px 0", background:T.coral, color:"#fff", border:"none", borderRadius:8, fontSize:12, fontWeight:800, cursor:"pointer" }}>✕ Dispute</button>
          ...
          <div style={{ background:T.card, borderRadius:"22px 22px 0 0", padding:"24px 20px 36px", width:"100%", maxWidth:430, maxHeight:"85vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            ...
            {uaTargetTenancy && (
              <>
                ...
                <textarea style={{ ...S, resize:"none" }} rows={2} placeholder={uaAction==="correction" ? "Explain this correction (required)" : "Note (optional)"} value={uaNote} onChange={e=>setUaNote(e.target.value)} />
...
function TenantRentView({ phone, userName, showToast }) {
  ...
  async function submitPayment() {
    if (!amount || !mpesaCode.trim()) { showToast("Amount and M-Pesa code are required", "error"); return; }
    setSubmitting(true);
    try {
      const res = await fetch("/api/rent-submit-payment", {
        method:"POST", headers:{ "Content-Type":"application/json" },
        body: JSON.stringify({ tenancyId: active.id, landlordPhone: active.landlord_phone, tenantPhone: phone, amount: parseFloat(amount), mpesaCode, screenshotUrl: screenshot, note }),
      });
      const data = await res.json();
      ...
      setAmount(""); setMpesaCode(""); setNote(""); setScreenshot("");
      setTab("balance");
      loadLedger();
    } catch(err) {
      showToast("Couldn't reach the server. Try again.", "error");
    ...
    <div style={{ padding:"0 16px 30px" }}>
      ...
      {tab === "pay" && (
        <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:14, padding:16 }}>
          ...
          <textarea style={{ ...S, resize:"none" }} rows={2} placeholder="Note (optional)" value={note} onChange={e=>setNote(e.target.value)} />
          <label style={{ display:"block", marginBottom:12 }}>
            <div style={{ fontSize:12, fontWeight:700, color:T.text, marginBottom:7 }}>M-Pesa confirmation screenshot</div>
