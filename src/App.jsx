...
function LandlordRentView({ phone, userName, showToast, onDataChange }) {
  ...
  async function endTenancy() {
    let msg = "End this tenancy? The unit will be marked vacant.";
    if (tenancy.deposit_amount > 0) {
      ...
      if (depRefundable > 0) msg = `Refundable deposit is Ksh ${depRefundable}. Record any deductions or refund now — you won't be able to from this screen once the tenancy ends.\n\nEnd this tenancy anyway?`;
    }
...
function TenantRentView({ phone, userName, showToast }) {
  ...
  return (
    <div style={{ padding:"0 16px 30px" }}>
      ...
      {(active?.emergency_contact_name || active?.next_of_kin) && (
        <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:12, padding:14, marginBottom:16 }}>
          <div style={{ fontWeight:700, color:T.text, fontSize:12, marginBottom:8 }}>🆘 Emergency Contact on File</div>
