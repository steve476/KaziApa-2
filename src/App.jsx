...
function MarketProfileScreen({ market, phone, userName, showToast, onBack, onAddBusiness, onRequestGuide }) {
  ...
  async function reportNotice(noticeId, reason) {
    ...
    const { error } = await sb.from("market_notice_reports").insert({ notice_id:noticeId, reporter_phone:phone, reason });
    if (error) {
      ...
      {reportingNotice && (
        <div style={{ position:"fixed",inset:0,background:T.overlay,zIndex:100,display:"flex",alignItems:"flex-end" }} onClick={()=>setReportingNotice(null)}>
          <div style={{ background:T.card,borderRadius:"20px 20px 0 0",padding:20,width:"100%" }} onClick={e=>e.stopPropagation()}>
            ...
            {["Spam","Scam","False information","Harassment","Duplicate","Wrong market","Illegal content","Other"].map(reason=>(
              <div key={reason} onClick={()=>reportNotice(reportingNotice.id, reason)} style={{ padding:"12px 4px",borderBottom:`1px solid ${T.border}`,fontSize:14,color:T.text,cursor:"pointer" }}>{reason}</div>
            ))}
