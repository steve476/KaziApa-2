...
const ROLE_LABELS = { property_manager:"Property Manager", caretaker:"Caretaker", accountant:"Accountant" };
const ROLE_DESCRIPTIONS = {
  property_manager: "Full access — properties, tenants, payments, maintenance, settings. Can't manage the team.",
  caretaker: "Views tenants and units, handles maintenance requests. No payment or financial access.",
  accountant: "Views and confirms payments, records manual entries. No property or maintenance access.",
};
...
// ─── PHOTO UPLOAD ────────────────────────────────────────────────────────────
function MarketPicker({ marketId, marketName, onPick, label="Tag to a Market (optional)" }) {
  const [q, setQ] = useState(marketName || "");
  const [results, setResults] = useState([]);
  const [open, setOpen] = useState(false);

  async function search(text) {
    setQ(text);
    if(!text.trim()) { setResults([]); onPick(null,""); return; }
    const { data } = await sb.from("markets").select("id,name,town,county,type,level").eq("status","ACTIVE").ilike("name",`%${text}%`).limit(6);
    setResults(data||[]);
    setOpen(true);
  }
  ...
  return (
    <div style={{ marginBottom:14,position:"relative" }}>
      <label style={{ fontSize:12,fontWeight:700,color:T.text,display:"block",marginBottom:7 }}>{label}</label>
      <input value={q} onChange={e=>search(e.target.value)} onFocus={()=>{ if(results.length) setOpen(true); }} placeholder="Type a market name, then tap it below..."
        style={{ width:"100%",padding:"11px 13px",borderRadius:11,border:`1px solid ${T.border}`,fontSize:14,color:T.text,background:T.card }} />
      {!marketId && <div style={{ fontSize:11,color:T.textMuted,marginTop:4 }}>Tap a suggestion to tag it — typing alone won't tag it</div>}
      {marketId && <div style={{ fontSize:11,color:T.success,marginTop:4,fontWeight:700 }}>✅ Tagged to {marketName}</div>}
      {open && results.length>0 && (
        <div style={{ position:"absolute",top:"100%",left:0,right:0,background:T.card,border:`1px solid ${T.border}`,borderRadius:11,marginTop:4,zIndex:20,maxHeight:200,overflowY:"auto" }}>
          {results.map(m=>(
            <div key={m.id} onClick={()=>{ onPick(m.id,m.name); setQ(m.name); setOpen(false); }} style={{ padding:"10px 13px",borderBottom:`1px solid ${T.border}`,cursor:"pointer",fontSize:13 }}>
              {m.name} <span style={{ color:T.textMuted,fontSize:11 }}>{m.type==="hood" ? `— 🏘️ ${(m.level||"").toLowerCase()}, ${m.county||""}` : `— ${m.town||m.county||""}`}</span>
            </div>
          ))}
        </div>
      )}
...
function HoodPicker({ hoodId, hoodName, onPick, label="Tag to a Hood (optional)" }) {
  const [q, setQ] = useState(hoodName || "");
  const [results, setResults] = useState([]);
  const [open, setOpen] = useState(false);
  ...
  async function search(text) {
    setQ(text);
    if(!text.trim()) { setResults([]); onPick(null,""); return; }
    const { data } = await sb.from("markets").select("id,name,level,county").eq("type","hood").eq("status","ACTIVE").ilike("name",`%${text}%`).limit(6);
    setResults(data||[]);
    setOpen(true);
  }
  ...
  return (
    <div style={{ marginBottom:14,position:"relative" }}>
      <label style={{ fontSize:12,fontWeight:700,color:T.text,display:"block",marginBottom:7 }}>{label}</label>
      <input value={q} onChange={e=>search(e.target.value)} onFocus={()=>{ if(results.length) setOpen(true); }} placeholder="Type your estate or area..."
        style={{ width:"100%",padding:"11px 13px",borderRadius:11,border:`1px solid ${T.border}`,fontSize:14,color:T.text,background:T.card }} />
      {!hoodId && <div style={{ fontSize:11,color:T.textMuted,marginTop:4 }}>Tap a suggestion to tag it — typing alone won't tag it</div>}
      {hoodId && <div style={{ fontSize:11,color:T.success,marginTop:4,fontWeight:700 }}>✅ Tagged to {hoodName}</div>}
      {open && results.length>0 && (
        <div style={{ position:"absolute",top:"100%",left:0,right:0,background:T.card,border:`1px solid ${T.border}`,borderRadius:11,marginTop:4,zIndex:20,maxHeight:200,overflowY:"auto" }}>
          {results.map(h=>(
            <div key={h.id} onClick={()=>{ onPick(h.id,h.name); setQ(h.name); setOpen(false); }} style={{ padding:"10px 13px",borderBottom:`1px solid ${T.border}`,cursor:"pointer",fontSize:13 }}>
              {h.name} <span style={{ color:T.textMuted,fontSize:11 }}>— {h.level.toLowerCase()}, {h.county}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ padding:"10px 12px",flex:1,minWidth:0 }}>
        ...
        <div style={{ fontSize:11,color:T.textMuted,marginTop:2,display:"flex",alignItems:"center",gap:4 }}>
          ...
          {listing.seller_avatar_url
            ? <img src={listing.seller_avatar_url} alt="" style={{ width:14,height:14,borderRadius:"50%",objectFit:"cover" }} />
            : null
          }
          {listing.seller_name}{listing._distanceKm!=null ? ` · ${listing._distanceKm<1 ? Math.round(listing._distanceKm*1000)+"m" : listing._distanceKm.toFixed(1)+"km"} away` : ""}
        </div>
...
// Shared by every chat thread component (SmartChat, SimpleChatThread,
// NitumeChatThread, UsafiriChatThread) — compresses then uploads a
// single image, returns its public URL.
async function compressAndUploadChatImage(file) {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob(async (blob) => {
          try {
            const name = `chat-${Date.now()}-${Math.random().toString(36).slice(2)}.jpg`;
            const { error } = await sb.storage.from("kaziapa-photos").upload(name, blob, { contentType:"image/jpeg" });
...
function SmartChat({ listing, currentPhone, currentName, buyerPhone, buyerName, sellerPhone, sellerName, onBack, onCheckout, onNeedUpgrade }) {
  ...
  const otherPhone = isBuyer ? sellerPhone : buyerPhone;
  ...
  useEffect(()=>{
    ...
    (async ()=>{
      const { data } = await sb.from("users").select("avatar_url").eq("phone",otherPhone).single();
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",height:"100vh",maxWidth:430,margin:"0 auto",background:T.surface }}>
      <div style={{ background:T.primary,padding:"14px 16px",display:"flex",alignItems:"center",gap:12 }}>
        <div onClick={onBack} style={{ color:"#fff",fontSize:22,cursor:"pointer" }}>←</div>
        <div style={{ width:40,height:40,borderRadius:"50%",background:T.accent,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,color:T.primary,fontSize:18,overflow:"hidden",flexShrink:0 }}>
        ...
        {msgs.map((msg,i)=>(
          <div key={i} style={{ display:"flex",justifyContent:msg.sender_phone==="system"?"center":msg.sender_phone===currentPhone?"flex-end":"flex-start" }}>
            {msg.sender_phone==="system"
              ...
              : <div style={{ background:msg.sender_phone===currentPhone?T.primary:T.card,color:msg.sender_phone===currentPhone?"#fff":T.text,borderRadius:msg.sender_phone===currentPhone?"16px 16px 4px 16px":"16px 16px 16px 4px",padding:msg.image_url?6:"10px 14px",maxWidth:"76%",fontSize:14,lineHeight:1.4,border:msg.sender_phone===currentPhone?"none":`1px solid ${T.border}` }}>
                  {msg.image_url ? (
                    <img src={msg.image_url} alt="" onClick={()=>window.open(msg.image_url,"_blank")} style={{ maxWidth:220,borderRadius:12,display:"block",cursor:"pointer" }} />
      ...
      <div style={{ padding:"10px 16px 20px",background:T.card,borderTop:`1px solid ${T.border}` }}>
        ...
        <div style={{ display:"flex",gap:8 }}>
          <input type="file" accept="image/*" ref={chatImgRef} onChange={sendImage} style={{ display:"none" }} />
          <button onClick={()=>chatImgRef.current?.click()} disabled={sendingImage} style={{ width:44,height:44,borderRadius:"50%",background:T.surface,border:`1px solid ${T.border}`,color:T.textMid,fontSize:18,cursor:sendingImage?"default":"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
            {sendingImage ? "⏳" : "📷"}
          </button>
...
function ChatsList({ phone, onOpenChat, onBack }) {
  ...
  async function load() {
    ...
    setThreads(list);
    setLoading(false);
    const others = [...new Set(list.map(t=>t.buyer_phone===phone?t.seller_phone:t.buyer_phone).filter(Boolean))];
    if(others.length) {
      const { data:users } = await sb.from("users").select("phone,avatar_url").in("phone",others);
      const map = {}; (users||[]).forEach(u=>{ if(u.avatar_url) map[u.phone]=u.avatar_url; });
      setAvatarMap(map);
    }
  }
    ...
    <div>
      ...
      <div style={{ padding:16 }}>
        ...
        : threads.map((t,i)=>{
            ...
            const otherPhone = isBuyer ? t.seller_phone : t.buyer_phone;
            const otherAvatar = avatarMap[otherPhone];
            return (
              <div key={i} onClick={()=>onOpenChat(t)} style={{ background:T.card,borderRadius:14,padding:14,marginBottom:10,border:`1px solid ${T.border}`,cursor:"pointer",display:"flex",alignItems:"center",gap:12 }}>
                <div style={{ width:42,height:42,borderRadius:"50%",background:T.primary,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,overflow:"hidden",flexShrink:0 }}>
                  {otherAvatar ? <img src={otherAvatar} alt="" style={{ width:"100%",height:"100%",objectFit:"cover" }} /> : otherName?.[0]}
                </div>
...
export default function KaziApa() {
  ...
  const [userAvatar,setUserAvatar] = useState(()=>getSaved("avatar"));
  ...
  const SAFE_SCREENS = ["home","browse","requests","post","profile","chats","kiosk","guidelines","privacy","terms","about","admin","housing","orders","nitume","market"];
  // Note: "marketProfile" is intentionally NOT in SAFE_SCREENS — it depends on
  // activeMarket, which isn't safe to restore blindly after a raw page refresh.
  ...
  const [postMarketId,setPostMarketId] = useState(null);
  const [postMarketName,setPostMarketName] = useState("");
  ...
  const [hoodNavState, setHoodNavState] = useState({ view:"root", county:null, town:null, search:"" });
  ...
  useEffect(()=>{
    ...
    (async ()=>{
      const { data } = await sb.from("users").select("nitume_available,chat_unlimited_until,chat_plan_tier,is_verified,verified_expires_at,free_boosts_remaining,avatar_url").eq("phone",phone).single();
      setNitumeAvailable(!!data?.nitume_available);
      setPlanStatus(data||null);
      if(data?.avatar_url && data.avatar_url!==userAvatar) {
        setUserAvatar(data.avatar_url);
        const saved = JSON.parse(localStorage.getItem("kaziapa_user")||"{}");
        localStorage.setItem("kaziapa_user", JSON.stringify({ ...saved, avatar:data.avatar_url }));
      }
    })();
  },[screen]);
  ...
  async function loadListings() {
    ...
    if(nearMeActive && userCoords) {
      // True distance sort: pull listings that have coordinates, rank by real distance.
      let q = sb.from("listings").select("*").not("lat","is",null).not("lng","is",null)
        .order("featured",{ ascending:false }).order("created_at",{ ascending:false }).limit(300);
      if(catFilter) q = q.eq("category",catFilter);
      else q = q.not("category","in","(kazi,fundi,usafiri)");
      if(search) q = q.or(`title.ilike.%${search}%,town.ilike.%${search}%`);
      const { data } = await q;
      const withDist = (data||[]).map(l=>({ ...l, _distanceKm: distanceKm(userCoords.lat,userCoords.lng,l.lat,l.lng) }))
        .sort((a,b)=>a._distanceKm-b._distanceKm).slice(0,50);
      setListings(withDist);
      setLoading(false);
      return;
    }
    let q = sb.from("listings").select("*").order("featured",{ ascending:false }).order("created_at",{ ascending:false });
    if(town!=="All") q = q.ilike("town",`%${town}%`);
    if(catFilter) q = q.eq("category",catFilter);
    else q = q.not("category","in","(kazi,fundi,usafiri)");
    if(search) q = q.or(`title.ilike.%${search}%,town.ilike.%${search}%`);
  ...
  // Detects the user's current GPS position and sorts listings by real distance.
  // Falls back to matching the reverse-geocoded town name if too few listings have coordinates yet.
  function findNearMe() {
    ...
    navigator.geolocation.getCurrentPosition(async(pos)=>{
      ...
      setSearch("");
      try {
        let q = sb.from("listings").select("id,lat,lng").not("lat","is",null).not("lng","is",null).limit(300);
        const { data } = await q;
        if((data||[]).length >= 3) {
          setTown("All");
          await loadListingsNear({ lat:latitude, lng:longitude });
          showToast("Showing listings sorted by distance from you","success");
        } else {
          // Not enough sellers have shared exact location yet — fall back to town name.
          ...
          const place = addr.town||addr.city||addr.suburb||addr.village||addr.county||addr.state_district||"";
          setNearMeActive(false);
          if(place) {
            setTown(place);
            setTimeout(loadListings,300);
            showToast(`Showing listings near ${place}`,"success");
          } else {
            showToast("Couldn't detect your town — try typing it instead","error");
          }
        }
      } catch(e) {
        showToast("Couldn't detect your location — check your connection","error");
      } finally {
        setLocatingMe(false);
      }
    }, ()=>{ setLocatingMe(false); showToast("Location permission denied","error"); }, { timeout:10000, enableHighAccuracy:true });
  }

  async function loadListingsNear(coords) {
    setLoading(true);
    let q = sb.from("listings").select("*").not("lat","is",null).not("lng","is",null)
      .order("featured",{ ascending:false }).order("created_at",{ ascending:false }).limit(300);
    if(catFilter) q = q.eq("category",catFilter);
    ...
    if(search) q = q.or(`title.ilike.%${search}%,town.ilike.%${search}%`);
    const { data } = await q;
    const withDist = (data||[]).map(l=>({ ...l, _distanceKm: distanceKm(coords.lat,coords.lng,l.lat,l.lng) }))
      .sort((a,b)=>a._distanceKm-b._distanceKm).slice(0,50);
    setListings(withDist);
    setLoading(false);
  }

  // Lets a seller attach their exact GPS coordinates to a listing at post time.
  function capturePostLocation() {
    ...
    navigator.geolocation.getCurrentPosition((pos)=>{
      ...
      showToast("Location attached to your listing 📍","success");
    }, ()=>{ setPostLocating(false); showToast("Location permission denied","error"); }, { timeout:10000, enableHighAccuracy:true });
  }
  ...
  async function postListing() {
    ...
    const { error } = await sb.from("listings").insert({
      ...
      lat:postLat, lng:postLng, seller_avatar_url:userAvatar||null, market_id:postMarketId, market_name:postMarketName||null,
    });
    if(error) { showToast("Error posting. Try again.","error"); }
    else {
      showToast("Listing posted and live immediately! 🎉");
      setPostTitle(""); setPostPrice(""); setPostDesc(""); setPostTown(""); setPostPhotos([]); setPostBoost(false); setPostLat(null); setPostLng(null);
      setPostMarketId(null); setPostMarketName("");
      await loadListings();
      nav("home","home");
    }
    setPosting(false);
  ...
  async function saveUser() {
    ...
    localStorage.setItem("kaziapa_user", JSON.stringify({ phone, name:userName, town:userTown, avatar:userAvatar }));
    setAuthStep("app"); setActiveNav("home"); showToast(`Welcome ${userName}! 🤝`);
  }

  // Compress + upload a profile photo, save it to the user's account, and keep localStorage in sync.
    ...
    try {
      const compressed = await new Promise((resolve)=>{
        ...
        reader.onload = (e)=>{
          ...
          img.onload = ()=>{
            const canvas = document.createElement("canvas");
            const MAX = 300; // avatars are small — no need for the 900px listing-photo size
            let w = img.width, h = img.height;
            if(w > MAX) { h = Math.round(h * MAX / w); w = MAX; }
            if(h > MAX) { w = Math.round(w * MAX / h); h = MAX; }
            canvas.width = w; canvas.height = h;
      ...
      const name = `avatar-${phone}-${Date.now()}.jpg`;
      const { error } = await sb.storage.from("kaziapa-photos").upload(name, compressed, { contentType:"image/jpeg" });
      if(error) { showToast("Upload failed — try again","error"); return; }
      const { data:pub } = sb.storage.from("kaziapa-photos").getPublicUrl(name);
      const url = pub.publicUrl;
      await sb.from("users").update({ avatar_url:url }).eq("phone",phone);
      setUserAvatar(url);
      const saved = JSON.parse(localStorage.getItem("kaziapa_user")||"{}");
      localStorage.setItem("kaziapa_user", JSON.stringify({ ...saved, avatar:url }));
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif",background:T.surface,minHeight:"100vh",maxWidth:430,margin:"0 auto",position:"relative",paddingBottom:72 }}>
        ...
        <div>
          ...
          <div style={{ padding:"14px 16px 4px" }}>
            <div style={{ background:T.card,borderRadius:13,padding:"11px 14px",display:"flex",alignItems:"center",gap:8,border:`1px solid ${T.border}` }}>
              <span style={{ fontSize:16 }}>🔍</span>
              <input value={search} onChange={e=>{ setSearch(e.target.value); setNearMeActive(false); setTimeout(loadListings,500); }} placeholder="Search item or town..."
              ...
              <span onClick={()=>{ nav("browse","browse"); setBrowseTab("listings"); setCatFilter(null); findNearMe(); }} style={{ color:T.primary,cursor:"pointer",fontSize:16 }}>📍</span>
            </div>
          </div>
          ...
          <div style={{ margin:"10px 16px 4px",background:`linear-gradient(130deg,${T.success},#0E6655)`,borderRadius:16,padding:"16px 18px",cursor:"pointer" }}
            onClick={()=>nav("market","home")}>
            <div style={{ color:"#fff",fontSize:10,fontWeight:700,letterSpacing:1.2,marginBottom:5,opacity:0.85 }}>MY MARKET</div>
            <div style={{ color:"#fff",fontWeight:800,fontSize:15 }}>🏬 Find your local market</div>
            <div style={{ color:"rgba(255,255,255,0.75)",fontSize:12,marginTop:3 }}>Gikomba, Kibuye, and 60+ markets across Kenya →</div>
          </div>
          <div style={{ margin:"10px 16px 4px",background:`linear-gradient(130deg,#6B4EFF,#9B7EFF)`,borderRadius:16,padding:"16px 18px",cursor:"pointer" }}
            onClick={()=>nav("hood","home")}>
            <div style={{ color:"#fff",fontSize:10,fontWeight:700,letterSpacing:1.2,marginBottom:5,opacity:0.85 }}>MY HOOD</div>
            <div style={{ color:"#fff",fontWeight:800,fontSize:15 }}>🏘️ Find your estate or area</div>
            <div style={{ color:"rgba(255,255,255,0.75)",fontSize:12,marginTop:3 }}>Notices, jobs & requests just for your hood →</div>
          </div>
          ...
          {browseTab==="listings" && (
            <div style={{ padding:"14px 16px" }}>
              ...
              <div style={{ display:"flex",gap:8,marginBottom:12 }}>
                <input value={town==="All"?"":town} onChange={e=>{ setTown(e.target.value||"All"); setNearMeActive(false); setTimeout(loadListings,500); }} placeholder="Town..."
                  style={{ flex:1,padding:"9px 10px",borderRadius:11,border:`1px solid ${T.border}`,fontSize:12,color:T.text,background:T.card }} />
                <div onClick={()=>{ if(nearMeActive){ setNearMeActive(false); setTown("All"); setTimeout(loadListings,50); } else { findNearMe(); } }} style={{ display:"flex",alignItems:"center",gap:5,padding:"9px 12px",borderRadius:11,border:nearMeActive?`1px solid ${T.primary}`:`1px solid ${T.border}`,fontSize:12,fontWeight:700,color:nearMeActive?"#fff":T.primary,background:nearMeActive?T.primary:T.card,cursor:"pointer",whiteSpace:"nowrap" }}>
                  {locatingMe ? "📍 Locating..." : nearMeActive ? "✕ Nearest" : "📍 Near Me"}
                </div>
              </div>
              ...
              <div style={{ display:"flex",gap:8,marginBottom:10,flexWrap:"wrap" }}>
                ...
                <button onClick={async()=>{
                  if(window.confirm("Delete this listing?")) {
                    const { error } = await sb.from("listings").delete().eq("id",activeListing.id);
      ...
      {screen==="post" && (
        <div>
          ...
          <div style={{ padding:16 }}>
            ...
            <div onClick={capturePostLocation} style={{ display:"flex",alignItems:"center",gap:6,marginTop:-8,marginBottom:14,fontSize:12,fontWeight:700,color:postLat?T.success:T.primary,cursor:"pointer" }}>
              📍 {postLocating ? "Getting your location..." : postLat ? "Location attached — buyers can find you with Near Me" : "Attach my exact location (helps buyers find you nearby)"}
            </div>
            <MarketPicker marketId={postMarketId} marketName={postMarketName} onPick={(id,name)=>{ setPostMarketId(id); setPostMarketName(name); }} />
            <div style={{ background:T.accentSoft,borderRadius:14,padding:16,marginBottom:18,border:`1px solid ${T.accent}` }}>
        ...
        <div>
          <div style={{ background:T.primary,padding:"22px 16px 18px" }}>
            <div style={{ display:"flex",alignItems:"center",gap:14 }}>
              <label style={{ position:"relative",width:58,height:58,borderRadius:"50%",background:T.accent,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,fontWeight:800,color:T.primary,cursor:"pointer",flexShrink:0,overflow:"hidden" }}>
                {userAvatar
                  ? <img src={userAvatar} alt="" style={{ width:"100%",height:"100%",objectFit:"cover" }} />
                  : userName[0]?.toUpperCase()
                }
                {avatarUploading && <div style={{ position:"absolute",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:"#fff" }}>...</div>}
                <div style={{ position:"absolute",bottom:0,right:0,width:20,height:20,borderRadius:"50%",background:T.primary,border:"2px solid #fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10 }}>📷</div>
      ...
      {/* MY MARKET */}
      {screen==="market" && (
        <MarketListScreen phone={phone} userName={userName} town={userTown} showToast={showToast}
          onBack={()=>nav("home","home")}
          onOpenMarket={(m)=>{ setActiveMarket(m); nav("marketProfile","home"); }} />
      )}
      {screen==="marketProfile" && activeMarket && (
        <MarketProfileScreen market={activeMarket} phone={phone} userName={userName} showToast={showToast}
          onBack={()=>nav(activeMarket.type==="hood" ? "hood" : "market","home")}
          onAddBusiness={()=>{ nav("kiosk","profile"); }}
          onRequestGuide={()=>{
            try { sessionStorage.setItem("kaziapa_pending_guide_request", JSON.stringify({ marketName:activeMarket.name })); } catch(e) {}
            nav("nitume","home");
          }} />
      )}
      ...
      {/* MY HOOD */}
      {screen==="hood" && (
        <HoodListScreen phone={phone} userName={userName} showToast={showToast}
          onBack={()=>nav("home","home")}
          onOpenMarket={(h)=>{ setActiveMarket(h); nav("marketProfile","home"); }}
          navState={hoodNavState} setNavState={setHoodNavState} />
      )}
...
// ─── JOBS SCREEN ─────────────────────────────────────────────────────────────
function SimpleChatThread({ chatKey, title, otherName, otherPhone, currentPhone, currentName, themeColor, onClose, banner }) {
  const [msgs, setMsgs] = useState([]);
  const [input, setInput] = useState("");
  ...
  async function load() {
    const { data } = await sb.from("chats").select("*").eq("listing_id",chatKey).order("created_at",{ ascending:true });
    setMsgs(data||[]);
    await sb.from("chats").update({ read:true }).eq("listing_id",chatKey).neq("sender_phone",currentPhone);
  }
  ...
  async function send() {
    ...
    await sb.from("chats").insert({
      listing_id: chatKey, listing_title: title,
      buyer_phone: currentPhone, buyer_name: currentName,
      seller_phone: otherPhone, seller_name: otherName,
      sender_phone: currentPhone, sender_name: currentName,
      message: input, read:false,
    });
    setInput("");
    setSending(false);
    load();
  ...
  async function sendImage(e) {
    ...
    try {
      ...
      await sb.from("chats").insert({
        ...
        seller_phone: otherPhone, seller_name: otherName,
        sender_phone: currentPhone, sender_name: currentName,
        message: "📷 Photo", image_url: url, read:false,
      });
      load();
    } catch(err) { alert("Couldn't send that photo — try again."); }
    setSendingImage(false);
    e.target.value = "";
  }
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#fff", zIndex:200, display:"flex", flexDirection:"column" }}>
      <div style={{ background:themeColor||T.primary, padding:"16px 16px 14px", color:"#fff", display:"flex", alignItems:"center", gap:10 }}>
        <span onClick={onClose} style={{ cursor:"pointer", fontSize:20 }}>←</span>
        <div>
          <div style={{ fontWeight:800, fontSize:15 }}>{otherName}</div>
          <div style={{ fontSize:11, opacity:0.85 }}>{title}</div>
        </div>
      </div>
      {banner}
      <div style={{ flex:1, overflowY:"auto", padding:14, background:T.bg }}>
        {msgs.length===0 && <div style={{ textAlign:"center", color:T.textMuted, fontSize:12, marginTop:20 }}>No messages yet — say hello 👋</div>}
        {msgs.map(m=>(
          <div key={m.id} style={{ display:"flex", justifyContent:m.sender_phone===currentPhone?"flex-end":"flex-start", marginBottom:8 }}>
            <div style={{ maxWidth:"75%", padding:m.image_url?6:"9px 13px", borderRadius:14, fontSize:13, background:m.sender_phone===currentPhone?(themeColor||T.primary):T.card, color:m.sender_phone===currentPhone?"#fff":T.text, border:m.sender_phone===currentPhone?"none":`1px solid ${T.border}` }}>
              {m.image_url ? <img src={m.image_url} alt="" onClick={()=>window.open(m.image_url,"_blank")} style={{ maxWidth:200,borderRadius:10,display:"block",cursor:"pointer" }} /> : m.message}
            </div>
          </div>
        ))}
      </div>
      <div style={{ display:"flex", gap:8, padding:12, borderTop:`1px solid ${T.border}`, background:"#fff" }}>
        <input type="file" accept="image/*" ref={imgRef} onChange={sendImage} style={{ display:"none" }} />
        <button onClick={()=>imgRef.current?.click()} disabled={sendingImage} style={{ width:44,height:44,borderRadius:"50%",background:T.surface,border:`1px solid ${T.border}`,color:T.textMid,fontSize:16,cursor:sendingImage?"default":"pointer",flexShrink:0 }}>{sendingImage?"⏳":"📷"}</button>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter") send(); }} placeholder="Type a message..."
          style={{ flex:1, padding:12, borderRadius:20, border:`1px solid ${T.border}`, fontSize:14 }} />
        <button onClick={send} disabled={sending||!input.trim()} style={{ padding:"0 18px", background:themeColor||T.primary, color:"#fff", border:"none", borderRadius:20, fontWeight:700, cursor:"pointer" }}>Send</button>
      </div>
    </div>
  );
}
...
function JobsScreen({ userName, phone, town, showToast, onBack, lang }) {
  ...
  const [jobChat, setJobChat] = useState(null);
  ...
  const [posting, setPosting]     = useState(false);
  const [jobMarketId, setJobMarketId] = useState(null);
  const [jobMarketName, setJobMarketName] = useState("");
  ...
  async function postJob() {
    ...
    const { error } = await sb.from("listings").insert({
      ...
      market_id: jobMarketId, market_name: jobMarketName||null,
    });
    if (error) { showToast("Error posting. Try again.","error"); }
    else {
      ...
      setJobMarketId(null); setJobMarketName("");
      setShowForm(false);
      await loadJobs();
    }
    setPosting(false);
  ...
  if (activeItem) return (
    <div>
      ...
      <div style={{ padding: 16 }}>
        ...
        {activeItem.seller_phone !== phone && (
          <>
            <button style={{ width:"100%", padding:16, background:T.primary, color:"#fff", border:"none", borderRadius:14, fontSize:16, fontWeight:800, cursor:"pointer", marginBottom:10 }}
              onClick={() => setJobChat(activeItem)}>
              💬 {tab === "need" ? "Apply for this Job" : "Contact this Person"}
            </button>
            <a href={`tel:${activeItem.seller_phone}`} style={{ textDecoration:"none" }}>
              <button style={{ width:"100%", padding:14, background:T.successSoft, color:T.success, border:"none", borderRadius:14, fontSize:14, fontWeight:800, cursor:"pointer" }}>
                📞 Call {activeItem.seller_name}
              </button>
            </a>
          </>
        )}
        {activeItem.seller_phone === phone && (
          <button onClick={async()=>{
            if(window.confirm("Delete this listing?")) {
              const { error } = await sb.from("listings").delete().eq("id",activeItem.id);
              if (error) { showToast("Delete failed: " + error.message, "error"); return; }
              showToast("Listing deleted."); setActiveItem(null); await load();
            }
          }} style={{ width:"100%", padding:14, background:T.coral, color:"#fff", border:"none", borderRadius:14, fontSize:14, fontWeight:800, cursor:"pointer" }}>
            🗑️ Delete This Listing
          </button>
        )}
      </div>
      {jobChat && (
        <SimpleChatThread chatKey={`jobs-${activeItem.id}`} title={activeItem.title} otherName={activeItem.seller_name} otherPhone={activeItem.seller_phone}
          currentPhone={phone} currentName={userName} themeColor={T.primary} onClose={()=>setJobChat(null)} />
      )}
    </div>
  );
      ...
      {showForm && (
        <div style={{ position:"fixed", inset:0, background:T.overlay, zIndex:150, display:"flex", alignItems:"flex-end", justifyContent:"center" }} onClick={() => setShowForm(false)}>
          <div style={{ background:T.card, borderRadius:"22px 22px 0 0", padding:"24px 20px 36px", width:"100%", maxWidth:430, maxHeight:"85vh", overflowY:"auto" }} onClick={e => e.stopPropagation()}>
            ...
            <MarketPicker marketId={jobMarketId} marketName={jobMarketName} onPick={(id,name)=>{ setJobMarketId(id); setJobMarketName(name); }} />
...
function UsafiriChatThread({ listing, currentPhone, currentName, onClose }) {
  const chatKey = `usafiri-${listing.id}`;
  const [msgs, setMsgs] = useState([]);
  const [input, setInput] = useState("");
  const [sending, setSending] = useState(false);
  const [sendingImage, setSendingImage] = useState(false);
  ...
  async function send() {
    ...
    setSending(true);
    await sb.from("chats").insert({
      listing_id: chatKey, listing_title: listing.title,
      buyer_phone: currentPhone, buyer_name: currentName,
      seller_phone: listing.seller_phone, seller_name: listing.seller_name,
      sender_phone: currentPhone, sender_name: currentName,
      message: input, read:false,
    });
    setInput("");
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#fff", zIndex:200, display:"flex", flexDirection:"column" }}>
      <div style={{ background:"#7D3C98", padding:"16px 16px 14px", color:"#fff", display:"flex", alignItems:"center", gap:10 }}>
        <span onClick={onClose} style={{ cursor:"pointer", fontSize:20 }}>←</span>
        <div>
          <div style={{ fontWeight:800, fontSize:15 }}>{listing.seller_name}</div>
          <div style={{ fontSize:11, opacity:0.85 }}>{listing.title}</div>
        </div>
      </div>
      <div style={{ flex:1, overflowY:"auto", padding:14, background:T.bg }}>
        {msgs.length===0 && <div style={{ textAlign:"center", color:T.textMuted, fontSize:12, marginTop:20 }}>No messages yet — say hello 👋</div>}
        {msgs.map(m=>(
          <div key={m.id} style={{ display:"flex", justifyContent:m.sender_phone===currentPhone?"flex-end":"flex-start", marginBottom:8 }}>
            <div style={{ maxWidth:"75%", padding:m.image_url?6:"9px 13px", borderRadius:14, fontSize:13, background:m.sender_phone===currentPhone?"#7D3C98":T.card, color:m.sender_phone===currentPhone?"#fff":T.text, border:m.sender_phone===currentPhone?"none":`1px solid ${T.border}` }}>
              {m.image_url ? <img src={m.image_url} alt="" onClick={()=>window.open(m.image_url,"_blank")} style={{ maxWidth:200,borderRadius:10,display:"block",cursor:"pointer" }} /> : m.message}
            </div>
          </div>
        ))}
      ...
      <div style={{ display:"flex", gap:8, padding:12, borderTop:`1px solid ${T.border}`, background:"#fff" }}>
        ...
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter") send(); }} placeholder="Type a message..."
          style={{ flex:1, padding:12, borderRadius:20, border:`1px solid ${T.border}`, fontSize:14 }} />
        <button onClick={send} disabled={sending||!input.trim()} style={{ padding:"0 18px", background:"#7D3C98", color:"#fff", border:"none", borderRadius:20, fontWeight:700, cursor:"pointer" }}>Send</button>
      </div>
    </div>
  );
}
...
function UsafiriScreen({ userName, phone, town, showToast, onBack, lang }) {
  ...
  const [vMarketId, setVMarketId] = useState(null);
  const [vMarketName, setVMarketName] = useState("");
  ...
  async function post() {
    ...
    const { error } = await sb.from("listings").insert({
      ...
      photos: vPhotos,
      market_id: vMarketId, market_name: vMarketName||null,
    ...
    if(error) showToast("Error posting. Try again.","error");
      ...
      setVMarketId(null); setVMarketName("");
      await load();
    }
    setPosting(false);
  }
      ...
      <div>
        ...
        <div style={{ padding:16 }}>
          ...
          <div style={{ background:T.card,borderRadius:16,padding:16,marginBottom:12,border:`1px solid ${T.border}` }}>
            ...
            {selected.market_id ? (
              <div style={{ background:T.successSoft,borderRadius:10,padding:"8px 14px",marginBottom:10,fontSize:12,color:T.success,fontWeight:700 }}>
                🏬 Tagged to {selected.market_name||"a market"}
              </div>
            ) : (
              <div style={{ background:T.surface,borderRadius:10,padding:"8px 14px",marginBottom:10,fontSize:12,color:T.textMuted }}>
                🏬 Not tagged to any My Market page — won't appear there
              </div>
            )}
          ...
          {selected.usafiri_available ? (
            <>
              {selected.seller_phone !== phone ? (
                <>
                  <div style={{ background:T.successSoft,borderRadius:12,padding:12,marginBottom:16,fontSize:12,color:"#145A36",lineHeight:1.5 }}>
                    📞 This rider is available now. Chat, call, or WhatsApp directly.
                  </div>
                  <button onClick={()=>setChatListing(selected)} style={{ width:"100%",padding:15,background:T.primary,color:"#fff",border:"none",borderRadius:13,fontSize:15,fontWeight:800,cursor:"pointer",marginBottom:10 }}>
                    💬 Chat with {selected.seller_name}
                  </button>
                ...
                <div style={{ background:T.successSoft,borderRadius:12,padding:12,marginBottom:6,fontSize:12,color:"#145A36",lineHeight:1.5,textAlign:"center" }}>
                  ✅ Your profile is live and showing as available.
                </div>
              )}
            </>
          ) : (
            <div style={{ background:T.coralSoft,borderRadius:12,padding:14,textAlign:"center",color:T.coral,fontWeight:700,marginBottom:selected.seller_phone!==phone?10:0 }}>
              ⛔ This rider is currently off duty. Try another one.
            </div>
          )}

          {!selected.usafiri_available && selected.seller_phone !== phone && (
            <button onClick={()=>setChatListing(selected)} style={{ width:"100%",padding:14,background:T.primary,color:"#fff",border:"none",borderRadius:13,fontSize:14,fontWeight:800,cursor:"pointer" }}>
              💬 Chat with {selected.seller_name}
            </button>
          )}

          {/* Own listing controls — toggle only, no delete for registered transport profiles */}
          {(selected.seller_phone === phone || selected.created_by_phone === phone) && (
            <div style={{ marginTop:16 }}>
              <button onClick={()=>toggleAvailability(selected)}
                style={{ width:"100%",padding:12,background:selected.usafiri_available?T.coral:T.success,color:"#fff",border:"none",borderRadius:12,fontSize:13,fontWeight:700,cursor:"pointer" }}>
        ...
        {chatListing && (
          <UsafiriChatThread listing={chatListing} currentPhone={phone} currentName={userName} onClose={()=>setChatListing(null)} />
        )}
      </div>
    );
  }
      ...
      {showForm && (
        <div style={{ position:"fixed",inset:0,background:T.overlay,zIndex:150,display:"flex",alignItems:"flex-end",justifyContent:"center" }} onClick={()=>setShowForm(false)}>
          <div style={{ background:T.card,borderRadius:"22px 22px 0 0",padding:"24px 20px 36px",width:"100%",maxWidth:430,maxHeight:"88vh",overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            ...
            <MarketPicker marketId={vMarketId} marketName={vMarketName} onPick={(id,name)=>{ setVMarketId(id); setVMarketName(name); }} />
...
function FundiScreen({ userName, phone, town, showToast, onBack, lang }) {
  ...
  const [loading, setLoading]     = useState(false);
  const [selected, setSelected]   = useState(null);
  const [fundiChat, setFundiChat] = useState(null);
  ...
  const [posting, setPosting]                 = useState(false);
  const [fMarketId, setFMarketId] = useState(null);
  const [fMarketName, setFMarketName] = useState("");
  ...
  async function post() {
    ...
    const { error } = await sb.from("listings").insert({
      ...
      photos: fPhotos,
      market_id: fMarketId, market_name: fMarketName||null,
    });
    if(error) { showToast("Error posting. Try again.","error"); }
    else {
      ...
      setFMarketId(null); setFMarketName("");
      await load();
    }
    setPosting(false);
  }
    ...
    return (
      <div>
        ...
        <div style={{ padding:16 }}>
          ...
          {selected.seller_phone !== phone && (
            <>
              <Btn onClick={()=>setFundiChat(selected)}>💬 Contact This Fundi</Btn>
              <a href={`tel:${selected.seller_phone}`} style={{ textDecoration:"none" }}>
                <button style={{ width:"100%",padding:13,marginTop:10,background:T.successSoft,color:T.success,border:"none",borderRadius:13,fontSize:14,fontWeight:800,cursor:"pointer" }}>
                  📞 Call {selected.seller_name}
                </button>
              </a>
            </>
          ...
          {selected.seller_phone === phone && (
            <div style={{ display:"flex",gap:8 }}>
              <button onClick={async()=>{
                await sb.from("listings").update({ fundi_available: !selected.fundi_available }).eq("id",selected.id);
                showToast(selected.fundi_available ? "Marked as busy" : "Marked as available");
                setSelected(null); await load();
              }} style={{ flex:1,padding:13,background:selected.fundi_available?T.coral:T.success,color:"#fff",border:"none",borderRadius:13,fontSize:13,fontWeight:800,cursor:"pointer" }}>
                {selected.fundi_available ? "Mark Busy" : "Mark Available"}
              </button>
              <button onClick={async()=>{
                if(window.confirm("Delete this profile?")) {
                  const { error } = await sb.from("listings").delete().eq("id",selected.id);
                  if (error) { showToast("Delete failed: " + error.message, "error"); return; }
                  showToast("Profile deleted."); setSelected(null); await load();
                }
              }} style={{ flex:1,padding:13,background:T.coral,color:"#fff",border:"none",borderRadius:13,fontSize:13,fontWeight:800,cursor:"pointer" }}>
                🗑️ Delete
              </button>
            </div>
          )}
        ...
        {fundiChat && (
          <SimpleChatThread chatKey={`fundi-${selected.id}`} title={cat?.label||selected.fundi_custom_spec||"Fundi"} otherName={selected.seller_name} otherPhone={selected.seller_phone}
            currentPhone={phone} currentName={userName} themeColor="#1A5276" onClose={()=>setFundiChat(null)} />
        )}
      </div>
    );
  }
      ...
      {showForm && (
        <div style={{ position:"fixed",inset:0,background:T.overlay,zIndex:150,display:"flex",alignItems:"flex-end",justifyContent:"center" }} onClick={()=>setShowForm(false)}>
          <div style={{ background:T.card,borderRadius:"22px 22px 0 0",padding:"24px 20px 36px",width:"100%",maxWidth:430,maxHeight:"88vh",overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            ...
            <MarketPicker marketId={fMarketId} marketName={fMarketName} onPick={(id,name)=>{ setFMarketId(id); setFMarketName(name); }} />
...
function PublicKiosk({ phone, onClose }) {
  ...
  useEffect(()=>{
    (async ()=>{
      const { data:matches } = await sb.from("kiosks").select("*").or(`slug.eq.${phone},phone.eq.${phone}`).limit(1);
      const k = matches && matches[0];
      if (!k) { setNotFound(true); setLoading(false); return; }
      const { data:l } = await sb.from("listings").select("*").eq("seller_phone",k.phone).order("created_at",{ ascending:false }).limit(30);
...
const NITUME_CATEGORIES = [
  ...
  { id:"market_guide", label:"🧭 Market Guide" },
...
function RentTrackerScreen({ town, showToast, onBack, userName, phone }) {
  ...
  const [teamContexts, setTeamContexts] = useState([]); // [{owner_phone, owner_name, role}]
  const [activeContext, setActiveContext] = useState(null); // null = own properties, else a teamContexts entry

  useEffect(() => { detectRole(); }, []);

  async function detectRole() {
    setLoading(true);
    const [{ data: props }, { data: tenancies }, { data: team }] = await Promise.all([
      ...
      sb.from("rt_team_members").select("owner_phone,role").eq("member_phone", phone).eq("status", "active"),
    ]);
    ...
    let contexts = [];
    if ((team||[]).length) {
      const ownerPhones = [...new Set(team.map(t=>t.owner_phone))];
      const { data: owners } = await sb.from("users").select("phone,name").in("phone", ownerPhones);
      const nameMap = {}; (owners||[]).forEach(o=>{ nameMap[o.phone]=o.name; });
      contexts = team.map(t=>({ owner_phone:t.owner_phone, owner_name:nameMap[t.owner_phone]||t.owner_phone, role:t.role }));
    }
    ...
    else if (isTeamMember) { setRole("landlord"); setActiveContext(contexts[0]); }
  ...
  return (
    <div>
      ...
      {role === "landlord" && (hasLandlordData ? teamContexts.length>0 : teamContexts.length>1) && (
        <div style={{ padding:"0 16px", marginBottom:12 }}>
          <div style={{ fontSize:11, color:T.textMuted, marginBottom:6, fontWeight:700 }}>Managing:</div>
          <div style={{ display:"flex", gap:6, overflowX:"auto" }}>
            {hasLandlordData && (
              <div onClick={()=>setActiveContext(null)} style={{ flexShrink:0, background:!activeContext?T.primary:T.card, color:!activeContext?"#fff":T.text, border:`1px solid ${!activeContext?T.primary:T.border}`, borderRadius:20, padding:"7px 14px", fontSize:12, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap" }}>My Properties</div>
            )}
            {teamContexts.map(ctx=>(
              <div key={ctx.owner_phone} onClick={()=>setActiveContext(ctx)} style={{ flexShrink:0, background:activeContext?.owner_phone===ctx.owner_phone?T.primary:T.card, color:activeContext?.owner_phone===ctx.owner_phone?"#fff":T.text, border:`1px solid ${activeContext?.owner_phone===ctx.owner_phone?T.primary:T.border}`, borderRadius:20, padding:"7px 14px", fontSize:12, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap" }}>{ctx.owner_name}</div>
            ))}
          </div>
        </div>
      )}
      {role === "landlord"
        ? <LandlordRentView phone={phone} landlordPhone={activeContext?activeContext.owner_phone:phone} teamRole={activeContext?activeContext.role:null} userName={userName} showToast={showToast} onDataChange={detectRole} />
...
// ─── LANDLORD VIEW ───────────────────────────────────────────────────────────
function LandlordRentView({ phone, landlordPhone, teamRole, userName, showToast, onDataChange }) {
  landlordPhone = landlordPhone || phone; // owner acting on their own data by default
  const isOwner = !teamRole;
  const can = {
    properties: isOwner || teamRole==="property_manager",
    payments:   isOwner || teamRole==="property_manager" || teamRole==="accountant",
    maintenance:isOwner || teamRole==="property_manager" || teamRole==="caretaker",
    settings:   isOwner || teamRole==="property_manager",
    team:       isOwner,
  };
  const [view, setView]         = useState("dashboard"); // dashboard | property | inbox | maintenance | team
  const [properties, setProperties] = useState([]);
  ...
  const [tmPhone, setTmPhone] = useState(""); const [tmName, setTmName] = useState(""); const [tmRole, setTmRole] = useState("property_manager");
  ...
  const [bulkSelected, setBulkSelected] = useState({}); // tenancyId -> bool
  const [bulkSending, setBulkSending] = useState(false);

  const [showTenantHistory, setShowTenantHistory] = useState(false);
  const [tenantHistory, setTenantHistory] = useState(null); // { tenancies:[], ledgerByTenancy:{} }
  const [tenantHistoryLoading, setTenantHistoryLoading] = useState(false);
  const [pdfGenerating, setPdfGenerating] = useState(false);

  const [showImport, setShowImport] = useState(false);
  const [importRows, setImportRows] = useState([]); // parsed rows with status
  ...
  async function loadAll() {
    const { data: props } = await sb.from("rt_properties").select("*").eq("landlord_phone", landlordPhone).order("created_at", { ascending:false });
    setProperties(props || []);
    const { data: allUnits } = await sb.from("rt_units").select("*").eq("landlord_phone", landlordPhone);
    const grouped = {};
    (allUnits || []).forEach(u => { (grouped[u.property_id] ||= []).push(u); });
    setUnits(grouped);
    const { data: inbox } = await sb.from("rt_ledger").select("*").eq("landlord_phone", landlordPhone).eq("status", "pending").eq("entry_type", "payment_submission").order("submitted_at", { ascending:false });
    setPendingInbox(inbox || []);
    const { data: maint } = await sb.from("rt_maintenance_requests").select("*").eq("landlord_phone", landlordPhone).neq("status", "closed").order("created_at", { ascending:false });
    setMaintenance(maint || []);
    const { data: userRow } = await sb.from("users").select("rent_pro_until").eq("phone", landlordPhone).single();
    setRentProUntil(userRow?.rent_pro_until || null);
    if (isOwner) {
      const { data: team } = await sb.from("rt_team_members").select("*").eq("owner_phone", landlordPhone).eq("status", "active").order("created_at", { ascending:false });
      setTeamMembers(team || []);
    }
    ...
    const { data: tenancyRows } = await sb.from("rt_tenancies").select("*").eq("landlord_phone", landlordPhone).eq("status", "active");
    setActiveTenanciesForHealth(tenancyRows || []);
    const monthStart = new Date(); monthStart.setDate(1); monthStart.setHours(0,0,0,0);
    const { data: monthLedger } = await sb.from("rt_ledger").select("*").eq("landlord_phone", landlordPhone).eq("status", "confirmed").gte("submitted_at", monthStart.toISOString());
  ...
  async function logAudit(actionType, description, tenancyId, unitId) {
    try {
      await sb.from("rt_audit_log").insert({ landlord_phone: landlordPhone, actor_phone: phone, action_type: actionType, description, tenancy_id: tenancyId || null, unit_id: unitId || null });
    } catch(e) { /* audit logging is best-effort, never blocks the real action */ }
  }

  async function loadSettings() {
    const { data } = await sb.from("rt_settings").select("*").eq("landlord_phone", landlordPhone).single();
    if (data) {
      setSettings(data);
  ...
  async function saveSettings() {
    ...
    const payload = {
      landlord_phone: landlordPhone, currency: stCurrency, grace_period_days: parseInt(stGraceDays)||0,
  ...
  async function loadAuditLog() {
    const { data } = await sb.from("rt_audit_log").select("*").eq("landlord_phone", landlordPhone).order("created_at", { ascending:false }).limit(100);
    setAuditEntries(data || []);
    setShowAuditLog(true);
  }

  async function inviteTeamMember() {
    if(!tmPhone.trim()||!tmName.trim()) { showToast("Enter a name and phone number","error"); return; }
    const cleanPhone = tmPhone.replace(/\D/g,"");
    if(cleanPhone === landlordPhone.replace(/\D/g,"")) { showToast("That's your own number","error"); return; }
    setTmSaving(true);
    const { error } = await sb.from("rt_team_members").insert({
      owner_phone: landlordPhone, member_phone: cleanPhone, member_name: tmName, role: tmRole, status: "active",
    });
    setTmSaving(false);
    if(error) { showToast(error.code==="23505" ? "Already on your team" : "Couldn't add — try again","error"); return; }
    await sb.from("notifications").insert({
      recipient_phone: cleanPhone, icon: "👥", read: false,
      title: "Added to a property team",
      body: `${userName} added you as ${ROLE_LABELS[tmRole]} on KaziApa Rent Tracker. Open Rent Tracker to get started.`,
    });
    logAudit("team_member_added", `Added ${tmName} (${cleanPhone}) as ${ROLE_LABELS[tmRole]}`);
    showToast(`${tmName} added to your team`);
    setShowInviteTeam(false); setTmPhone(""); setTmName(""); setTmRole("property_manager");
    loadAll();
  }

  async function removeTeamMember(tm) {
    if(!window.confirm(`Remove ${tm.member_name} from your team?`)) return;
    await sb.from("rt_team_members").update({ status:"removed" }).eq("id", tm.id);
    logAudit("team_member_removed", `Removed ${tm.member_name} (${tm.member_phone})`);
    showToast("Removed from team");
    loadAll();
  }

  async function changeTeamMemberRole(tm, newRole) {
    await sb.from("rt_team_members").update({ role:newRole }).eq("id", tm.id);
    logAudit("team_member_role_changed", `Changed ${tm.member_name}'s role to ${ROLE_LABELS[newRole]}`);
  ...
  async function addProperty() {
    ...
    setSaving(true);
    await sb.from("rt_properties").insert({ landlord_phone: landlordPhone, name: npName, town: npTown, area: npArea });
  ...
  async function addUnit() {
    ...
    setSaving(true);
    const { data: uRow } = await sb.from("rt_units").insert({ property_id: activeProperty.id, landlord_phone: landlordPhone, unit_label: nuLabel, rent_amount: parseFloat(nuRent), due_day: parseInt(nuDueDay) || 5, status: "vacant" }).select().single();
  ...
  async function openUnit(unit) {
    setActiveUnit(unit);
    setDocuments([]); setShowAddDoc(false);
    setComms([]); setShowAddComm(false);
    if (unit.status === "occupied") {
      ...
      if (data) {
        ...
        setLedger(entries || []);
        const { data: docs } = await sb.from("rt_documents").select("*").eq("tenancy_id", data.id).order("created_at", { ascending:false });
        setDocuments(docs || []);
        const { data: log } = await sb.from("rt_communications").select("*").eq("tenancy_id", data.id).order("created_at", { ascending:false });
        setComms(log || []);
      }
    } else {
  ...
  async function addComm() {
    if (!commNote.trim()) { showToast("Add a note first", "error"); return; }
    setCommSaving(true);
    const { error } = await sb.from("rt_communications").insert({
      tenancy_id: tenancy.id, unit_id: activeUnit.id, landlord_phone: landlordPhone,
      comm_type: commType, note: commNote.trim(), logged_by_phone: phone, logged_by_name: userName,
    });
    setCommSaving(false);
    if (error) { showToast("Save failed: " + error.message, "error"); return; }
    showToast("Logged");
    setCommNote(""); setCommType("call"); setShowAddComm(false);
    const { data: log } = await sb.from("rt_communications").select("*").eq("tenancy_id", tenancy.id).order("created_at", { ascending:false });
    setComms(log || []);
  }

  async function addDocument(file) {
    if (!docLabel.trim()) { showToast("Add a label first (e.g. \"Lease Agreement 2026\")", "error"); return; }
    setDocUploading(true);
    try {
      const compressed = await compressImageRT(file);
      const name = `rtdoc-${Date.now()}-${Math.random().toString(36).slice(2)}.jpg`;
      const { error: upErr } = await sb.storage.from("kaziapa-photos").upload(name, compressed, { contentType:"image/jpeg" });
      if (upErr) { showToast("Upload failed. Try again.", "error"); setDocUploading(false); return; }
      const { data:pub } = sb.storage.from("kaziapa-photos").getPublicUrl(name);
      const { error } = await sb.from("rt_documents").insert({
        tenancy_id: tenancy.id, unit_id: activeUnit.id, landlord_phone: landlordPhone,
        doc_type: docType, label: docLabel.trim(), file_url: pub.publicUrl,
        uploaded_by_phone: phone, uploaded_by_name: userName,
      });
      if (error) { showToast("Save failed: " + error.message, "error"); setDocUploading(false); return; }
      showToast("Document added 📄");
      setDocLabel(""); setDocType("lease"); setShowAddDoc(false);
      const { data: docs } = await sb.from("rt_documents").select("*").eq("tenancy_id", tenancy.id).order("created_at", { ascending:false });
      setDocuments(docs || []);
    } catch(err) {
      showToast("Upload failed. Try again.", "error");
    }
  ...
  async function deleteDocument(id) {
    ...
    const { error } = await sb.from("rt_documents").delete().eq("id", id);
    if (error) { showToast("Delete failed: " + error.message, "error"); return; }
  ...
  async function assignTenant() {
    ...
    const { data: tRow } = await sb.from("rt_tenancies").insert({
      unit_id: activeUnit.id, landlord_phone: landlordPhone, tenant_phone: atPhone.replace(/\D/g,""), tenant_name: atName,
  ...
  async function openTenantHistory(tenantPhone) {
    setTenantHistoryLoading(true);
    setShowTenantHistory(true);
    const { data: allT } = await sb.from("rt_tenancies").select("*, rt_units(unit_label, rt_properties(name))").eq("landlord_phone", landlordPhone).eq("tenant_phone", tenantPhone).order("started_at", { ascending:false });
    const tenancies = allT || [];
    const ledgerByTenancy = {};
    for (const t of tenancies) {
      const { data: entries } = await sb.from("rt_ledger").select("*").eq("tenancy_id", t.id);
      const paid = (entries||[]).filter(e=>e.status==="confirmed" && ["payment_submission","landlord_payment","credit","discount"].includes(e.entry_type)).reduce((s,e)=>s+Number(e.amount||0),0);
      const disputes = (entries||[]).filter(e=>e.status==="disputed").length;
      ledgerByTenancy[t.id] = { paid, entryCount: (entries||[]).length, disputes };
    }
    setTenantHistory({ tenancies, ledgerByTenancy });
  ...
  async function shareStatementPDF() {
    ...
    try {
      ...
      doc.text(settings?.receipt_branding || "Rent Statement", 14, y); y += 8;
      doc.setFontSize(10); doc.setFont(undefined,"normal");
      doc.text(`Generated: ${new Date().toLocaleDateString()}`, 14, y); y += 10;

      doc.setFontSize(11); doc.setFont(undefined,"bold");
      doc.text(`Tenant: ${tenancy.tenant_name}`, 14, y); y += 6;
      doc.setFont(undefined,"normal");
      doc.text(`Phone: ${tenancy.tenant_phone}`, 14, y); y += 6;
      doc.text(`Unit: ${activeUnit.unit_label}${activeProperty ? " — " + activeProperty.name : ""}`, 14, y); y += 6;
      doc.text(`Rent: Ksh ${tenancy.rent_amount} · Due day ${tenancy.due_day}`, 14, y); y += 10;

      doc.setFont(undefined,"bold");
      doc.text("Date", 14, y); doc.text("Type", 55, y); doc.text("Amount", 110, y); doc.text("Status", 150, y);
      y += 5; doc.line(14, y, 196, y); y += 6;
      doc.setFont(undefined,"normal"); doc.setFontSize(9);

      const sorted = [...ledger].sort((a,b)=>new Date(a.submitted_at)-new Date(b.submitted_at));
      let totalPaid = 0;
      for (const e of sorted) {
        if (y > 275) { doc.addPage(); y = 20; }
        const d = new Date(e.payment_date || e.submitted_at).toLocaleDateString();
        doc.text(d, 14, y);
        doc.text(String(e.entry_type||"").replace(/_/g," "), 55, y);
        doc.text(`Ksh ${e.amount}`, 110, y);
        doc.text(e.status||"", 150, y);
        if (e.status === "confirmed" && ["payment_submission","landlord_payment","credit","discount"].includes(e.entry_type)) totalPaid += Number(e.amount||0);
        y += 6;
      }
      y += 4; doc.line(14, y, 196, y); y += 8;
      doc.setFont(undefined,"bold"); doc.setFontSize(11);
      doc.text(`Total Confirmed Paid: Ksh ${totalPaid}`, 14, y);

      const blob = doc.output("blob");
      const fileName = `Rent-Statement-${tenancy.tenant_name.replace(/\s+/g,"-")}.pdf`;
      const file = new File([blob], fileName, { type: "application/pdf" });
      if (navigator.canShare && navigator.canShare({ files:[file] })) {
        await navigator.share({ files:[file], title:"Rent Statement", text:`Rent statement for ${tenancy.tenant_name}` });
      } else {
        doc.save(fileName);
        showToast("PDF downloaded — share it via WhatsApp from your Downloads.");
      }
    } catch(err) {
  ...
  function ensureXLSX() {
    return new Promise((resolve, reject) => {
      ...
      const existing = document.getElementById("xlsx-cdn");
      if (existing) { existing.addEventListener("load", ()=>resolve()); existing.addEventListener("error", reject); return; }
      const s = document.createElement("script");
      s.id = "xlsx-cdn";
  ...
  function importField(row, ...aliases) {
  ...
  async function parseImportFile(file) {
    ...
    try {
      ...
      const rows = raw.map(r => {
        const property = importField(r, "property","property name");
        const unit = importField(r, "unit","unit label","house","house number");
        const rent = importField(r, "rent","rent amount","rent (ksh)");
        const dueDay = importField(r, "due day","due date","due");
        const tenantName = importField(r, "tenant name","tenant","name");
        const tenantPhone = importField(r, "tenant phone","phone","tenant phone number");
        const deposit = importField(r, "deposit","deposit amount");
        let error = null;
        if (!property) error = "Missing Property";
        else if (!unit) error = "Missing Unit";
        else if (!rent || isNaN(parseFloat(rent))) error = "Missing/invalid Rent";
        return { property, unit, rent, dueDay, tenantName, tenantPhone, deposit, status: error ? "error" : "pending", error };
      });
      setImportRows(rows);
      if (rows.length === 0) showToast("No rows found in that file", "error");
    } catch(err) {
      showToast("Couldn't read that file. Use .xlsx or .csv", "error");
    }
  ...
  async function runImport() {
    ...
    properties.forEach(p => { propertyIdByName[p.name.trim().toLowerCase()] = p.id; });
    const updated = [...importRows];
    for (let i = 0; i < updated.length; i++) {
      const row = updated[i];
      if (row.status === "error") continue;
      try {
        const key = row.property.trim().toLowerCase();
        let propertyId = propertyIdByName[key];
        if (!propertyId) {
          const { data: newProp, error: propErr } = await sb.from("rt_properties").insert({ landlord_phone: landlordPhone, name: row.property, town: "" }).select().single();
          if (propErr) { row.status = "error"; row.error = "Property: " + propErr.message; setImportRows([...updated]); continue; }
          propertyId = newProp.id;
          propertyIdByName[key] = propertyId;
        }
        const { data: newUnit, error: unitErr } = await sb.from("rt_units").insert({
          property_id: propertyId, landlord_phone: landlordPhone, unit_label: row.unit,
          rent_amount: parseFloat(row.rent), due_day: parseInt(row.dueDay) || 5, status: "vacant",
        }).select().single();
        if (unitErr) { row.status = "error"; row.error = "Unit: " + unitErr.message; setImportRows([...updated]); continue; }

        if (row.tenantName && row.tenantPhone) {
          const { error: tErr } = await sb.from("rt_tenancies").insert({
            unit_id: newUnit.id, landlord_phone: landlordPhone,
            tenant_phone: row.tenantPhone.replace(/\D/g,""), tenant_name: row.tenantName,
            rent_amount: parseFloat(row.rent), due_day: parseInt(row.dueDay) || 5,
            deposit_amount: parseFloat(row.deposit) || 0, status: "active",
          });
          if (tErr) { row.status = "error"; row.error = "Tenant: " + tErr.message; setImportRows([...updated]); continue; }
          await sb.from("rt_units").update({ status:"occupied" }).eq("id", newUnit.id);
        }
        row.status = "success"; row.error = null;
      } catch(err) {
        row.status = "error"; row.error = "Unexpected error";
      }
  ...
  async function openReceivedOutside() {
    const { data } = await sb.from("rt_tenancies").select("*, rt_units(unit_label, rt_properties(name))").eq("landlord_phone", landlordPhone).eq("status", "active");
  ...
  if (activeUnit) {
    return (
      <div style={{ padding:"0 16px 30px" }}>
        ...
        {showTenantHistory && (
          <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:100, display:"flex", alignItems:"flex-end" }} onClick={()=>setShowTenantHistory(false)}>
            <div style={{ background:"#fff", borderRadius:"18px 18px 0 0", padding:20, width:"100%", maxHeight:"85vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
              <div style={{ fontWeight:800, fontSize:16, color:T.text, marginBottom:4 }}>👤 Tenant History</div>
              {tenantHistoryLoading ? (
                <div style={{ padding:20, textAlign:"center", color:T.textMuted }}>Loading...</div>
              ) : !tenantHistory || tenantHistory.tenancies.length===0 ? (
                <div style={{ padding:20, textAlign:"center", color:T.textMuted }}>No tenancy records found.</div>
              ) : (
                <>
                  <div style={{ fontSize:12, color:T.textMuted, marginBottom:14 }}>{tenantHistory.tenancies.length} tenanc{tenantHistory.tenancies.length===1?"y":"ies"} across your properties</div>
                  {tenantHistory.tenancies.map(t => {
                    const led = tenantHistory.ledgerByTenancy[t.id] || { paid:0, entryCount:0, disputes:0 };
                    return (
                      <div key={t.id} style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:12, padding:14, marginBottom:10 }}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                          <div>
                            <div style={{ fontWeight:700, fontSize:13, color:T.text }}>{t.rt_units?.rt_properties?.name} — {t.rt_units?.unit_label}</div>
                            <div style={{ fontSize:11, color:T.textMuted, marginTop:2 }}>
                              {new Date(t.started_at).toLocaleDateString()} → {t.ended_at ? new Date(t.ended_at).toLocaleDateString() : "Present"}
                            </div>
                          </div>
                          <Pill color={t.status==="active"?T.success:T.textMuted} bg={t.status==="active"?T.successSoft:T.surface}>{t.status==="active"?"Current":"Past"}</Pill>
                        </div>
                        <div style={{ display:"flex", gap:14, marginTop:10, fontSize:11, color:T.textMid }}>
                          <div>💰 Ksh {led.paid} paid</div>
                          <div>📋 {led.entryCount} entries</div>
                          {led.disputes > 0 && <div style={{ color:T.coral, fontWeight:700 }}>⚠️ {led.disputes} disputed</div>}
                        </div>
                        {t.status==="ended" && t.final_inspection_notes && (
                          <div style={{ fontSize:11, color:T.textMuted, marginTop:8, borderTop:`1px solid ${T.border}`, paddingTop:8 }}>Move-out notes: {t.final_inspection_notes}</div>
                        )}
                      </div>
                    );
                  })}
              ...
              <Btn onClick={()=>setShowTenantHistory(false)} color={T.border} text={T.text} style={{ marginTop:6 }}>Close</Btn>
            </div>
          </div>
        )}
        ...
        {activeUnit.status === "vacant" && !showAssignTenant && can.properties && (
          ...
          <>
            <div style={{ background:T.primarySoft, borderRadius:14, padding:16, marginBottom:16 }}>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:10, flexWrap:"wrap", gap:8 }}>
                {can.payments && <div onClick={()=>openUpdateAccount(tenancy)} style={{ color:T.primary, fontSize:12, fontWeight:700, cursor:"pointer" }}>📝 Update Tenant Account</div>}
                {can.properties && <div onClick={()=>{ setAtEmergencyName(tenancy.emergency_contact_name||""); setAtEmergencyPhone(tenancy.emergency_contact_phone||""); setAtNextOfKin(tenancy.next_of_kin||""); setShowEditEmergency(true); }} style={{ color:T.accentDark, fontSize:12, fontWeight:700, cursor:"pointer" }}>🆘 {tenancy.emergency_contact_name ? "Edit" : "Add"} Emergency Contact</div>}
                {can.properties && <div onClick={openMoveOut} style={{ color:T.coral, fontSize:12, fontWeight:700, cursor:"pointer" }}>End Tenancy</div>}
                {can.properties && <div onClick={()=>openTenantHistory(tenancy.tenant_phone)} style={{ color:T.textMid, fontSize:12, fontWeight:700, cursor:"pointer" }}>👤 Full History</div>}
                {can.payments && <div onClick={shareStatementPDF} style={{ color:T.textMid, fontSize:12, fontWeight:700, cursor: pdfGenerating?"default":"pointer", opacity:pdfGenerating?0.6:1 }}>{pdfGenerating ? "Generating..." : "📄 Share Statement"}</div>}
              </div>
              {showEditEmergency && (
            ...
            <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:14, padding:16, marginBottom:16 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:documents.length||showAddDoc?12:0 }}>
                <div style={{ fontWeight:700, color:T.text, fontSize:14 }}>📄 Documents</div>
                {can.properties && !showAddDoc && <div onClick={()=>setShowAddDoc(true)} style={{ color:T.primary, fontSize:12, fontWeight:700, cursor:"pointer" }}>+ Add Document</div>}
              </div>
              {showAddDoc && (
                <div style={{ marginBottom:12, paddingBottom:12, borderBottom:`1px solid ${T.border}` }}>
                  <select style={S} value={docType} onChange={e=>setDocType(e.target.value)}>
                    <option value="lease">Lease Agreement</option>
                    <option value="id">ID / KRA PIN Copy</option>
                    <option value="inspection">Inspection Report</option>
                    <option value="other">Other</option>
                  </select>
                  <input style={S} placeholder={`Label (e.g. "Lease Agreement 2026")`} value={docLabel} onChange={e=>setDocLabel(e.target.value)} />
                  <div style={{ display:"flex", gap:8, marginBottom:8 }}>
                    <label style={{ flex:1, textAlign:"center", padding:"8px 0", background:T.surface, border:`1px solid ${T.border}`, borderRadius:9, fontSize:12, fontWeight:700, color:T.text, cursor:"pointer" }}>
                      📸 Take Photo
                      <input type="file" accept="image/*" capture="environment" onChange={e=>e.target.files[0] && addDocument(e.target.files[0])} style={{ display:"none" }} />
                    </label>
                    <label style={{ flex:1, textAlign:"center", padding:"8px 0", background:T.surface, border:`1px solid ${T.border}`, borderRadius:9, fontSize:12, fontWeight:700, color:T.text, cursor:"pointer" }}>
                      🖼️ Gallery / File
                      <input type="file" accept="image/*" onChange={e=>e.target.files[0] && addDocument(e.target.files[0])} style={{ display:"none" }} />
                    </label>
                  </div>
                  {docUploading && <div style={{ fontSize:12, color:T.textMuted, marginBottom:8 }}>Uploading...</div>}
                  <div onClick={()=>{ setShowAddDoc(false); setDocLabel(""); }} style={{ color:T.textMuted, fontSize:12, fontWeight:700, cursor:"pointer", textAlign:"center" }}>Cancel</div>
                </div>
              )}
              {documents.length === 0 && !showAddDoc ? (
                <div style={{ fontSize:12, color:T.textMuted }}>No documents uploaded yet.</div>
              ) : (
                documents.map(d => {
                  const typeLabel = { lease:"📜 Lease", id:"🪪 ID/KRA", inspection:"🔍 Inspection", other:"📎 Other" }[d.doc_type] || "📎";
                  return (
                    <a key={d.id} href={d.file_url} target="_blank" rel="noreferrer" style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", textDecoration:"none", borderBottom:`1px solid ${T.border}` }}>
                      <img src={d.file_url} style={{ width:44, height:44, objectFit:"cover", borderRadius:8, flexShrink:0 }} />
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontSize:12, fontWeight:700, color:T.text, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{d.label}</div>
                        <div style={{ fontSize:10, color:T.textMuted }}>{typeLabel} · {new Date(d.created_at).toLocaleDateString()}</div>
                      </div>
                      {can.properties && <span onClick={(e)=>{ e.preventDefault(); deleteDocument(d.id); }} style={{ color:T.coral, fontSize:11, fontWeight:700, cursor:"pointer", flexShrink:0 }}>Delete</span>}
                    </a>
                  );
                })
              )}
            ...
            <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:14, padding:16, marginBottom:16 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:comms.length||showAddComm?12:0 }}>
                <div style={{ fontWeight:700, color:T.text, fontSize:14 }}>💬 Communication Timeline</div>
                {can.maintenance && !showAddComm && <div onClick={()=>setShowAddComm(true)} style={{ color:T.primary, fontSize:12, fontWeight:700, cursor:"pointer" }}>+ Log</div>}
              </div>
              {showAddComm && (
                <div style={{ marginBottom:12, paddingBottom:12, borderBottom:`1px solid ${T.border}` }}>
                  <select style={S} value={commType} onChange={e=>setCommType(e.target.value)}>
                  ...
                  <textarea style={{ ...S, resize:"none" }} rows={3} placeholder="What was discussed / sent..." value={commNote} onChange={e=>setCommNote(e.target.value)} />
                  <div style={{ display:"flex", gap:10 }}>
                    <Btn onClick={addComm} disabled={commSaving} style={{ flex:1 }}>{commSaving ? "Saving..." : "Save"}</Btn>
                    <Btn onClick={()=>{ setShowAddComm(false); setCommNote(""); }} color={T.border} text={T.text} style={{ flex:1 }}>Cancel</Btn>
                  </div>
                </div>
              )}
              {comms.length === 0 && !showAddComm ? (
                <div style={{ fontSize:12, color:T.textMuted }}>No communication logged yet.</div>
              ) : (
                comms.map(c => {
                  const typeLabel = { call:"📞 Call", sms:"💬 SMS", whatsapp:"🟢 WhatsApp", notice:"📋 Notice", in_person:"🤝 In-Person", other:"Other" }[c.comm_type] || "Other";
                  return (
                    <div key={c.id} style={{ padding:"8px 0", borderBottom:`1px solid ${T.border}` }}>
                      <div style={{ fontSize:11, color:T.textMuted, marginBottom:2 }}>{typeLabel} · {new Date(c.created_at).toLocaleDateString()} · {c.logged_by_name || c.logged_by_phone}</div>
                      <div style={{ fontSize:12, color:T.text }}>{c.note}</div>
                    </div>
                  );
                })
              )}
              ...
              return (
                <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:14, padding:16, marginBottom:16 }}>
                  ...
                  <div style={{ display:"flex", gap:14, marginTop:12 }}>
                    {can.payments && <div onClick={()=>{ openUpdateAccount(tenancy); setUaAction("deposit_payment"); }} style={{ color:T.primary, fontSize:12, fontWeight:700, cursor:"pointer" }}>+ Record Payment</div>}
                    {can.payments && <div onClick={()=>{ openUpdateAccount(tenancy); setUaAction("deposit_deduction"); }} style={{ color:T.coral, fontSize:12, fontWeight:700, cursor:"pointer" }}>− Deduction</div>}
                    {can.payments && <div onClick={()=>{ openUpdateAccount(tenancy); setUaAction("deposit_refund"); }} style={{ color:T.accentDark, fontSize:12, fontWeight:700, cursor:"pointer" }}>↩ Refund</div>}
                  </div>
                </div>
              );
            })()}
            ...
            {ledger.map(entry => (
              <div key={entry.id} style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:12, padding:14, marginBottom:10 }}>
                ...
                {entry.status === "pending" && can.payments && (
  ...
  if (activeProperty) {
    ...
    return (
      <div style={{ padding:"0 16px 30px" }}>
        ...
        {can.properties && <Btn onClick={()=>atUnitLimit ? setShowUpgrade(true) : setShowAddUnit(true)} style={{ marginBottom:16 }}>+ Add Unit</Btn>}
  ...
  if (view === "team") {
    return (
      <div style={{ padding:"0 16px 30px" }}>
        <div onClick={()=>setView("dashboard")} style={{ color:T.primary, fontWeight:700, fontSize:13, marginBottom:14, cursor:"pointer" }}>← Dashboard</div>
        <div style={{ fontWeight:800, fontSize:17, color:T.text, marginBottom:4 }}>Team</div>
        <div style={{ fontSize:12, color:T.textMuted, marginBottom:16 }}>Give trusted people access to help manage your properties.</div>
        <Btn onClick={()=>setShowInviteTeam(true)} style={{ marginBottom:16 }}>+ Add Team Member</Btn>
        {showInviteTeam && (
          <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:14, padding:16, marginBottom:16 }}>
            <input style={S} placeholder="Their name" value={tmName} onChange={e=>setTmName(e.target.value)} />
            <input style={S} placeholder="Their phone (07XXXXXXXX)" value={tmPhone} onChange={e=>setTmPhone(e.target.value)} />
            <div style={{ fontSize:12, fontWeight:700, color:T.text, marginBottom:6 }}>Role</div>
            {["property_manager","caretaker","accountant"].map(r=>(
              <div key={r} onClick={()=>setTmRole(r)} style={{ background:tmRole===r?T.primarySoft||T.accentSoft:T.surface, border:`1.5px solid ${tmRole===r?T.primary:T.border}`, borderRadius:12, padding:12, marginBottom:8, cursor:"pointer" }}>
                <div style={{ fontWeight:700, fontSize:13, color:T.text }}>{ROLE_LABELS[r]}</div>
                <div style={{ fontSize:11, color:T.textMuted, marginTop:2 }}>{ROLE_DESCRIPTIONS[r]}</div>
              </div>
            ))}
            <div style={{ display:"flex", gap:10, marginTop:8 }}>
              <Btn onClick={inviteTeamMember} disabled={tmSaving} style={{ flex:1 }}>{tmSaving?"Adding...":"Add to Team"}</Btn>
              <Btn onClick={()=>setShowInviteTeam(false)} color={T.border} text={T.text} style={{ flex:1 }}>Cancel</Btn>
            </div>
          </div>
        )}
        {teamMembers.length === 0 && !showInviteTeam && <div style={{ color:T.textMuted, fontSize:13, textAlign:"center", padding:30 }}>No team members yet — add one above.</div>}
        {teamMembers.map(tm => (
          <div key={tm.id} style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:12, padding:14, marginBottom:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                <div style={{ fontWeight:700, color:T.text, fontSize:14 }}>{tm.member_name}</div>
                <div style={{ fontSize:11, color:T.textMuted }}>{tm.member_phone}</div>
              </div>
              <Pill color={T.primary} bg={T.accentSoft}>{ROLE_LABELS[tm.role]}</Pill>
            </div>
            <div style={{ display:"flex", gap:14, marginTop:10 }}>
              <select value={tm.role} onChange={e=>changeTeamMemberRole(tm,e.target.value)} style={{ fontSize:12, padding:"5px 8px", borderRadius:8, border:`1px solid ${T.border}`, color:T.text, background:T.surface }}>
                <option value="property_manager">Property Manager</option>
                <option value="caretaker">Caretaker</option>
                <option value="accountant">Accountant</option>
              </select>
              <div onClick={()=>removeTeamMember(tm)} style={{ color:T.coral, fontSize:12, fontWeight:700, cursor:"pointer" }}>Remove</div>
            </div>
          </div>
        ))}
      </div>
  ...
  const unitById = {}; Object.values(units).flat().forEach(u=>{ unitById[u.id]=u; });
  const collectedByTenancy = {};
  monthLedger.filter(l=>reducesTypes.includes(l.entry_type)).forEach(l=>{
    collectedByTenancy[l.tenancy_id] = (collectedByTenancy[l.tenancy_id]||0) + Number(l.amount||0);
  });
  const todayDate = new Date().getDate();
  const overdueTenants = activeTenanciesForHealth
    .filter(t => (collectedByTenancy[t.id]||0) < Number(t.rent_amount||0) && todayDate > t.due_day)
    .map(t => ({ ...t, owed: Number(t.rent_amount||0) - (collectedByTenancy[t.id]||0), daysOverdue: todayDate - t.due_day, unit: unitById[t.unit_id] }));

  function openBulkReminder() {
    const sel = {}; overdueTenants.forEach(t => sel[t.id] = true);
    setBulkSelected(sel);
    setShowBulkReminder(true);
  }

  async function sendBulkReminders() {
    const targets = overdueTenants.filter(t => bulkSelected[t.id]);
    if (targets.length === 0) { showToast("Select at least one tenant", "error"); return; }
    setBulkSending(true);
    for (const t of targets) {
      const body = `Hi ${t.tenant_name}, this is a reminder that Ksh ${t.owed} rent is overdue (${t.daysOverdue} day${t.daysOverdue===1?"":"s"} past due day ${t.due_day}). Please make payment as soon as possible.`;
      await sb.from("notifications").insert({
        recipient_phone: t.tenant_phone, icon: "⏰", read: false,
        title: "Rent Reminder", body,
      });
      await sb.from("rt_communications").insert({
        tenancy_id: t.id, unit_id: t.unit_id, landlord_phone: landlordPhone,
        comm_type: "notice", note: `Bulk reminder sent: ${body}`, logged_by_phone: phone, logged_by_name: userName,
      });
    }
    setBulkSending(false);
    setShowBulkReminder(false);
    showToast(`Reminder sent to ${targets.length} tenant${targets.length===1?"":"s"}`);
  }

  return (
    <div style={{ padding:"0 16px 30px" }}>
      <div style={{ display:"flex", justifyContent:"flex-end", gap:16, marginBottom:10 }}>
        {can.payments && overdueTenants.length > 0 && <div onClick={openBulkReminder} style={{ fontSize:12, color:T.coral, fontWeight:700, cursor:"pointer" }}>⏰ Remind Overdue ({overdueTenants.length})</div>}
        {can.team && <div onClick={()=>setView("team")} style={{ fontSize:12, color:T.textMuted, fontWeight:700, cursor:"pointer" }}>👥 Team</div>}
        {can.settings && <div onClick={loadAuditLog} style={{ fontSize:12, color:T.textMuted, fontWeight:700, cursor:"pointer" }}>📋 Audit Log</div>}
        {can.settings && <div onClick={loadSettings} style={{ fontSize:12, color:T.textMuted, fontWeight:700, cursor:"pointer" }}>⚙️ Settings</div>}
      </div>
      {showBulkReminder && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:100, display:"flex", alignItems:"flex-end" }} onClick={()=>setShowBulkReminder(false)}>
          <div style={{ background:"#fff", borderRadius:"18px 18px 0 0", padding:20, width:"100%", maxHeight:"80vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            <div style={{ fontWeight:800, fontSize:16, color:T.text, marginBottom:4 }}>⏰ Remind Overdue Tenants</div>
            <div style={{ fontSize:12, color:T.textMuted, marginBottom:14 }}>Sends an in-app reminder (bell notification) and logs it to each tenant's Communication Timeline.</div>
            {overdueTenants.map(t => (
              <label key={t.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", borderBottom:`1px solid ${T.border}`, cursor:"pointer" }}>
                <input type="checkbox" checked={!!bulkSelected[t.id]} onChange={e=>setBulkSelected(prev=>({ ...prev, [t.id]:e.target.checked }))} />
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:T.text }}>{t.tenant_name} — {t.unit?.unit_label || ""}</div>
                  <div style={{ fontSize:11, color:T.coral }}>Ksh {t.owed} owed · {t.daysOverdue} day{t.daysOverdue===1?"":"s"} overdue</div>
                </div>
              </label>
            ))}
            <div style={{ display:"flex", gap:10, marginTop:16 }}>
              <Btn onClick={sendBulkReminders} disabled={bulkSending} style={{ flex:1 }}>{bulkSending ? "Sending..." : "Send Reminders"}</Btn>
              <Btn onClick={()=>setShowBulkReminder(false)} color={T.border} text={T.text} style={{ flex:1 }}>Cancel</Btn>
            </div>
          </div>
        </div>
      )}
      {!isOwner && (
        <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:12, padding:"8px 12px", marginBottom:12, fontSize:11, color:T.textMuted, fontWeight:700 }}>
          👥 You're managing this as {ROLE_LABELS[teamRole]}
        </div>
      )}
      {isOwner && !isPro && (
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
        ...
        <div style={{ display:"flex", gap:14 }}>
          {can.properties && <div onClick={()=>{ setShowImport(true); setImportRows([]); setImportDone(false); }} style={{ color:T.textMid, fontWeight:700, fontSize:13, cursor:"pointer" }}>📥 Import</div>}
          {can.properties && <div onClick={()=>setShowAddProperty(true)} style={{ color:T.primary, fontWeight:700, fontSize:13, cursor:"pointer" }}>+ Add</div>}
        </div>
      </div>

      {can.properties && showImport && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:100, display:"flex", alignItems:"flex-end" }} onClick={()=>{ if(!importRunning) setShowImport(false); }}>
          <div style={{ background:"#fff", borderRadius:"18px 18px 0 0", padding:20, width:"100%", maxHeight:"85vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            <div style={{ fontWeight:800, fontSize:16, color:T.text, marginBottom:4 }}>📥 Import from Excel</div>
            <div style={{ fontSize:12, color:T.textMuted, marginBottom:14 }}>
              Upload a .xlsx or .csv with columns: <b>Property</b>, <b>Unit</b>, <b>Rent</b>, Due Day, Tenant Name, Tenant Phone, Deposit. Only Property, Unit and Rent are required — leave tenant columns blank for vacant units.
            </div>
            {importRows.length === 0 && (
              <label style={{ display:"block", textAlign:"center", padding:"16px 0", background:T.surface, border:`1px dashed ${T.border}`, borderRadius:12, fontSize:13, fontWeight:700, color:T.primary, cursor:"pointer" }}>
                {importParsing ? "Reading file..." : "Choose File"}
                <input type="file" accept=".xlsx,.xls,.csv" onChange={e=>e.target.files[0] && parseImportFile(e.target.files[0])} style={{ display:"none" }} />
              </label>
            )}
            {importRows.length > 0 && (
              <>
                <div style={{ fontSize:12, color:T.textMuted, marginBottom:10 }}>
                  ...
                  {importDone && <> — {importRows.filter(r=>r.status==="success").length} imported, {importRows.filter(r=>r.status==="error").length} failed</>}
                </div>
                {importRows.map((r,i) => (
                  <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"7px 0", borderBottom:`1px solid ${T.border}` }}>
                    <div style={{ fontSize:12, color:T.text }}>
                      {r.property || "—"} · {r.unit || "—"} · Ksh {r.rent || "—"}{r.tenantName ? ` · ${r.tenantName}` : ""}
                    </div>
                    <div style={{ fontSize:11, fontWeight:700, color: r.status==="success"?T.success : r.status==="error"?T.coral : T.textMuted }}>
                      {r.status==="success" ? "✓ Done" : r.status==="error" ? r.error : "Pending"}
                    </div>
                  </div>
                ))}
                <div style={{ display:"flex", gap:10, marginTop:16 }}>
                  {!importDone && (
                    <Btn onClick={runImport} disabled={importRunning || importRows.every(r=>r.status==="error")} style={{ flex:1 }}>
                      {importRunning ? "Importing..." : `Import ${importRows.filter(r=>r.status!=="error").length} Row${importRows.filter(r=>r.status!=="error").length===1?"":"s"}`}
                    </Btn>
                  )}
                  <Btn onClick={()=>{ setShowImport(false); setImportRows([]); }} color={T.border} text={T.text} style={{ flex:1 }}>{importDone ? "Close" : "Cancel"}</Btn>
                </div>
              </>
            )}
          </div>
      ...
      {can.properties && showAddProperty && (
        <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:14, padding:16, marginBottom:16 }}>
...
function TenantRentView({ phone, userName, showToast }) {
  ...
  async function shareMyStatementPDF() {
    if (!active) return;
    setPdfGenerating(true);
    try {
      await ensureJsPDF();
      ...
      let y = 20;
      doc.setFontSize(16); doc.setFont(undefined,"bold");
      doc.text("Rent Statement", 14, y); y += 8;
      doc.setFontSize(10); doc.setFont(undefined,"normal");
      doc.text(`Generated: ${new Date().toLocaleDateString()}`, 14, y); y += 10;

      doc.setFontSize(11); doc.setFont(undefined,"bold");
      doc.text(`Tenant: ${active.tenant_name}`, 14, y); y += 6;
      doc.setFont(undefined,"normal");
      doc.text(`Phone: ${active.tenant_phone}`, 14, y); y += 6;
      doc.text(`Unit: ${active.rt_units?.unit_label || ""}${active.rt_units?.rt_properties?.name ? " — " + active.rt_units.rt_properties.name : ""}`, 14, y); y += 6;
      doc.text(`Rent: Ksh ${active.rent_amount} · Due day ${active.due_day}`, 14, y); y += 10;
      ...
      const blob = doc.output("blob");
      const fileName = `Rent-Statement-${active.tenant_name.replace(/\s+/g,"-")}.pdf`;
      const file = new File([blob], fileName, { type: "application/pdf" });
      if (navigator.canShare && navigator.canShare({ files:[file] })) {
        await navigator.share({ files:[file], title:"Rent Statement", text:"My rent statement" });
      } else {
        doc.save(fileName);
        showToast("PDF downloaded — share it via WhatsApp from your Downloads.");
      }
  ...
  return (
    <div style={{ padding:"0 16px 30px" }}>
      ...
      {tab === "balance" && (
        <>
          {ledger.length > 0 && (
            <div onClick={pdfGenerating ? undefined : shareMyStatementPDF} style={{ textAlign:"center", padding:"9px 0", marginBottom:12, background:T.card, border:`1px solid ${T.border}`, borderRadius:12, fontSize:12, fontWeight:700, color:T.primary, cursor:pdfGenerating?"default":"pointer", opacity:pdfGenerating?0.6:1 }}>
              {pdfGenerating ? "Generating..." : "📄 Share My Statement (PDF)"}
            </div>
          )}
...
function NitumeChatThread({ req, currentPhone, currentName, onClose }) {
  const isCustomer = currentPhone === req.customer_phone;
  const otherName = isCustomer ? (req.provider_name||"Provider") : (req.customer_name||"Customer");
  const chatKey = `nitume-${req.id}`;
  const [msgs, setMsgs] = useState([]);
  const [input, setInput] = useState("");
  const [sending, setSending] = useState(false);
  const [sendingImage, setSendingImage] = useState(false);
  ...
  async function send() {
    ...
    setSending(true);
    await sb.from("chats").insert({
      listing_id: chatKey, listing_title: req.title,
      buyer_phone: req.customer_phone, buyer_name: req.customer_name,
      seller_phone: req.provider_phone, seller_name: req.provider_name,
      sender_phone: currentPhone, sender_name: currentName,
      message: input, read:false,
    });
    setInput("");
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#fff", zIndex:200, display:"flex", flexDirection:"column" }}>
      <div style={{ background:`linear-gradient(130deg,${T.primary},${T.primaryMid})`, padding:"16px 16px 14px", color:"#fff", display:"flex", alignItems:"center", gap:10 }}>
        <span onClick={onClose} style={{ cursor:"pointer", fontSize:20 }}>←</span>
        <div>
          <div style={{ fontWeight:800, fontSize:15 }}>{otherName}</div>
          <div style={{ fontSize:11, opacity:0.85 }}>{req.title}</div>
        </div>
      </div>
      <div style={{ flex:1, overflowY:"auto", padding:14, background:T.bg }}>
        {msgs.length===0 && <div style={{ textAlign:"center", color:T.textMuted, fontSize:12, marginTop:20 }}>No messages yet — say hello 👋</div>}
        {msgs.map(m=>(
          <div key={m.id} style={{ display:"flex", justifyContent:m.sender_phone===currentPhone?"flex-end":"flex-start", marginBottom:8 }}>
            <div style={{ maxWidth:"75%", padding:m.image_url?6:"9px 13px", borderRadius:14, fontSize:13, background:m.sender_phone===currentPhone?T.primary:T.card, color:m.sender_phone===currentPhone?"#fff":T.text, border:m.sender_phone===currentPhone?"none":`1px solid ${T.border}` }}>
              {m.image_url ? <img src={m.image_url} alt="" onClick={()=>window.open(m.image_url,"_blank")} style={{ maxWidth:200,borderRadius:10,display:"block",cursor:"pointer" }} /> : m.message}
            </div>
          </div>
        ))}
      ...
      <div style={{ display:"flex", gap:8, padding:12, borderTop:`1px solid ${T.border}`, background:"#fff" }}>
        ...
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter") send(); }} placeholder="Type a message..."
          style={{ flex:1, padding:12, borderRadius:20, border:`1px solid ${T.border}`, fontSize:14 }} />
        <button onClick={send} disabled={sending||!input.trim()} style={{ padding:"0 18px", background:T.primary, color:"#fff", border:"none", borderRadius:20, fontWeight:700, cursor:"pointer" }}>Send</button>
      </div>
    </div>
  );
}
...
function NitumeScreen({ town, showToast, onBack, userName, phone }) {
  ...
  const [guideMarkets, setGuideMarkets] = useState([]);
  const [showGuideAdd, setShowGuideAdd] = useState(false);
  const [guidePickId, setGuidePickId] = useState(null);
  const [guidePickName, setGuidePickName] = useState("");

  useEffect(()=>{ if(tab==="browse") loadRequests(); else if(tab==="providers") loadProviders(); else loadMyNitume(); },[tab]);
  ...
  useEffect(()=>{
    ...
    loadGuideMarkets();
    // Consume a pending "Request Guide" prefill set from a market page, if any
    try {
      const raw = sessionStorage.getItem("kaziapa_pending_guide_request");
      if(raw) {
        const pending = JSON.parse(raw);
        sessionStorage.removeItem("kaziapa_pending_guide_request");
        setFCategory("market_guide");
        setFLocation(pending.marketName || "");
        setFTitle(`Need a guide at ${pending.marketName || "the market"}`);
        setFormError("Almost done — enter the Fee you're offering below, then tap Post Request.");
        setShowForm(true);
      }
    } catch(e) {}
  },[]);

  async function loadGuideMarkets() {
    const { data } = await sb.from("nitume_guide_markets").select("id,market_id,markets:market_id(name,town)").eq("phone",phone);
    if(data) setGuideMarkets(data);
  }

  async function addGuideMarket() {
    if(!guidePickId) { showToast("Pick a market first","error"); return; }
    await sb.from("nitume_guide_markets").upsert({ phone, market_id:guidePickId }, { onConflict:"phone,market_id" });
    showToast(`Added ${guidePickName} to your guide markets 🧭`);
    setGuidePickId(null); setGuidePickName(""); setShowGuideAdd(false);
    loadGuideMarkets();
  }

  async function removeGuideMarket(id) {
    await sb.from("nitume_guide_markets").delete().eq("id",id);
    loadGuideMarkets();
  }
  ...
  async function loadRequests() {
    ...
    const { data } = await sb.from("nitume_requests").select("*")
      ...
      .or(`target_provider_phone.is.null,target_provider_phone.eq.${phone}`)
  ...
  async function loadProviders() {
    ...
    let q = sb.from("users")
      .select("phone,name,town,avatar_url,nitume_rating_avg,nitume_completed_count,is_verified_shopper")
      .eq("nitume_available",true).neq("phone",phone);
    if(providerSearch.trim()) q = q.ilike("town", `%${providerSearch.trim()}%`);
    const { data } = await q
      .order("is_verified_shopper",{ ascending:false })
      .order("nitume_rating_avg",{ ascending:false })
      .limit(50);
    setProviders(data||[]);
  ...
  async function cancelNitumeRequest(id) {
    setCancelling(true);
    const { error } = await sb.from("nitume_requests").delete().eq("id",id).eq("customer_phone",phone);
    setCancelling(false);
    if(error) { showToast("Couldn't cancel — try again","error"); return; }
  ...
  async function postNitumeRequest() {
    if(!fTitle.trim()||!fLocation.trim()||!fFee) { setFormError("Please fill in: " + [!fTitle.trim()&&"what you need",!fLocation.trim()&&"location",!fFee&&"fee you're offering"].filter(Boolean).join(", ")); showToast("Fill in title, location and offered fee","error"); return; }
    setFormError("");
    setPosting(true);
    const { error } = await sb.from("nitume_requests").insert({
      ...
      target_provider_phone: targetProvider?.phone || null,
      target_provider_name: targetProvider?.name || null,
    });
    setPosting(false);
    if(error) {
      setFormError("Couldn't post: " + (error.message || "please try again."));
      showToast("Error posting request — see details above","error");
      return;
    }
    showToast(targetProvider ? `Request sent directly to ${targetProvider.name}! 🎉` : "Nitume request posted! 🎉");
  ...
  async function postAndPayNow() {
    if(!fTitle.trim()||!fLocation.trim()||!fFee) { setFormError("Please fill in: " + [!fTitle.trim()&&"what you need",!fLocation.trim()&&"location",!fFee&&"fee you're offering"].filter(Boolean).join(", ")); showToast("Fill in title, location and offered fee","error"); return; }
    setFormError("");
    setPosting(true);
    const { data:rows, error } = await sb.from("nitume_requests").insert({
      customer_phone:phone, customer_name:userName, category:fCategory,
      title:fTitle, description:fDesc, location:fLocation, offered_fee:parseInt(fFee)||0,
      target_provider_phone: targetProvider.phone,
      target_provider_name: targetProvider.name,
    }).select();
    if(error || !rows?.[0]) {
      setPosting(false);
      setFormError("Couldn't post: " + (error?.message || "please try again."));
    ...
    try {
      const res = await fetch("/api/initiate-purchase", {
        method:"POST", headers:{ "Content-Type":"application/json" },
        body: JSON.stringify({ phone, userId:phone, type:"nitume_unlock", tier:"standard", listingId:newReq.id }),
      });
      const data = await res.json();
      if(!data.success) { showToast("Couldn't start payment. Try again.","error"); setPosting(false); return; }
      let attempts=0;
      const poll = setInterval(async ()=>{
        attempts++;
        const { data:checkRows } = await sb.from("nitume_requests").select("status,provider_phone").eq("id",newReq.id).limit(1);
        if(checkRows?.[0]?.status==="accepted" && checkRows[0].provider_phone===targetProvider.phone) {
          clearInterval(poll); setPosting(false);
          showToast(`Paid & confirmed with ${targetProvider.name}! 🎉 Check My Nitume.`);
          setFTitle(""); setFDesc(""); setFLocation(""); setFFee(""); setFCategory("shopping");
          setTargetProvider(null); setPayNow(false);
          setShowForm(false); setTab("my");
        } else if(attempts>=20) {
          clearInterval(poll); setPosting(false);
          showToast("Payment not confirmed yet. Check your M-Pesa messages, or check My Nitume shortly.","error");
          setShowForm(false); setTab("my");
        }
      },3000);
    } catch(err) {
    ...
    <div style={{ minHeight:"100vh", background:T.surface, paddingBottom:90 }}>
      <div style={{ background:`linear-gradient(130deg,${T.primary},${T.primaryMid})`, padding:"16px 16px 14px", color:"#fff" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
          ...
          <button onClick={()=>{ setFormError(""); setTargetProvider(null); setPayNow(false); setShowForm(true); }} style={{ padding:"8px 16px", background:T.accent, color:T.primary, border:"none", borderRadius:20, fontSize:13, fontWeight:800, cursor:"pointer" }}>+ Post</button>
        </div>
        <div style={{ display:"flex", gap:8 }}>
          {[["browse","Browse"],["providers","Providers"],["my","My Nitume"]].map(([key,label])=>(
        ...
        {nitumeAvailable && (
          <div style={{ background:T.card,border:`1px solid ${T.border}`,borderRadius:14,padding:14,marginBottom:14 }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
              <div style={{ fontSize:13,fontWeight:800,color:T.text }}>🧭 My Guide Markets</div>
              <span onClick={()=>setShowGuideAdd(true)} style={{ color:T.primary,fontSize:12,fontWeight:700,cursor:"pointer" }}>+ Add</span>
            </div>
            <div style={{ fontSize:11,color:T.textMuted,marginBottom:10 }}>Markets where you can help newcomers navigate, find sellers, and shop. Shows up as "Nitume Verified Market Guide" on that market's page.</div>
            {guideMarkets.length===0 ? (
              <div style={{ fontSize:12,color:T.textMuted }}>Not registered as a guide for any market yet.</div>
            ) : guideMarkets.map(g=>(
              <div key={g.id} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderTop:`1px solid ${T.border}` }}>
                <div style={{ fontSize:13,color:T.text }}>{g.markets?.name || "Market"}</div>
                <span onClick={()=>removeGuideMarket(g.id)} style={{ color:T.coral,fontSize:11,fontWeight:700,cursor:"pointer" }}>Remove</span>
              </div>
            ))}
            {showGuideAdd && (
              <div style={{ marginTop:12,paddingTop:12,borderTop:`1px solid ${T.border}` }}>
                <MarketPicker marketId={guidePickId} marketName={guidePickName} onPick={(id,name)=>{ setGuidePickId(id); setGuidePickName(name); }} label="Which market?" />
                <div style={{ display:"flex",gap:8 }}>
                  <Btn onClick={addGuideMarket} style={{ flex:1 }}>Add →</Btn>
                  <button onClick={()=>{ setShowGuideAdd(false); setGuidePickId(null); setGuidePickName(""); }} style={{ padding:"11px 16px",background:"transparent",border:`1px solid ${T.border}`,borderRadius:11,color:T.textMuted,fontSize:13,cursor:"pointer" }}>Cancel</button>
                </div>
              </div>
            )}
          </div>
      ...
      <div style={{ padding:"0 16px 16px" }}>
        {loading && <div style={{ textAlign:"center", padding:30, color:T.textMuted }}>Loading...</div>}
        {loadingProviders && tab==="providers" && <div style={{ textAlign:"center", padding:30, color:T.textMuted }}>Loading...</div>}
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:4 }}>
                    <span style={{ fontSize:11, color:T.primary, fontWeight:700 }}>{catLabel(r.category)}</span>
                    {r.target_provider_phone===phone && <span style={{ fontSize:10, fontWeight:800, color:T.accentDark, background:T.accentSoft, borderRadius:6, padding:"1px 6px" }}>🎯 Direct for you</span>}
                  </div>
        ...
        {!loadingProviders && tab==="providers" && (
          <>
            <div style={{ display:"flex", gap:8, marginBottom:14 }}>
              <input value={providerSearch} onChange={e=>setProviderSearch(e.target.value)}
                onKeyDown={e=>{ if(e.key==="Enter") loadProviders(); }}
                placeholder="Search by town or place (e.g. CBD)"
                style={{ flex:1, padding:11, borderRadius:11, border:`1px solid ${T.border}`, fontSize:13 }} />
              <button onClick={loadProviders} style={{ padding:"0 16px", background:T.primary, color:"#fff", border:"none", borderRadius:11, fontSize:13, fontWeight:700, cursor:"pointer" }}>Search</button>
            </div>
            {providers.length===0 ? (
              <div style={{ textAlign:"center", padding:40, color:T.textMuted }}>
                <div style={{ fontSize:36, marginBottom:10 }}>🧍</div>
                <div style={{ fontWeight:700, color:T.text, marginBottom:4 }}>No one's available right now</div>
                <div style={{ fontSize:13 }}>Post a request instead and providers can accept it as they come online.</div>
              </div>
            ) : providers.map(p=>(
              <div key={p.phone} style={{ background:T.card, borderRadius:14, padding:14, marginBottom:10, border:`1px solid ${T.border}`, display:"flex", gap:12, alignItems:"center" }}>
                {p.avatar_url
                  ? <img src={p.avatar_url} style={{ width:44,height:44,borderRadius:"50%",objectFit:"cover",flexShrink:0 }} />
                  : <div style={{ width:44,height:44,borderRadius:"50%",background:T.primarySoft,color:T.primary,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:16,flexShrink:0 }}>{(p.name||"?")[0].toUpperCase()}</div>}
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    <div style={{ fontWeight:700, color:T.text, fontSize:14 }}>{p.name||"Nitume Provider"}</div>
                    {p.is_verified_shopper && <span style={{ fontSize:10, fontWeight:800, color:T.success }}>✅ Verified</span>}
                  </div>
                  <div style={{ fontSize:12, color:T.textMuted, marginTop:2 }}>{p.town||"Kenya"} · ⭐ {(p.nitume_rating_avg||0).toFixed(1)} ({p.nitume_completed_count||0} completed)</div>
                </div>
                <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                  <button onClick={()=>{ setTargetProvider(p); setPayNow(false); setFormError(""); setShowForm(true); }} style={{ padding:"9px 14px", background:T.accentSoft, color:T.accentDark, border:"none", borderRadius:10, fontSize:12, fontWeight:800, cursor:"pointer", whiteSpace:"nowrap" }}>👉 Nudge</button>
                </div>
              </div>
            ))}
          </>
        ...
        {!loading && tab==="my" && (
          <>
            ...
            {myPosted.length===0 ? <div style={{ fontSize:12, color:T.textMuted, marginBottom:20 }}>You haven't posted any Nitume requests yet.</div> :
              myPosted.map(r=>(
                <div key={r.id} onClick={()=>setViewReq(r)} style={{ background:T.card, borderRadius:14, padding:14, marginBottom:10, border:`1px solid ${T.border}`, cursor:"pointer" }}>
                  <div style={{ display:"flex", justifyContent:"space-between" }}>
                    <div style={{ fontWeight:700, color:T.text, fontSize:14 }}>{r.title}</div>
                    <Pill color={
                  ...
                  {r.target_provider_phone && r.status==="open" && <div style={{ fontSize:11, color:T.accentDark, marginTop:4, fontWeight:700 }}>🎯 Sent directly to {r.target_provider_name}</div>}
                  ...
                  {r.provider_phone && r.status!=="open" && (
                    <div style={{ display:"flex", gap:8, marginTop:10 }}>
                      <button onClick={e=>{ e.stopPropagation(); setChatReq(r); }} style={{ flex:1, padding:10, background:T.primarySoft, color:T.primary, border:"none", borderRadius:11, fontSize:13, fontWeight:700, cursor:"pointer" }}>
                        💬 Chat
                      </button>
                      <a href={`tel:${r.provider_phone}`} onClick={e=>e.stopPropagation()} style={{ flex:1, textAlign:"center", padding:10, background:T.successSoft, color:T.success, border:"none", borderRadius:11, fontSize:13, fontWeight:700, textDecoration:"none" }}>
                        📞 Call
                      </a>
                    </div>
                  )}
            ...
            {myAccepted.length===0 ? <div style={{ fontSize:12, color:T.textMuted }}>You haven't accepted any jobs yet — check Browse.</div> :
              myAccepted.map(r=>(
                <div key={r.id} style={{ background:T.card, borderRadius:14, padding:14, marginBottom:10, border:`1px solid ${T.border}` }}>
                  ...
                  <div style={{ fontSize:12, color:T.textMuted, marginTop:2 }}>📍 {r.location}</div>
                  {r.customer_phone && (r.status==="accepted"||r.status==="delivered") && (
                    <div style={{ display:"flex", gap:8, marginTop:10 }}>
                      <button onClick={e=>{ e.stopPropagation(); setChatReq(r); }} style={{ flex:1, padding:10, background:T.primarySoft, color:T.primary, border:"none", borderRadius:11, fontSize:13, fontWeight:700, cursor:"pointer" }}>
                        💬 Chat
                      </button>
                      <a href={`tel:${r.customer_phone}`} onClick={e=>e.stopPropagation()} style={{ flex:1, textAlign:"center", padding:10, background:T.successSoft, color:T.success, border:"none", borderRadius:11, fontSize:13, fontWeight:700, textDecoration:"none" }}>
                        📞 Call
                      </a>
                    </div>
                  )}
      ...
      {/* Post request form */}
      {showForm && (
        <div style={{ position:"fixed", inset:0, background:T.overlay, zIndex:150, display:"flex", alignItems:"flex-end", justifyContent:"center" }} onClick={()=>{ setShowForm(false); setTargetProvider(null); }}>
          <div style={{ background:T.card, borderRadius:"22px 22px 0 0", padding:"24px 20px 32px", width:"100%", maxWidth:430, maxHeight:"85vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            <div style={{ fontWeight:800, fontSize:17, color:T.text, marginBottom:targetProvider?6:16 }}>{targetProvider ? "Send Nitume Request" : "Post a Nitume Request"}</div>
            {targetProvider && (
              <div style={{ background:T.primarySoft, borderRadius:10, padding:"8px 12px", fontSize:12, color:T.primary, fontWeight:700, marginBottom:14 }}>
                🎯 Going directly to {targetProvider.name} — only they can accept this
              </div>
            )}

            {formError && (
              <div style={{ background:"#FDECEA", border:"1px solid #F5C2C0", color:"#B3261E", borderRadius:11, padding:"10px 13px", fontSize:12, fontWeight:700, marginBottom:14 }}>
                ⚠️ {formError}
              </div>
            )}
            ...
            {featuredProviders.length>0 && (
              <div style={{ marginBottom:14 }}>
                ...
                {featuredProviders.map(k=>(
                  <div key={k.phone} onClick={()=>window.open(`/?kiosk=${k.slug||k.phone}`,"_blank")}
            ...
            {targetProvider ? (
              <>
                <button onClick={postNitumeRequest} disabled={posting}
                  ...
                  {posting && !payNow ? "Sending..." : `👉 Nudge — ${targetProvider.name} pays Ksh 20 to accept`}
                </button>
                <button onClick={()=>{ setPayNow(true); postAndPayNow(); }} disabled={posting}
                  style={{ width:"100%", padding:14, background:posting?T.border:T.accent, color:T.primary, border:"none", borderRadius:12, fontSize:14, fontWeight:800, cursor:posting?"default":"pointer", marginTop:8 }}>
                  {posting && payNow ? "📲 Check your phone for the M-Pesa prompt..." : "⚡ Pay Ksh 20 now — guarantee priority"}
                </button>
                <div style={{ fontSize:11, color:T.textMuted, textAlign:"center", marginTop:6 }}>Paying now confirms {targetProvider.name} immediately, no waiting for them to accept.</div>
              </>
            ) : (
              <button onClick={postNitumeRequest} disabled={posting}
                style={{ width:"100%", padding:14, background:posting?T.border:T.primary, color:"#fff", border:"none", borderRadius:12, fontSize:14, fontWeight:800, cursor:posting?"default":"pointer", marginTop:8 }}>
            ...
            <button onClick={()=>{ setShowForm(false); setFormError(""); setTargetProvider(null); setPayNow(false); }} style={{ width:"100%", padding:11, marginTop:8, background:"transparent", border:"none", color:T.textMuted, fontSize:13, cursor:"pointer" }}>Cancel</button>
          </div>
        </div>
      )}

      {viewReq && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:100, display:"flex", alignItems:"flex-end" }} onClick={()=>setViewReq(null)}>
          <div style={{ background:"#fff", borderRadius:"20px 20px 0 0", padding:20, width:"100%", maxHeight:"80vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6 }}>
              <div style={{ fontWeight:800, fontSize:17, color:T.text }}>{viewReq.title}</div>
              <Pill color={
                viewReq.status==="open"?T.primary : viewReq.status==="accepted"?T.accentDark : viewReq.status==="delivered"?T.accentDark : T.success
              } bg={
                viewReq.status==="open"?T.primarySoft : viewReq.status==="accepted"?T.accentSoft : viewReq.status==="delivered"?T.accentSoft : T.successSoft
              }>{viewReq.status}</Pill>
            </div>
            <div style={{ fontSize:13, color:T.textMuted, marginBottom:14 }}>{catLabel(viewReq.category)} · Ksh {(viewReq.offered_fee||0).toLocaleString()}</div>
            {viewReq.description && <div style={{ fontSize:13, color:T.text, marginBottom:10, lineHeight:1.5 }}>{viewReq.description}</div>}
            <div style={{ fontSize:13, color:T.textMid, marginBottom:6 }}>📍 {viewReq.location}</div>
            {viewReq.status!=="open" && (viewReq.provider_name || viewReq.provider_phone) && (
              <div style={{ fontSize:13, color:T.text, marginTop:6 }}>Provider: {viewReq.provider_name||viewReq.provider_phone}</div>
            )}
            {viewReq.provider_phone && viewReq.status!=="open" && (
              <div style={{ display:"flex", gap:8, marginTop:10 }}>
                <button onClick={()=>{ setChatReq(viewReq); setViewReq(null); }} style={{ flex:1, padding:11, background:T.primarySoft, color:T.primary, border:"none", borderRadius:11, fontSize:14, fontWeight:700, cursor:"pointer" }}>
                  💬 Chat
                </button>
                <a href={`tel:${viewReq.provider_phone}`} style={{ flex:1, textAlign:"center", padding:11, background:T.successSoft, color:T.success, border:"none", borderRadius:11, fontSize:14, fontWeight:700, textDecoration:"none" }}>
                  📞 Call
                </a>
              </div>
            )}
            {viewReq.status==="delivered" && (
              <div style={{ marginTop:12, background:T.accentSoft, borderRadius:10, padding:10, textAlign:"center" }}>
                <div style={{ fontSize:11, color:T.textMuted, marginBottom:4 }}>Give this PIN to the provider on receipt:</div>
                <div style={{ fontSize:22, fontWeight:900, color:T.accentDark, letterSpacing:4 }}>{viewReq.pin_code}</div>
              </div>
            )}
            {viewReq.status==="open" && (
              <button onClick={()=>cancelNitumeRequest(viewReq.id)} disabled={cancelling}
                style={{ width:"100%", padding:13, marginTop:16, background:cancelling?T.border:"#FDECEA", color:"#B3261E", border:"1px solid #F5C2C0", borderRadius:12, fontSize:14, fontWeight:800, cursor:cancelling?"default":"pointer" }}>
                {cancelling ? "Cancelling..." : "Cancel This Request"}
              </button>
            )}
            <button onClick={()=>setViewReq(null)} style={{ width:"100%", padding:11, marginTop:8, background:"transparent", border:"none", color:T.textMuted, fontSize:13, cursor:"pointer" }}>Close</button>
          </div>
        </div>
      )}

      {chatReq && (
        <NitumeChatThread req={chatReq} currentPhone={phone} currentName={userName} onClose={()=>setChatReq(null)} />
      )}
    </div>
  );
}
...
function KioskScreen({ phone, userName, town, showToast, onBack, onAddItem }) {
  ...
  async function save() {
    ...
    if(kiosk) {
      ...
      const slug = (name.trim()+"-"+phone.slice(-4)).toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"");
      await sb.from("kiosks").insert({ phone, seller_name:userName, name, description:desc, location, category, logo_url, banner_url, accepts_nitume:acceptsNitume, slug });
    }
    let pendingMarketId = null;
    try { pendingMarketId = sessionStorage.getItem("kaziapa_pending_market_join"); } catch(e) {}
    if(pendingMarketId) {
      await sb.from("kiosk_markets").upsert({ kiosk_phone:phone, market_id:pendingMarketId }, { onConflict:"kiosk_phone,market_id" });
      try { sessionStorage.removeItem("kaziapa_pending_market_join"); } catch(e) {}
      showToast("Kiosk saved and added to the market! 🏬");
    } else {
  ...
  function shareKiosk() {
    const identifier = kiosk.slug || phone;
    const url = `https://kaziapa.co.ke/?kiosk=${identifier}`;
    if (navigator.share) {
...
function MarketListScreen({ phone, userName, town, showToast, onBack, onOpenMarket }) {
  ...
  const [sName, setSName] = useState(""); const [sCounty, setSCounty] = useState("");
  const [sTown, setSTown] = useState(""); const [sType, setSType] = useState("");
  const [sDesc, setSDesc] = useState(""); const [submitting, setSubmitting] = useState(false);
  const [dupeWarning, setDupeWarning] = useState(null);
  const [myMarkets, setMyMarkets] = useState([]);
  const [myMarketsLoading, setMyMarketsLoading] = useState(true);

  useEffect(()=>{ load(); loadMyMarkets(); },[]);

  async function loadMyMarkets() {
    setMyMarketsLoading(true);
    const { data:mem } = await sb.from("market_members").select("market_id").eq("phone",phone);
    if(mem && mem.length) {
      const { data:ms } = await sb.from("markets").select("*").in("id", mem.map(m=>m.market_id)).eq("status","ACTIVE");
      setMyMarkets(ms||[]);
    } else setMyMarkets([]);
  ...
  async function load() {
    setLoading(true);
    const { data } = await sb.from("markets").select("*").eq("status","ACTIVE").order("is_featured",{ ascending:false }).order("name");
    setMarkets(data||[]);
    setLoading(false);
  }
  ...
  function findNearMe() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      ()=>{ showToast("Couldn't get your location","error"); setLocating(false); },
  ...
  const filtered = markets.filter(m => {
    ...
    return m.name.toLowerCase().includes(q) || (m.town||"").toLowerCase().includes(q) || (m.county||"").toLowerCase().includes(q);
  });

  const withDistance = coords ? markets
    .filter(m=>m.latitude && m.longitude)
    .map(m=>({ ...m, dist:haversineKm(coords.lat,coords.lng,m.latitude,m.longitude) }))
    .sort((a,b)=>a.dist-b.dist)
    .slice(0,8)
    : [];

  const featured = markets.filter(m=>m.is_featured).slice(0,10);

  async function checkDupeAndSuggest() {
    if(!sName.trim() || !sTown.trim()) { showToast("Enter at least a market name and town","error"); return; }
    const q = sName.trim().toLowerCase();
    const possibleDupe = markets.find(m => m.name.toLowerCase().includes(q) || q.includes(m.name.toLowerCase()));
    if(possibleDupe && !dupeWarning) { setDupeWarning(possibleDupe); return; }
    setSubmitting(true);
    const slug = (sName+"-"+sTown+"-"+Date.now()).toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"");
    await sb.from("markets").insert({
      name:sName.trim(), slug, county:sCounty.trim()||null, town:sTown.trim(),
      description:sDesc.trim()||null, market_level:"COMMUNITY",
      source_type:"COMMUNITY_SUGGESTED", status:"PENDING_REVIEW", suggested_by_phone:phone
    });
    showToast("Market suggested! We'll review it soon. 🙏");
    setShowSuggest(false); setSName(""); setSCounty(""); setSTown(""); setSType(""); setSDesc(""); setDupeWarning(null);
    setSubmitting(false);
  }

  return (
    <div>
      <BackHeader title="🏬 My Market" onBack={onBack} />
      <div style={{ padding:16 }}>
        <div style={{ background:T.card,borderRadius:13,padding:"11px 14px",display:"flex",alignItems:"center",gap:8,border:`1px solid ${T.border}`,marginBottom:16 }}>
          <span style={{ fontSize:16 }}>🔍</span>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search market, town or county..."
            style={{ border:"none",outline:"none",flex:1,fontSize:14,color:T.text,background:"transparent" }} />
          {search && <span onClick={()=>setSearch("")} style={{ color:T.textMuted,cursor:"pointer",fontSize:20 }}>×</span>}
        </div>

        {loading ? <div style={{ padding:40,textAlign:"center",color:T.textMuted }}>Loading markets...</div> : search ? (
          <div>
            <div style={{ fontSize:13,fontWeight:800,color:T.text,marginBottom:10 }}>{filtered.length} result{filtered.length!==1?"s":""}</div>
            {filtered.map(m=><MarketRow key={m.id} m={m} onClick={()=>onOpenMarket(m)} />)}
            {filtered.length===0 && <div style={{ textAlign:"center",padding:"20px 0",color:T.textMuted,fontSize:13 }}>No matching market found.</div>}
          </div>
        ) : (
          <div>
            {!myMarketsLoading && myMarkets.length>0 && (
              <>
                <div style={{ fontSize:13,fontWeight:800,color:T.text,marginBottom:10 }}>📌 My Markets</div>
                {myMarkets.map(m=><MarketRow key={m.id} m={m} onClick={()=>onOpenMarket(m)} />)}
              </>
            )}
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,marginTop:myMarkets.length>0?20:0 }}>
              <div style={{ fontSize:13,fontWeight:800,color:T.text }}>📍 Markets Near You</div>
              {!coords && <span onClick={findNearMe} style={{ color:T.primary,fontSize:12,fontWeight:700,cursor:"pointer" }}>{locating?"Locating...":"Use my location"}</span>}
            </div>
            {coords ? (withDistance.length ? withDistance.map(m=>
              <MarketRow key={m.id} m={m} sub={`${m.dist.toFixed(1)} km away`} onClick={()=>onOpenMarket(m)} />
            ) : <div style={{ fontSize:12,color:T.textMuted,marginBottom:16 }}>No markets with location data nearby yet — try search below.</div>
            ) : <div style={{ fontSize:12,color:T.textMuted,marginBottom:16 }}>Turn on location to see markets near you.</div>}

            <div style={{ fontSize:13,fontWeight:800,color:T.text,margin:"20px 0 10px" }}>⭐ Popular Markets</div>
            {featured.map(m=><MarketRow key={m.id} m={m} onClick={()=>onOpenMarket(m)} />)}

            <div style={{ fontSize:13,fontWeight:800,color:T.text,margin:"20px 0 10px" }}>🇰🇪 All Markets ({markets.length})</div>
            {markets.slice(0,25).map(m=><MarketRow key={m.id} m={m} onClick={()=>onOpenMarket(m)} />)}
          </div>
        )}

        <div onClick={()=>setShowSuggest(true)} style={{ marginTop:20,textAlign:"center",padding:14,border:`1.5px dashed ${T.border}`,borderRadius:13,cursor:"pointer" }}>
          <div style={{ fontWeight:800,color:T.primary,fontSize:13 }}>➕ Can't find your market?</div>
          <div style={{ fontSize:11,color:T.textMuted,marginTop:2 }}>Suggest it — we'll review and add it</div>
        </div>
      </div>

      {showSuggest && (
        <div style={{ position:"fixed",inset:0,background:T.overlay,zIndex:100,display:"flex",alignItems:"flex-end" }} onClick={()=>setShowSuggest(false)}>
          <div style={{ background:T.card,borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxHeight:"85vh",overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            <div style={{ fontWeight:800,fontSize:16,color:T.text,marginBottom:16 }}>➕ Suggest a Market</div>
            <Field label="Market Name" value={sName} onChange={setSName} placeholder="e.g. Kondele Market" />
            <Field label="Town / Trading Centre" value={sTown} onChange={setSTown} placeholder="e.g. Kisumu" />
            <Field label="County" value={sCounty} onChange={setSCounty} placeholder="e.g. Kisumu" />
            <Field label="Description (optional)" value={sDesc} onChange={setSDesc} placeholder="What's it known for?" rows={3} />
            {dupeWarning && (
              <div style={{ background:T.accentSoft,borderRadius:12,padding:12,marginBottom:16,fontSize:12,color:T.accentDark }}>
                ⚠️ We may already have this market: <b>{dupeWarning.name}</b> ({dupeWarning.town}). Tap Submit again to suggest it anyway, or close this and search for it.
              </div>
            )}
            <Btn onClick={checkDupeAndSuggest} disabled={submitting}>{submitting?"Submitting...":dupeWarning?"Submit Anyway →":"Submit for Review →"}</Btn>
            <button onClick={()=>{ setShowSuggest(false); setDupeWarning(null); }} style={{ width:"100%",padding:11,marginTop:8,background:"transparent",border:"none",color:T.textMuted,fontSize:13,cursor:"pointer" }}>Cancel</button>
          </div>
        </div>
      )}
    </div>
...
function MarketRow({ m, sub, onClick }) {
  const levelIcon = m.market_level==="MAJOR" ? "⭐" : m.market_level==="TOWN" ? "🏬" : "🏪";
  return (
    <div onClick={onClick} style={{ background:T.card,borderRadius:13,padding:14,marginBottom:10,border:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:12,cursor:"pointer" }}>
      <div style={{ fontSize:24 }}>{levelIcon}</div>
      <div style={{ flex:1,minWidth:0 }}>
        <div style={{ fontWeight:700,color:T.text,fontSize:14 }}>{m.name}</div>
        <div style={{ fontSize:12,color:T.textMuted,marginTop:2 }}>{sub || [m.town,m.county].filter(Boolean).join(", ")}</div>
      </div>
...
function HoodRow({ h, sub, onClick }) {
  const icon = h.level==="COUNTY" ? "🗺️" : h.level==="TOWN" ? "🏘️" : "📍";
  return (
    <div onClick={onClick} style={{ background:T.card,borderRadius:13,padding:14,marginBottom:10,border:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:12,cursor:"pointer" }}>
...
function HoodListScreen({ phone, userName, showToast, onBack, onOpenMarket, navState, setNavState }) {
  const { view, county, town, search } = navState;
  function setView(v){ setNavState(s=>({ ...s, view:v })); }
  function setCounty(c){ setNavState(s=>({ ...s, county:c })); }
  function setTown(t){ setNavState(s=>({ ...s, town:t })); }
  function setSearch(sv){ setNavState(s=>({ ...s, search:sv })); }
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  // Restore whatever view (root / county / town) and search the user left off at,
  // instead of always resetting to root when this screen remounts.
  useEffect(()=>{
    if(view==="town" && town) loadEstatesForTown(town);
    else if(view==="county" && county) loadTownsForCounty(county);
    else loadRoot();
  ...
  async function loadRoot() {
    setLoading(true);
    const { data } = await sb.from("markets").select("*").eq("type","hood").eq("level","COUNTY").eq("status","ACTIVE").order("name");
    setItems(data||[]); setLoading(false);
  }

  async function loadTownsForCounty(c) {
    setLoading(true);
    const { data } = await sb.from("markets").select("*").eq("type","hood").eq("level","TOWN").eq("parent_id",c.id).eq("status","ACTIVE").order("name");
    setItems(data||[]); setLoading(false);
  }

  async function loadEstatesForTown(t) {
    setLoading(true);
    const { data } = await sb.from("markets").select("*").eq("type","hood").eq("level","ESTATE").eq("parent_id",t.id).eq("status","ACTIVE").order("name");
    setItems(data||[]); setLoading(false);
  }

  function openCounty(c) { setCounty(c); setTown(null); setView("county"); loadTownsForCounty(c); }
  function openTown(t) { setTown(t); setView("town"); loadEstatesForTown(t); }

  function goBackOneLevel() {
    if(view==="town") { setView("county"); setTown(null); loadTownsForCounty(county); }
    else if(view==="county") { setView("root"); setCounty(null); loadRoot(); }
    else onBack();
  }
  ...
  async function runSearch(text) {
    ...
    const { data } = await sb.from("markets").select("*").eq("type","hood").eq("status","ACTIVE").ilike("name",`%${text.trim()}%`).limit(20);
    setSearchResults(data||[]);
  }

  async function submitSuggestion() {
    if(!sName.trim()) { showToast("Enter the name first","error"); return; }
    setSubmitting(true);
    const parentCtx = view==="town" ? town : view==="county" ? county : null;
    const childLevel = view==="town" ? "ESTATE" : view==="county" ? "TOWN" : "COUNTY";
    const slug = (sName+"-"+Date.now()).toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"");
    await sb.from("markets").insert({
      type:"hood", level:childLevel, name:sName.trim(), county: county?.name || sName.trim(),
      parent_id: parentCtx?.id || null, status:"PENDING", suggested_by_phone:phone, slug
    });
    showToast("Suggested! We'll review and add it soon 🙏");
    setShowSuggest(false); setSName(""); setSubmitting(false);
  }

  const crumb = [county?.name, town?.name].filter(Boolean).join(" › ");

  return (
    <div>
      <BackHeader title={`🏘️ ${town?.name || county?.name || "My Hood"}`} onBack={goBackOneLevel} />
      <div style={{ padding:16 }}>
        {view==="root" && (
          <div style={{ background:T.card,borderRadius:13,padding:"11px 14px",display:"flex",alignItems:"center",gap:8,border:`1px solid ${T.border}`,marginBottom:16 }}>
            <span style={{ fontSize:16 }}>🔍</span>
            <input value={search} onChange={e=>runSearch(e.target.value)} placeholder="Search your estate, town or county..."
              style={{ border:"none",outline:"none",flex:1,fontSize:14,color:T.text,background:"transparent" }} />
            {search && <span onClick={()=>{ setSearch(""); setSearchResults([]); }} style={{ color:T.textMuted,cursor:"pointer",fontSize:20 }}>×</span>}
          </div>
        )}

        {crumb && <div style={{ fontSize:12,color:T.textMuted,marginBottom:14 }}>{crumb}</div>}

        {loading ? <div style={{ padding:40,textAlign:"center",color:T.textMuted }}>Loading...</div> : search.trim() ? (
          <div>
            <div style={{ fontSize:13,fontWeight:800,color:T.text,marginBottom:10 }}>{searchResults.length} result{searchResults.length!==1?"s":""}</div>
            {searchResults.map(h=>
              <HoodRow key={h.id} h={h} sub={h.level} onClick={()=>onOpenMarket(h)} />
            )}
            {searchResults.length===0 && <div style={{ textAlign:"center",padding:"20px 0",color:T.textMuted,fontSize:13 }}>No match — suggest it below.</div>}
          </div>
        ) : (
          <div>
            {items.map(h=>
              <HoodRow key={h.id} h={h}
                onClick={()=> h.level==="COUNTY" ? openCounty(h) : h.level==="TOWN" ? openTown(h) : onOpenMarket(h)} />
            )}
            {items.length===0 && (
              <div style={{ textAlign:"center",padding:"20px 0",color:T.textMuted,fontSize:13 }}>
                Nothing seeded here yet — be the first to suggest it below.
              </div>
            )}
            {view!=="root" && (
              <div onClick={()=>onOpenMarket(view==="town" ? town : county)}
                style={{ marginTop:10,textAlign:"center",padding:12,border:`1px solid ${T.border}`,borderRadius:13,cursor:"pointer",fontSize:13,fontWeight:700,color:T.primary }}>
                Use "{view==="town" ? town.name : county.name}" as my Hood →
              </div>
            )}
          </div>
        )}
        ...
        <div onClick={()=>setShowSuggest(true)} style={{ marginTop:20,textAlign:"center",padding:14,border:`1.5px dashed ${T.border}`,borderRadius:13,cursor:"pointer" }}>
          <div style={{ fontWeight:800,color:T.primary,fontSize:13 }}>➕ Don't see your area?</div>
          <div style={{ fontSize:11,color:T.textMuted,marginTop:2 }}>Suggest it — we'll review and add it</div>
        </div>
      </div>
      ...
      {showSuggest && (
        <div style={{ position:"fixed",inset:0,background:T.overlay,zIndex:100,display:"flex",alignItems:"flex-end" }} onClick={()=>setShowSuggest(false)}>
          <div style={{ background:T.card,borderRadius:"20px 20px 0 0",padding:20,width:"100%" }} onClick={e=>e.stopPropagation()}>
            <div style={{ fontWeight:800,fontSize:16,color:T.text,marginBottom:16 }}>
              ➕ Suggest {view==="town" ? "an estate/area" : view==="county" ? "a town" : "a county"}
            </div>
            <Field label="Name" value={sName} onChange={setSName} placeholder="e.g. Kaptembwa" />
            <Btn onClick={submitSuggestion} disabled={submitting}>{submitting?"Submitting...":"Submit for Review →"}</Btn>
            <button onClick={()=>setShowSuggest(false)} style={{ width:"100%",padding:11,marginTop:8,background:"transparent",border:"none",color:T.textMuted,fontSize:13,cursor:"pointer" }}>Cancel</button>
          </div>
        </div>
      )}
    </div>
...
const MARKET_ROLES = ["Buyer","Seller","Retailer","Wholesaler","Shop Owner","Agent","Supplier","Hotel/Food Vendor","Transporter","Shopper","Service Provider","Worker","Other"];
const HOOD_ROLES = ["Resident","Buyer","Seller","Fundi/Service Provider","Landlord/Agent","Job Seeker","Employer","Shop Owner","Transporter","Other"];
const NON_KIOSK_ROLES = ["Buyer","Worker","Resident","Job Seeker"]; // roles that don't imply "I have a stall/business here"

function MarketProfileScreen({ market, phone, userName, showToast, onBack, onAddBusiness, onRequestGuide }) {
  const [tab, setTab] = useState(market.type==="hood" ? "notices" : "directory");
  const [kiosks, setKiosks] = useState([]);
  const [loading, setLoading] = useState(true);
  const [joined, setJoined] = useState(false);
  const [myRoles, setMyRoles] = useState([]);
  const [showRoleModal, setShowRoleModal] = useState(false);
  const [modalStep, setModalStep] = useState("roles"); // roles | kioskOffer
  const [pickedRoles, setPickedRoles] = useState([]);
  const [savingRoles, setSavingRoles] = useState(false);
  const [existingKioskInfo, setExistingKioskInfo] = useState(null);
  ...
  const [marketTransport, setMarketTransport] = useState([]);
  const [marketGuides, setMarketGuides] = useState([]);
  const [quickChat, setQuickChat] = useState(null);
  const [notices, setNotices] = useState([]);
  const [showPostNotice, setShowPostNotice] = useState(false);
  const [noticeCategory, setNoticeCategory] = useState("community");
  ...
  const [showPostSpace, setShowPostSpace] = useState(false);
  const [spaceType, setSpaceType] = useState("stall");
  const [spaceOwnership, setSpaceOwnership] = useState("owner");
  const [spaceSection, setSpaceSection] = useState("");
  const [spaceSize, setSpaceSize] = useState("");
  const [spaceRent, setSpaceRent] = useState("");
  const [spaceDeposit, setSpaceDeposit] = useState("");
  const [spaceDesc, setSpaceDesc] = useState("");
  const [spacePhotos, setSpacePhotos] = useState([]);
  const [spaceTemporary, setSpaceTemporary] = useState(false);
  const [postingSpace, setPostingSpace] = useState(false);
  const [spaceError, setSpaceError] = useState("");
  const [tabLoading, setTabLoading] = useState(false);
  const [loadedTabs, setLoadedTabs] = useState({});

  useEffect(()=>{ load(); },[market.id]);
  useEffect(()=>{ loadTab(tab); },[tab, market.id]);
  useEffect(()=>{ if(tab==="notices") loadTab("notices"); },[noticeFilterCat]);

  async function loadTab(t) {
    if(t==="directory" || t==="info") return; // handled by load()
    setTabLoading(true);
    if(t==="shop") {
      const { data } = await sb.from("listings").select("*").eq("market_id",market.id).not("category","in","(kazi,fundi,usafiri)").order("created_at",{ ascending:false }).limit(30);
      setShopListings(data||[]);
    } else if(t==="requests") {
      const { data } = await sb.from("requests").select("*").eq("market_id",market.id).order("created_at",{ ascending:false }).limit(30);
      setMarketRequests(data||[]);
    } else if(t==="jobs") {
      const { data } = await sb.from("listings").select("*").eq("market_id",market.id).eq("category","kazi").order("created_at",{ ascending:false }).limit(30);
      setMarketJobs(data||[]);
    } else if(t==="services") {
      const { data } = await sb.from("listings").select("*").eq("market_id",market.id).eq("category","fundi").order("created_at",{ ascending:false }).limit(30);
      setMarketServices(data||[]);
    } else if(t==="transport") {
      const { data } = await sb.from("listings").select("*").eq("market_id",market.id).eq("category","usafiri").order("created_at",{ ascending:false }).limit(30);
      setMarketTransport(data||[]);
    } else if(t==="guides") {
      const { data:gm } = await sb.from("nitume_guide_markets").select("phone,knows_notes").eq("market_id",market.id);
      if(gm && gm.length) {
        const { data:us } = await sb.from("users").select("phone,name,town,nitume_rating_avg,nitume_completed_count,is_verified_shopper").in("phone", gm.map(g=>g.phone));
        const merged = (us||[]).map(u => ({ ...u, knows_notes: gm.find(g=>g.phone===u.phone)?.knows_notes }));
        setMarketGuides(merged);
      } else setMarketGuides([]);
    } else if(t==="notices") {
      let nq = sb.from("market_notices").select("*").eq("market_id",market.id).eq("status","active").gt("expires_at",new Date().toISOString()).order("created_at",{ ascending:false }).limit(50);
      if(noticeFilterCat!=="all") nq = nq.eq("category",noticeFilterCat);
      if(noticeSearch.trim()) nq = nq.ilike("message",`%${noticeSearch.trim()}%`);
      const { data } = await nq;
      setNotices(data||[]);
    } else if(t==="spaces") {
      const { data } = await sb.from("market_spaces").select("*").eq("market_id",market.id).eq("status","active").gt("expires_at",new Date().toISOString()).order("created_at",{ ascending:false }).limit(50);
      setSpaces(data||[]);
    }
    setLoadedTabs(p=>({...p,[t]:true}));
    setTabLoading(false);
  }

  async function load() {
    setLoading(true);
    const { data:km } = await sb.from("kiosk_markets").select("kiosk_phone").eq("market_id", market.id);
    if(km && km.length) {
      const { data:ks } = await sb.from("kiosks").select("*").in("phone", km.map(r=>r.kiosk_phone));
      setKiosks(ks||[]);
    } else setKiosks([]);
    const { data:mem } = await sb.from("market_members").select("id,roles").eq("phone",phone).eq("market_id",market.id).limit(1);
    if(mem && mem.length) { setJoined(true); setMyRoles(mem[0].roles||[]); }
    else { setJoined(false); setMyRoles([]); }
    setLoading(false);
  }

  function openRolePicker() {
    setPickedRoles(myRoles.length ? myRoles : []);
    setModalStep("roles");
  ...
  async function confirmRoles() {
    if(pickedRoles.length===0) { showToast("Pick at least one role","error"); return; }
    setSavingRoles(true);
    await sb.from("market_members").upsert({ phone, market_id:market.id, roles:pickedRoles }, { onConflict:"phone,market_id" });
    setJoined(true); setMyRoles(pickedRoles);
    showToast(`Joined ${market.name} 🏬`);
    setSavingRoles(false);

    const isBusinessRole = pickedRoles.some(r => !NON_KIOSK_ROLES.includes(r));
    if(!isBusinessRole) { setShowRoleModal(false); load(); return; }

    // Already linked a kiosk to this specific market? Skip the offer.
    const { data:existingLink } = await sb.from("kiosk_markets").select("id").eq("kiosk_phone",phone).eq("market_id",market.id).limit(1);
    if(existingLink && existingLink.length>0) { setShowRoleModal(false); load(); return; }

    const { data:existingKiosk } = await sb.from("kiosks").select("*").eq("phone",phone).limit(1);
    setExistingKioskInfo(existingKiosk && existingKiosk.length ? existingKiosk[0] : null);
    setModalStep("kioskOffer");
  }

  async function linkExistingKiosk() {
    await sb.from("kiosk_markets").upsert({ kiosk_phone:phone, market_id:market.id }, { onConflict:"kiosk_phone,market_id" });
    showToast(`${existingKioskInfo.name} added to ${market.name} 🏪`);
    setShowRoleModal(false);
    load();
  }

  function goSetupKiosk() {
    try { sessionStorage.setItem("kaziapa_pending_market_join", market.id); } catch(e) {}
    setShowRoleModal(false);
    onAddBusiness();
  }
  ...
  async function addMyBusiness() {
    // Join the market first — this always succeeds, kiosk or not, so it can never dead-end.
    await sb.from("kiosk_markets").upsert({ kiosk_phone:phone, market_id:market.id }, { onConflict:"kiosk_phone,market_id" });
    const newRoles = Array.from(new Set([...(myRoles||[]), "Seller"]));
    await sb.from("market_members").upsert({ phone, market_id:market.id, roles:newRoles }, { onConflict:"phone,market_id" });
    setJoined(true); setMyRoles(newRoles);
    const { data:existingKiosk } = await sb.from("kiosks").select("phone").eq("phone",phone).limit(1);
    if(existingKiosk && existingKiosk.length>0) {
      showToast(`${market.name} added to your kiosk! 🏪`);
      load();
    } else {
      try { sessionStorage.setItem("kaziapa_pending_market_join", market.id); } catch(e) {}
      showToast(`Joined — now set up your kiosk details`);
      onAddBusiness();
    }
  }

  async function unlinkKiosk() {
    await sb.from("kiosk_markets").delete().eq("kiosk_phone",phone).eq("market_id",market.id);
    showToast(`Removed your kiosk from ${market.name}`);
    load();
  }

  async function postNotice() {
    if(!noticeMessage.trim()) { setNoticeError("Write a message first"); return; }
    if(noticeCategory==="business" && !kiosks.some(k=>k.phone===phone)) {
      setNoticeError("Business notices need a kiosk linked to this market — link one from the Directory tab first.");
      return;
    }
    setPostingNotice(true);
    const since = new Date(Date.now() - 24*60*60*1000).toISOString();
    const { count } = await sb.from("market_notices").select("id",{ count:"exact", head:true }).eq("market_id",market.id).eq("phone",phone).gt("created_at",since);
    if((count||0) >= 5) {
      setPostingNotice(false);
      setNoticeError("You've hit today's limit of 5 notices for this market — try again tomorrow.");
      return;
    }
    const expiresDays = noticeCategory==="business" ? 14 : 7;
    const { error } = await sb.from("market_notices").insert({
      market_id: market.id, phone, author_name: userName, category: noticeCategory,
      message: noticeMessage.trim(), expires_at: new Date(Date.now()+expiresDays*86400000).toISOString(),
    });
    setPostingNotice(false);
    if(error) { setNoticeError("Couldn't post: " + (error.message || "try again")); return; }
    showToast("Notice posted 📢");
    setNoticeMessage(""); setNoticeCategory("community"); setShowPostNotice(false); setNoticeError("");
    loadTab("notices");
  }

  async function reportNotice(noticeId, reason) {
    const { data: existing } = await sb.from("market_notice_reports").select("id").eq("notice_id",noticeId).eq("reporter_phone",phone).limit(1);
    if (existing && existing.length) {
      showToast("You've already reported this notice.", "error");
      setReportingNotice(null);
      return;
    }
    const { error } = await sb.from("market_notice_reports").insert({ notice_id:noticeId, reporter_phone:phone, reason, is_urgent: reason==="Hate speech / incitement" });
    if (error) {
      if (error.code === "23505") { showToast("You've already reported this notice.", "error"); }
      else { showToast("Report failed: " + error.message, "error"); }
      setReportingNotice(null);
      return;
    }
    await sb.from("notifications").insert({
      recipient_phone: phone, icon: "🚩", read: false,
      title: "Report received",
      body: "Thanks — your report on a market notice has been sent to our team for review.",
    });
    showToast("Reported — thank you for keeping this market safe.");
    setReportingNotice(null);
  }

  async function deleteMyNotice(noticeId) {
    await sb.from("market_notices").delete().eq("id",noticeId).eq("phone",phone);
    showToast("Notice removed");
    loadTab("notices");
  }

  async function postSpace() {
    if(!spaceSize.trim() && !spaceSection.trim()) { setSpaceError("Add at least a section or size so people know what they're looking at"); return; }
    setPostingSpace(true);
    const expiresDays = spaceTemporary ? 3 : 30;
    const { error } = await sb.from("market_spaces").insert({
      market_id: market.id, phone, poster_name: userName,
      space_type: spaceType, ownership_role: spaceOwnership,
      section: spaceSection.trim()||null, size_desc: spaceSize.trim()||null,
      rent: spaceRent ? parseInt(spaceRent) : null, deposit: spaceDeposit ? parseInt(spaceDeposit) : null,
      description: spaceDesc.trim()||null, photos: spacePhotos,
      expires_at: new Date(Date.now()+expiresDays*86400000).toISOString(),
    });
    setPostingSpace(false);
    if(error) { setSpaceError("Couldn't post: " + (error.message || "try again")); return; }
    showToast("Space listed 🏪");
    setSpaceSection(""); setSpaceSize(""); setSpaceRent(""); setSpaceDeposit(""); setSpaceDesc(""); setSpacePhotos([]); setSpaceTemporary(false); setSpaceType("stall"); setSpaceOwnership("owner");
    setShowPostSpace(false); setSpaceError("");
    loadTab("spaces");
  }

  async function markSpaceTaken(id) {
    await sb.from("market_spaces").update({ status:"taken" }).eq("id",id).eq("phone",phone);
    showToast("Marked as taken");
    loadTab("spaces");
  }

  async function deleteMySpace(id) {
    await sb.from("market_spaces").delete().eq("id",id).eq("phone",phone);
    showToast("Listing removed");
    loadTab("spaces");
  }

  const myKiosk = kiosks.find(k=>k.phone===phone);

  return (
    <div>
      <BackHeader title={market.type==="hood" ? `🏘️ ${market.name}` : market.name} onBack={onBack} />
      <div style={{ padding:16 }}>
        <div style={{ background:T.card,borderRadius:13,padding:14,border:`1px solid ${T.border}`,marginBottom:16 }}>
          <div style={{ fontSize:13,color:T.textMuted }}>📍 {[market.town,market.county].filter(Boolean).join(", ")}</div>
          {market.description && <div style={{ fontSize:13,color:T.textMid,marginTop:8,lineHeight:1.5 }}>{market.description}</div>}
          {market.type==="hood" && !market.description && <div style={{ fontSize:13,color:T.textMid,marginTop:8,lineHeight:1.5 }}>Your local community space — share notices, find jobs, buy & sell, and connect with neighbors here.</div>}
          {!joined && <button onClick={openRolePicker} style={{ marginTop:12,width:"100%",padding:11,background:T.primary,color:"#fff",border:"none",borderRadius:11,fontWeight:700,fontSize:13,cursor:"pointer" }}>{market.type==="hood" ? "+ Join This Hood" : "+ Join This Market"}</button>}
          {joined && (
            <div style={{ marginTop:12,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8 }}>
              <div style={{ display:"flex",flexWrap:"wrap",gap:6 }}>
                <span style={{ fontSize:11,color:T.success,fontWeight:700 }}>✅ You're in as:</span>
                {myRoles.map(r=>(
                  <span key={r} style={{ background:T.successSoft,color:T.success,borderRadius:20,padding:"3px 10px",fontSize:11,fontWeight:700 }}>{r}</span>
                ))}
              </div>
              <span onClick={openRolePicker} style={{ color:T.primary,fontSize:12,fontWeight:700,cursor:"pointer" }}>Edit</span>
            </div>
          )}
          {joined && (
            <div style={{ marginTop:10,paddingTop:10,borderTop:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:8 }}>
              {myKiosk ? (
                <>
                  <div style={{ fontSize:12,color:T.textMid }}>🏪 Your kiosk here: <b>{myKiosk.name}</b></div>
                  <span onClick={unlinkKiosk} style={{ color:T.coral,fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0 }}>Remove</span>
                </>
              ) : (
                <>
                  <div style={{ fontSize:12,color:T.textMuted }}>No business linked here yet</div>
                  <span onClick={addMyBusiness} style={{ color:T.primary,fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0 }}>+ Link / Create Kiosk</span>
                </>
              )}
            </div>
          )}
        ...
        <div style={{ display:"flex",gap:6,marginBottom:16,overflowX:"auto" }}>
          {(market.type==="hood"
            ? [{k:"notices",l:"📢 Notices"},{k:"requests",l:"📋 Requests"},{k:"jobs",l:"💼 Jobs"},{k:"services",l:"🔧 Services"},{k:"transport",l:"🚚 Transport"},{k:"shop",l:"🛒 Shop"},{k:"directory",l:"🏪 Local Businesses"},{k:"spaces",l:"🏬 Spaces"},{k:"info",l:"ℹ️ Info"}]
            : [{k:"directory",l:"🏪 Directory"},{k:"shop",l:"🛒 Shop"},{k:"requests",l:"📋 Requests"},{k:"jobs",l:"💼 Jobs"},{k:"services",l:"🔧 Services"},{k:"transport",l:"🚚 Transport"},{k:"guides",l:"🧭 Guides"},{k:"notices",l:"📢 Notices"},{k:"spaces",l:"🏬 Spaces"},{k:"info",l:"ℹ️ Info"}]
          ).map(t=>(
            <div key={t.k} onClick={()=>setTab(t.k)} style={{ flexShrink:0,textAlign:"center",padding:"9px 14px",borderRadius:11,background:tab===t.k?T.primary:T.card,color:tab===t.k?"#fff":T.textMid,fontWeight:700,fontSize:12,border:`1px solid ${tab===t.k?T.primary:T.border}`,cursor:"pointer",whiteSpace:"nowrap" }}>{t.l}</div>
          ))}
        </div>

        {tab==="directory" && (
          <div>
            {!loading && kiosks.length>0 && !kiosks.some(k=>k.phone===phone) && (
              <button onClick={addMyBusiness} style={{ width:"100%",padding:11,marginBottom:14,background:T.accentSoft,color:T.accentDark,border:`1px solid ${T.accent}`,borderRadius:11,fontWeight:700,fontSize:13,cursor:"pointer" }}>+ Add My Business to {market.name}</button>
            )}
            {loading ? <div style={{ padding:30,textAlign:"center",color:T.textMuted }}>Loading...</div> : kiosks.length===0 ? (
              <div style={{ textAlign:"center",padding:"30px 0" }}>
                <div style={{ fontSize:40 }}>🏪</div>
                <div style={{ fontWeight:700,color:T.text,marginTop:10,fontSize:14 }}>{market.type==="hood" ? "No local businesses listed yet" : "No businesses here yet"}</div>
                <div style={{ fontSize:12,color:T.textMuted,marginTop:4 }}>{market.type==="hood" ? "Have a shop or business in this hood? Be the first to list it." : "Be the first business to join this market."}</div>
                <button onClick={addMyBusiness} style={{ marginTop:16,padding:"10px 20px",background:T.primary,color:"#fff",border:"none",borderRadius:20,fontSize:13,fontWeight:700,cursor:"pointer" }}>+ Add My Business</button>
              </div>
            ) : kiosks.map(k=>(
              <div key={k.phone} style={{ background:T.card,borderRadius:13,padding:12,marginBottom:10,border:`1px solid ${T.border}`,display:"flex",gap:10,alignItems:"center" }}>
                {k.logo_url ? <img src={k.logo_url} alt="" style={{ width:44,height:44,borderRadius:10,objectFit:"cover" }} /> : <div style={{ width:44,height:44,borderRadius:10,background:T.border,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20 }}>🏪</div>}
                <div style={{ flex:1,minWidth:0 }}>
                  <div style={{ fontWeight:700,color:T.text,fontSize:13 }}>{k.name}</div>
                  <div style={{ fontSize:11,color:T.textMuted }}>{k.category}</div>
                </div>
                {k.phone !== phone && (
                  <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                    <button onClick={()=>setQuickChat({ chatKey:`kiosk-${k.phone}`, title:k.name, otherName:k.name, otherPhone:k.phone })} style={{ background:T.primarySoft,color:T.primary,border:"none",borderRadius:20,padding:"6px 10px",fontSize:11,fontWeight:700,cursor:"pointer" }}>💬 Chat</button>
                    <a href={`tel:${k.phone}`} style={{ background:T.successSoft,color:T.success,borderRadius:20,padding:"6px 10px",fontSize:11,fontWeight:700,textDecoration:"none" }}>📞 Call</a>
                  </div>
                )}
              </div>
            ))}
        ...
        {tab==="shop" && (
          <div>
            {tabLoading && !loadedTabs.shop ? <div style={{ padding:30,textAlign:"center",color:T.textMuted }}>Loading...</div> : shopListings.length===0 ? (
              <div style={{ textAlign:"center",padding:"30px 0" }}>
                <div style={{ fontSize:40 }}>🛒</div>
                <div style={{ fontWeight:700,color:T.text,marginTop:10,fontSize:14 }}>No listings tagged to {market.name} yet</div>
                <div style={{ fontSize:12,color:T.textMuted,marginTop:4 }}>Post a listing and tag it to this market to show up here.</div>
              </div>
            ) : shopListings.map(l=>(
              <div key={l.id} style={{ background:T.card,borderRadius:13,padding:12,marginBottom:10,border:`1px solid ${T.border}`,display:"flex",gap:10,alignItems:"center" }}>
                {l.photos?.[0] ? <img src={l.photos[0]} alt="" style={{ width:48,height:48,borderRadius:10,objectFit:"cover" }} /> : <div style={{ width:48,height:48,borderRadius:10,background:T.border,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20 }}>🛒</div>}
                <div style={{ flex:1,minWidth:0 }}>
                  <div style={{ fontWeight:700,color:T.text,fontSize:13 }}>{l.title}</div>
                  <div style={{ fontSize:12,color:T.primary,fontWeight:700 }}>{l.price_label || `Ksh ${l.price}`}</div>
                </div>
                <a href={`tel:${l.seller_phone}`} style={{ background:T.successSoft,color:T.success,borderRadius:20,padding:"6px 12px",fontSize:11,fontWeight:700,textDecoration:"none" }}>📞 Call</a>
              </div>
            ))}
          </div>
        )}
        ...
        {tab==="requests" && (
          <div>
            {tabLoading && !loadedTabs.requests ? <div style={{ padding:30,textAlign:"center",color:T.textMuted }}>Loading...</div> : marketRequests.length===0 ? (
              <div style={{ textAlign:"center",padding:"30px 0" }}>
                <div style={{ fontSize:40 }}>📋</div>
                <div style={{ fontWeight:700,color:T.text,marginTop:10,fontSize:14 }}>No buyer requests here yet</div>
                <div style={{ fontSize:12,color:T.textMuted,marginTop:4 }}>Post a Buyer Request and tag it to {market.name} to reach sellers here.</div>
              </div>
            ) : marketRequests.map(r=>(
              <div key={r.id} style={{ background:T.card,borderRadius:13,padding:12,marginBottom:10,border:`1px solid ${T.border}` }}>
                <div style={{ fontWeight:700,color:T.text,fontSize:13 }}>{r.item}</div>
                <div style={{ fontSize:12,color:T.primary,fontWeight:700,marginTop:2 }}>Budget: Ksh {r.budget}</div>
                <div style={{ fontSize:11,color:T.textMuted,marginTop:2 }}>{r.buyer_name} · {r.town}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {tab==="jobs" && (
          <div>
            {tabLoading && !loadedTabs.jobs ? <div style={{ padding:30,textAlign:"center",color:T.textMuted }}>Loading...</div> : marketJobs.length===0 ? (
              <div style={{ textAlign:"center",padding:"30px 0" }}>
                <div style={{ fontSize:40 }}>💼</div>
                <div style={{ fontWeight:700,color:T.text,marginTop:10,fontSize:14 }}>No jobs tagged to {market.name} yet</div>
                <div style={{ fontSize:12,color:T.textMuted,marginTop:4 }}>Post a job or your profile from Jobs and tag it to this market.</div>
              </div>
            ) : marketJobs.map(j=>(
              <div key={j.id} style={{ background:T.card,borderRadius:13,padding:12,marginBottom:10,border:`1px solid ${T.border}` }}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8 }}>
                  <div style={{ flex:1,minWidth:0 }}>
                    <div style={{ fontWeight:700,color:T.text,fontSize:13 }}>{j.title}</div>
                    <div style={{ fontSize:12,color:T.textMuted,marginTop:2 }}>{j.job_mode==="need"?"Hiring":"Available for work"} · {j.job_type}</div>
                    {j.price>0 && <div style={{ fontSize:12,color:T.primary,fontWeight:700,marginTop:2 }}>Ksh {j.price}</div>}
                  </div>
                  {j.seller_phone !== phone && (
                    <div style={{ display:"flex",gap:6,flexShrink:0 }}>
                      <button onClick={()=>setQuickChat({ chatKey:`jobs-${j.id}`, title:j.title, otherName:j.seller_name, otherPhone:j.seller_phone })} style={{ background:T.primarySoft,color:T.primary,border:"none",borderRadius:20,padding:"6px 10px",fontSize:11,fontWeight:700,cursor:"pointer" }}>💬 Chat</button>
                      <a href={`tel:${j.seller_phone}`} style={{ background:T.successSoft,color:T.success,borderRadius:20,padding:"6px 10px",fontSize:11,fontWeight:700,textDecoration:"none" }}>📞 Call</a>
                    </div>
                  )}
                </div>
              </div>
        ...
        {tab==="services" && (
          <div>
            {tabLoading && !loadedTabs.services ? <div style={{ padding:30,textAlign:"center",color:T.textMuted }}>Loading...</div> : marketServices.length===0 ? (
              <div style={{ textAlign:"center",padding:"30px 0" }}>
                <div style={{ fontSize:40 }}>🔧</div>
                <div style={{ fontWeight:700,color:T.text,marginTop:10,fontSize:14 }}>No services tagged to {market.name} yet</div>
                <div style={{ fontSize:12,color:T.textMuted,marginTop:4 }}>Fundis can post their profile and tag it to this market.</div>
              </div>
            ) : marketServices.map(s=>(
              <div key={s.id} style={{ background:T.card,borderRadius:13,padding:12,marginBottom:10,border:`1px solid ${T.border}`,display:"flex",gap:10,alignItems:"center" }}>
                <div style={{ flex:1,minWidth:0 }}>
                  <div style={{ fontWeight:700,color:T.text,fontSize:13 }}>{s.title}</div>
                  <div style={{ fontSize:11,color:T.textMuted }}>{s.fundi_available?"Available now":"Currently busy"}</div>
                </div>
                {s.seller_phone !== phone && (
                  <div style={{ display:"flex",gap:6,flexShrink:0 }}>
                    <button onClick={()=>setQuickChat({ chatKey:`fundi-${s.id}`, title:s.title, otherName:s.seller_name, otherPhone:s.seller_phone })} style={{ background:T.primarySoft,color:T.primary,border:"none",borderRadius:20,padding:"6px 10px",fontSize:11,fontWeight:700,cursor:"pointer" }}>💬 Chat</button>
                    <a href={`tel:${s.seller_phone}`} style={{ background:T.successSoft,color:T.success,borderRadius:20,padding:"6px 10px",fontSize:11,fontWeight:700,textDecoration:"none" }}>📞 Call</a>
                  </div>
                )}
              </div>
            ))}
        ...
        {tab==="transport" && (
          <div>
            {tabLoading && !loadedTabs.transport ? <div style={{ padding:30,textAlign:"center",color:T.textMuted }}>Loading...</div> : marketTransport.length===0 ? (
              <div style={{ textAlign:"center",padding:"30px 0" }}>
                <div style={{ fontSize:40 }}>🚚</div>
                <div style={{ fontWeight:700,color:T.text,marginTop:10,fontSize:14 }}>No transporters tagged to {market.name} yet</div>
                <div style={{ fontSize:12,color:T.textMuted,marginTop:4 }}>Riders and drivers can register and tag their profile to this market.</div>
              </div>
            ) : marketTransport.map(v=>(
              <div key={v.id} style={{ background:T.card,borderRadius:13,padding:12,marginBottom:10,border:`1px solid ${T.border}`,display:"flex",gap:10,alignItems:"center" }}>
                <div style={{ flex:1,minWidth:0 }}>
                  <div style={{ fontWeight:700,color:T.text,fontSize:13 }}>{v.title}</div>
                  <div style={{ fontSize:11,color:T.textMuted }}>{v.usafiri_available?"Available now":"Off duty"} {v.usafiri_area && `· ${v.usafiri_area}`}</div>
                </div>
                {v.seller_phone !== phone && (
                  <div style={{ display:"flex",gap:6,flexShrink:0 }}>
                    <button onClick={()=>setQuickChat({ chatKey:`usafiri-${v.id}`, title:v.title, otherName:v.seller_name, otherPhone:v.seller_phone })} style={{ background:T.primarySoft,color:T.primary,border:"none",borderRadius:20,padding:"6px 10px",fontSize:11,fontWeight:700,cursor:"pointer" }}>💬 Chat</button>
                    <a href={`tel:${v.seller_phone}`} style={{ background:T.successSoft,color:T.success,borderRadius:20,padding:"6px 10px",fontSize:11,fontWeight:700,textDecoration:"none" }}>📞 Call</a>
                  </div>
                )}
              </div>
            ))}
        ...
        {tab==="guides" && (
          <div>
            <div style={{ background:T.accentSoft,border:`1px solid ${T.accent}`,borderRadius:13,padding:12,marginBottom:14 }}>
              <div style={{ fontWeight:700,fontSize:13,color:T.accentDark }}>🧭 Need help around {market.name}?</div>
              <div style={{ fontSize:11,color:T.textMuted,marginTop:2,marginBottom:10 }}>Nitume Verified Market Guides can help you navigate, find sellers, and shop.</div>
              <button onClick={onRequestGuide} style={{ width:"100%",padding:10,background:T.primary,color:"#fff",border:"none",borderRadius:11,fontWeight:700,fontSize:13,cursor:"pointer" }}>Request a Guide →</button>
            </div>
            {tabLoading && !loadedTabs.guides ? <div style={{ padding:30,textAlign:"center",color:T.textMuted }}>Loading...</div> : marketGuides.length===0 ? (
              <div style={{ textAlign:"center",padding:"20px 0" }}>
                <div style={{ fontSize:12,color:T.textMuted }}>No registered guides for {market.name} yet — be the first from Nitume.</div>
              </div>
            ) : marketGuides.map(g=>(
              <div key={g.phone} style={{ background:T.card,borderRadius:13,padding:12,marginBottom:10,border:`1px solid ${T.border}` }}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                  <div style={{ fontWeight:700,color:T.text,fontSize:13 }}>{g.name}</div>
                  {g.is_verified_shopper && <span style={{ background:T.successSoft,color:T.success,borderRadius:20,padding:"2px 8px",fontSize:10,fontWeight:700 }}>✅ Nitume Verified</span>}
                </div>
                <div style={{ fontSize:11,color:T.textMuted,marginTop:2 }}>
                  {g.nitume_rating_avg ? `⭐ ${g.nitume_rating_avg.toFixed(1)}` : "New guide"} {g.nitume_completed_count ? `· ${g.nitume_completed_count} jobs done` : ""}
                </div>
                {g.knows_notes && <div style={{ fontSize:11,color:T.textMid,marginTop:4 }}>Knows: {g.knows_notes}</div>}
              </div>
            ))}
          </div>
        )}
        ...
        {tab==="notices" && (
          <div>
            <button onClick={()=>{ setNoticeError(""); setShowPostNotice(true); }} style={{ width:"100%",padding:11,marginBottom:14,background:T.primary,color:"#fff",border:"none",borderRadius:11,fontWeight:700,fontSize:13,cursor:"pointer" }}>+ Post a Notice</button>
            <input value={noticeSearch} onChange={e=>setNoticeSearch(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter") loadTab("notices"); }} placeholder="Search notices..."
              style={{ width:"100%",padding:11,borderRadius:11,border:`1px solid ${T.border}`,fontSize:13,marginBottom:10 }} />
            <div style={{ display:"flex",gap:8,marginBottom:14,overflowX:"auto" }}>
              {[["all","All"],["community","📋 Community"],["lost_found","🔎 Lost & Found"],["business","🏪 Business"]].map(([k,l])=>(
                <div key={k} onClick={()=>setNoticeFilterCat(k)} style={{ padding:"7px 13px",borderRadius:20,fontSize:12,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",background:noticeFilterCat===k?T.primary:T.surface,color:noticeFilterCat===k?"#fff":T.textMid,border:`1px solid ${noticeFilterCat===k?T.primary:T.border}` }}>{l}</div>
              ))}
            </div>
            {tabLoading && !loadedTabs.notices ? <div style={{ padding:30,textAlign:"center",color:T.textMuted }}>Loading...</div> : notices.length===0 ? (
              <div style={{ textAlign:"center",padding:"30px 0" }}>
                <div style={{ fontSize:40 }}>📢</div>
                <div style={{ fontWeight:700,color:T.text,marginTop:10,fontSize:14 }}>{noticeFilterCat!=="all"||noticeSearch.trim() ? "No notices match your filter" : `No notices for ${market.name} yet`}</div>
                <div style={{ fontSize:12,color:T.textMuted,marginTop:4 }}>Lost & found, community info, or business updates can go here.</div>
              </div>
            ) : notices.map(n=>{
              const catInfo = { business:{icon:"🏪",label:"Business",color:T.primary,bg:T.primarySoft}, lost_found:{icon:"🔎",label:"Lost & Found",color:T.accentDark,bg:T.accentSoft}, community:{icon:"📋",label:"Community",color:T.success,bg:T.successSoft} }[n.category] || { icon:"📋",label:n.category,color:T.textMid,bg:T.card };
              return (
                <div key={n.id} style={{ background:T.card,borderRadius:13,padding:13,marginBottom:10,border:`1px solid ${T.border}` }}>
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
                    <span style={{ background:catInfo.bg,color:catInfo.color,borderRadius:20,padding:"3px 10px",fontSize:10,fontWeight:700 }}>{catInfo.icon} {catInfo.label}</span>
                    {n.phone===phone ? (
                      <span onClick={()=>deleteMyNotice(n.id)} style={{ color:T.coral,fontSize:11,fontWeight:700,cursor:"pointer" }}>Delete</span>
                    ) : (
                      <span onClick={()=>setReportingNotice(n)} style={{ color:T.textMuted,fontSize:16,cursor:"pointer",padding:"0 4px" }}>⋮</span>
                    )}
                  </div>
                  <div style={{ fontSize:13,color:T.text,marginTop:8,lineHeight:1.5 }}>{n.message}</div>
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:8 }}>
                    <div style={{ fontSize:11,color:T.textMuted }}>{n.author_name||"Someone"} · {new Date(n.created_at).toLocaleDateString()}</div>
                    {n.phone!==phone && (
                      <span onClick={()=>setQuickChat({ chatKey:`notice-${n.id}`, title:catInfo.label+" Notice", otherName:n.author_name||"Someone", otherPhone:n.phone })}
                        style={{ color:T.primary,fontSize:11,fontWeight:700,cursor:"pointer" }}>💬 Reply privately</span>
                    )}
                  </div>
                </div>
              );
        ...
        {tab==="spaces" && (
          <div>
            <button onClick={()=>{ setSpaceError(""); setShowPostSpace(true); }} style={{ width:"100%",padding:11,marginBottom:14,background:T.primary,color:"#fff",border:"none",borderRadius:11,fontWeight:700,fontSize:13,cursor:"pointer" }}>+ List a Shop or Space</button>
            {tabLoading && !loadedTabs.spaces ? <div style={{ padding:30,textAlign:"center",color:T.textMuted }}>Loading...</div> : spaces.length===0 ? (
              <div style={{ textAlign:"center",padding:"30px 0" }}>
                <div style={{ fontSize:40 }}>🏬</div>
                <div style={{ fontWeight:700,color:T.text,marginTop:10,fontSize:14 }}>No shops or spaces listed yet</div>
                <div style={{ fontSize:12,color:T.textMuted,marginTop:4 }}>Shop/stall for rent, temporary selling space, storage, and more.</div>
              </div>
            ) : spaces.map(s=>{
              const typeInfo = { shop:"🏪 Shop", stall:"🧺 Stall", kiosk_space:"🏬 Kiosk Space", container:"📦 Container", warehouse:"🏭 Warehouse", storage:"📥 Storage", open_space:"🌤️ Open Space", temporary:"⏳ Temporary Space" }[s.space_type] || s.space_type;
              return (
                <div key={s.id} style={{ background:T.card,borderRadius:13,padding:13,marginBottom:10,border:`1px solid ${T.border}` }}>
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
                    <span style={{ background:T.primarySoft,color:T.primary,borderRadius:20,padding:"3px 10px",fontSize:10,fontWeight:700 }}>{typeInfo}</span>
                    {s.phone===phone ? (
                      <div style={{ display:"flex",gap:10 }}>
                        <span onClick={()=>markSpaceTaken(s.id)} style={{ color:T.success,fontSize:11,fontWeight:700,cursor:"pointer" }}>Mark Taken</span>
                        <span onClick={()=>deleteMySpace(s.id)} style={{ color:T.coral,fontSize:11,fontWeight:700,cursor:"pointer" }}>Delete</span>
                      </div>
                    ) : null}
                  </div>
                  {s.photos?.[0] && <img src={s.photos[0]} alt="" style={{ width:"100%",height:130,objectFit:"cover",borderRadius:10,marginTop:10 }} />}
                  <div style={{ fontSize:13,color:T.text,marginTop:8,fontWeight:700 }}>{s.section||"Section not specified"} {s.size_desc && `· ${s.size_desc}`}</div>
                  {s.description && <div style={{ fontSize:12,color:T.textMid,marginTop:4,lineHeight:1.5 }}>{s.description}</div>}
                  <div style={{ display:"flex",gap:14,marginTop:8 }}>
                    {s.rent && <div style={{ fontSize:12,color:T.primary,fontWeight:700 }}>Rent: Ksh {s.rent.toLocaleString()}/mo</div>}
                    {s.deposit && <div style={{ fontSize:12,color:T.textMuted }}>Deposit: Ksh {s.deposit.toLocaleString()}</div>}
                  </div>
                  <div style={{ fontSize:11,color:T.textMuted,marginTop:6 }}>{s.poster_name||"Someone"} · {s.ownership_role}</div>
                  {s.phone!==phone && <a href={`tel:${s.phone}`} style={{ display:"block",textAlign:"center",marginTop:10,padding:10,background:T.successSoft,color:T.success,border:"none",borderRadius:11,fontSize:13,fontWeight:700,textDecoration:"none" }}>📞 Call</a>}
                </div>
              );
            })}
          </div>
        ...
        {tab==="info" && (
          <div style={{ fontSize:13,color:T.textMid,lineHeight:1.7 }}>
            <div><b>Level:</b> {market.market_level}</div>
            <div><b>County:</b> {market.county||"—"}</div>
            <div><b>Town:</b> {market.town||"—"}</div>
            {market.opening_information && <div style={{ marginTop:8 }}><b>Opening info:</b> {market.opening_information}</div>}
            <div style={{ marginTop:12,fontSize:11,color:T.textMuted }}>Transport for this market is coming in a future update.</div>
          </div>
        )}
      </div>

      {showRoleModal && (
        <div style={{ position:"fixed",inset:0,background:T.overlay,zIndex:100,display:"flex",alignItems:"flex-end" }} onClick={()=>setShowRoleModal(false)}>
          <div style={{ background:T.card,borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxHeight:"75vh",overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            {modalStep==="roles" ? (
              <>
                <div style={{ fontWeight:800,fontSize:16,color:T.text,marginBottom:4 }}>{market.type==="hood" ? `How do you fit into ${market.name}?` : `What do you do in ${market.name}?`}</div>
                <div style={{ fontSize:12,color:T.textMuted,marginBottom:16 }}>Pick all that apply</div>
                <div style={{ display:"flex",flexWrap:"wrap",gap:8,marginBottom:20 }}>
                  {(market.type==="hood" ? HOOD_ROLES : MARKET_ROLES).map(r=>{
                    const active = pickedRoles.includes(r);
                    return (
                      <div key={r} onClick={()=>toggleRole(r)} style={{ padding:"8px 14px",borderRadius:20,fontSize:13,fontWeight:700,cursor:"pointer",background:active?T.primary:T.surface,color:active?"#fff":T.textMid,border:`1px solid ${active?T.primary:T.border}` }}>{r}</div>
                    );
                  })}
                </div>
                <Btn onClick={confirmRoles} disabled={savingRoles}>{savingRoles?"Saving...":joined?"Update Roles →":(market.type==="hood"?"Join Hood →":"Join Market →")}</Btn>
                <button onClick={()=>setShowRoleModal(false)} style={{ width:"100%",padding:11,marginTop:8,background:"transparent",border:"none",color:T.textMuted,fontSize:13,cursor:"pointer" }}>Cancel</button>
              </>
            ) : (
              <>
                ...
                {existingKioskInfo ? (
                  <>
                    <div style={{ fontWeight:800,fontSize:16,color:T.text,textAlign:"center",marginBottom:6 }}>Add your kiosk to {market.name}?</div>
                    <div style={{ fontSize:13,color:T.textMuted,textAlign:"center",marginBottom:20,lineHeight:1.5 }}>You already have a kiosk — <b>{existingKioskInfo.name}</b>. Link it here so buyers browsing {market.name} can find you.</div>
                    <Btn onClick={linkExistingKiosk}>Yes, Add It →</Btn>
                  </>
                ) : (
                  <>
                    <div style={{ fontWeight:800,fontSize:16,color:T.text,textAlign:"center",marginBottom:6 }}>Set up your kiosk?</div>
                    <div style={{ fontSize:13,color:T.textMuted,textAlign:"center",marginBottom:20,lineHeight:1.5 }}>Optional — a kiosk shows buyers your name, what you sell, and lets them contact you directly. You can always do this later from your Profile.</div>
                    <Btn onClick={goSetupKiosk}>Set Up My Kiosk →</Btn>
                  </>
                )}
                <button onClick={skipKiosk} style={{ width:"100%",padding:11,marginTop:8,background:"transparent",border:"none",color:T.textMuted,fontSize:13,cursor:"pointer" }}>Not now</button>
              </>
            )}
          </div>
        </div>
      ...
      {showPostNotice && (
        <div style={{ position:"fixed",inset:0,background:T.overlay,zIndex:100,display:"flex",alignItems:"flex-end" }} onClick={()=>setShowPostNotice(false)}>
          <div style={{ background:T.card,borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxHeight:"80vh",overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            <div style={{ fontWeight:800,fontSize:16,color:T.text,marginBottom:16 }}>📢 Post a Notice</div>
            {noticeError && (
              <div style={{ background:"#FDECEA",border:"1px solid #F5C2C0",color:"#B3261E",borderRadius:11,padding:"10px 13px",fontSize:12,fontWeight:700,marginBottom:14 }}>
                ⚠️ {noticeError}
              </div>
            ...
            <div style={{ display:"flex",gap:8,marginBottom:14,flexWrap:"wrap" }}>
              {[["community","📋 Community"],["lost_found","🔎 Lost & Found"],["business","🏪 Business"]].map(([k,l])=>(
                <div key={k} onClick={()=>setNoticeCategory(k)} style={{ padding:"8px 14px",borderRadius:20,fontSize:12,fontWeight:700,cursor:"pointer",background:noticeCategory===k?T.primary:T.surface,color:noticeCategory===k?"#fff":T.textMid,border:`1px solid ${noticeCategory===k?T.primary:T.border}` }}>{l}</div>
              ))}
            </div>
            <label style={{ fontSize:12,fontWeight:700,color:T.text,display:"block",marginBottom:7 }}>Message</label>
            <textarea value={noticeMessage} onChange={e=>setNoticeMessage(e.target.value)} rows={4} placeholder="What do you want to tell the market?"
              style={{ width:"100%",padding:12,borderRadius:11,border:`1px solid ${T.border}`,fontSize:14,marginBottom:16,fontFamily:"inherit" }} />
            <Btn onClick={postNotice} disabled={postingNotice}>{postingNotice?"Posting...":"Post Notice →"}</Btn>
            <button onClick={()=>{ setShowPostNotice(false); setNoticeError(""); }} style={{ width:"100%",padding:11,marginTop:8,background:"transparent",border:"none",color:T.textMuted,fontSize:13,cursor:"pointer" }}>Cancel</button>
          </div>
        </div>
      )}

      {showPostSpace && (
        <div style={{ position:"fixed",inset:0,background:T.overlay,zIndex:100,display:"flex",alignItems:"flex-end" }} onClick={()=>setShowPostSpace(false)}>
          <div style={{ background:T.card,borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxHeight:"85vh",overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            <div style={{ fontWeight:800,fontSize:16,color:T.text,marginBottom:16 }}>🏬 List a Shop or Space</div>
            {spaceError && (
              <div style={{ background:"#FDECEA",border:"1px solid #F5C2C0",color:"#B3261E",borderRadius:11,padding:"10px 13px",fontSize:12,fontWeight:700,marginBottom:14 }}>
                ⚠️ {spaceError}
              </div>
            )}
            <label style={{ fontSize:12,fontWeight:700,color:T.text,display:"block",marginBottom:7 }}>Type of space</label>
            <select value={spaceType} onChange={e=>setSpaceType(e.target.value)} style={{ width:"100%",padding:12,borderRadius:11,border:`1px solid ${T.border}`,fontSize:14,marginBottom:14,background:"#fff" }}>
              <option value="shop">🏪 Shop</option>
              <option value="stall">🧺 Stall</option>
              <option value="kiosk_space">🏬 Kiosk Space</option>
              <option value="container">📦 Container</option>
              <option value="warehouse">🏭 Warehouse</option>
              <option value="storage">📥 Storage</option>
              <option value="open_space">🌤️ Open Selling Space</option>
              <option value="temporary">⏳ Temporary Space</option>
            </select>

            <label style={{ fontSize:12,fontWeight:700,color:T.text,display:"block",marginBottom:7 }}>You are the...</label>
            <select value={spaceOwnership} onChange={e=>setSpaceOwnership(e.target.value)} style={{ width:"100%",padding:12,borderRadius:11,border:`1px solid ${T.border}`,fontSize:14,marginBottom:14,background:"#fff" }}>
            ...
            <Field label="Section / Location in Market" value={spaceSection} onChange={setSpaceSection} placeholder="e.g. Kamukunji, Row 4" />
            <Field label="Size" value={spaceSize} onChange={setSpaceSize} placeholder="e.g. 10 x 10 ft" />
            <Field label="Rent (Ksh/month, optional)" value={spaceRent} onChange={setSpaceRent} placeholder="e.g. 8000" type="number" />
            <Field label="Deposit (Ksh, optional)" value={spaceDeposit} onChange={setSpaceDeposit} placeholder="e.g. 8000" type="number" />
            <label style={{ fontSize:12, fontWeight:700, color:T.text, display:"block", marginBottom:6 }}>Description (optional)</label>
            <textarea value={spaceDesc} onChange={e=>setSpaceDesc(e.target.value)} rows={3}
              style={{ width:"100%", padding:12, borderRadius:11, border:`1px solid ${T.border}`, fontSize:14, marginBottom:14, fontFamily:"inherit" }} />
            <PhotoUploader photos={spacePhotos} setPhotos={setSpacePhotos} maxPhotos={3} />
            <div onClick={()=>setSpaceTemporary(t=>!t)} style={{ display:"flex",alignItems:"center",gap:8,marginTop:6,marginBottom:16,cursor:"pointer" }}>
              <div style={{ width:20,height:20,borderRadius:5,border:`2px solid ${spaceTemporary?T.primary:T.border}`,background:spaceTemporary?T.primary:"transparent",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:13 }}>{spaceTemporary?"✓":""}</div>
              <div style={{ fontSize:13,color:T.text }}>This is a temporary/short-term space (expires in 3 days instead of 30)</div>
            </div>
            <Btn onClick={postSpace} disabled={postingSpace}>{postingSpace?"Posting...":"List Space →"}</Btn>
            <button onClick={()=>{ setShowPostSpace(false); setSpaceError(""); }} style={{ width:"100%",padding:11,marginTop:8,background:"transparent",border:"none",color:T.textMuted,fontSize:13,cursor:"pointer" }}>Cancel</button>
          </div>
        </div>
      )}

      {reportingNotice && (
        <div style={{ position:"fixed",inset:0,background:T.overlay,zIndex:100,display:"flex",alignItems:"flex-end" }} onClick={()=>setReportingNotice(null)}>
          <div style={{ background:T.card,borderRadius:"20px 20px 0 0",padding:20,width:"100%" }} onClick={e=>e.stopPropagation()}>
            <div style={{ fontWeight:800,fontSize:16,color:T.text,marginBottom:14 }}>Report this notice</div>
            {["Hate speech / incitement","Spam","Scam","False information","Harassment","Duplicate","Wrong market","Illegal content","Other"].map(reason=>(
              <div key={reason} onClick={()=>reportNotice(reportingNotice.id, reason)}
                style={{ padding:"12px 4px",borderBottom:`1px solid ${T.border}`,fontSize:14,cursor:"pointer",
                  color: reason==="Hate speech / incitement" ? T.coral : T.text,
                  fontWeight: reason==="Hate speech / incitement" ? 800 : 400 }}>
                {reason==="Hate speech / incitement" ? "🚩 " : ""}{reason}
              </div>
            ))}
            <button onClick={()=>setReportingNotice(null)} style={{ width:"100%",padding:11,marginTop:12,background:"transparent",border:"none",color:T.textMuted,fontSize:13,cursor:"pointer" }}>Cancel</button>
          </div>
        </div>
      )}

      {quickChat && (
        <SimpleChatThread chatKey={quickChat.chatKey} title={quickChat.title} otherName={quickChat.otherName} otherPhone={quickChat.otherPhone}
          currentPhone={phone} currentName={userName} themeColor={T.primary} onClose={()=>setQuickChat(null)} />
      )}
    </div>
  );
}
...
function AdminPanel({ sb, onBack, showToast }) {
  ...
  const [noticeReports, setNoticeReports] = useState([]);
  const [hoodSuggestions, setHoodSuggestions] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  async function loadTab() {
    ...
    if (tab==="reports") {
      ...
      const { data: reps } = await sb.from("market_notice_reports").select("*").order("created_at",{ ascending:false });
      const noticeIds = [...new Set((reps||[]).map(r=>r.notice_id))];
      let noticesById = {};
      if (noticeIds.length) {
        const { data: notices } = await sb.from("market_notices").select("*").in("id",noticeIds);
        (notices||[]).forEach(n=>{ noticesById[n.id]=n; });
      }
      setNoticeReports((reps||[]).map(r=>({ ...r, notice: noticesById[r.notice_id] || null })));
    } else if (tab==="revenue") {
      ...
      const { data: pending } = await sb.from("markets").select("*").eq("type","hood").eq("status","PENDING").order("created_at",{ ascending:false });
      const parentIds = [...new Set((pending||[]).map(h=>h.parent_id).filter(Boolean))];
      let parentsById = {};
      if (parentIds.length) {
        const { data: parents } = await sb.from("markets").select("id,name,level").in("id",parentIds);
        (parents||[]).forEach(p=>{ parentsById[p.id]=p; });
      }
      setHoodSuggestions((pending||[]).map(h=>({ ...h, parent: parentsById[h.parent_id] || null })));
    }
    setLoading(false);
  }
  ...
  async function dismissNoticeReport(reportId) {
    const report = noticeReports.find(r=>r.id===reportId);
    const { error } = await sb.from("market_notice_reports").delete().eq("id",reportId);
    if (error) { showToast("Dismiss failed: " + error.message, "error"); return; }
    if (report) {
      await sb.from("notifications").insert({
        recipient_phone: report.reporter_phone, icon: "✅", read: false,
        title: "Report reviewed",
        body: "We reviewed your report on a market notice and found no violation — no action was needed.",
      });
    }
    setNoticeReports(prev=>prev.filter(r=>r.id!==reportId));
    showToast("Report dismissed.");
  }
  async function removeNotice(noticeId) {
    if(!window.confirm("Permanently remove this notice?")) return;
    const reporterPhones = [...new Set(noticeReports.filter(r=>r.notice_id===noticeId).map(r=>r.reporter_phone))];
    const { error } = await sb.from("market_notices").delete().eq("id",noticeId);
    if (error) { showToast("Remove failed: " + error.message, "error"); return; }
    await sb.from("market_notice_reports").delete().eq("notice_id",noticeId);
    if (reporterPhones.length) {
      await sb.from("notifications").insert(reporterPhones.map(p=>({
        recipient_phone: p, icon: "✅", read: false,
        title: "Report reviewed",
        body: "We reviewed your report and removed the notice. Thanks for helping keep this market safe.",
      })));
    }
    setNoticeReports(prev=>prev.filter(r=>r.notice_id!==noticeId));
    showToast("Notice removed.");
  }
  async function approveHood(h) {
    const { error } = await sb.from("markets").update({ status:"ACTIVE" }).eq("id",h.id);
    if (error) { showToast("Approve failed: " + error.message, "error"); return; }
    if (h.suggested_by_phone) {
      await sb.from("notifications").insert({
        recipient_phone: h.suggested_by_phone, icon: "🎉", read: false,
        title: "Your Hood suggestion was approved!",
        body: `"${h.name}" is now live on KaziApa. Thanks for helping grow the map!`,
      });
    }
    setHoodSuggestions(prev=>prev.filter(x=>x.id!==h.id));
    showToast("Hood approved and published.");
  }
  async function rejectHood(h) {
    if(!window.confirm(`Reject "${h.name}"? It won't be published.`)) return;
    const { error } = await sb.from("markets").update({ status:"REJECTED" }).eq("id",h.id);
    if (error) { showToast("Reject failed: " + error.message, "error"); return; }
    if (h.suggested_by_phone) {
      await sb.from("notifications").insert({
        recipient_phone: h.suggested_by_phone, icon: "🙏", read: false,
        title: "Your Hood suggestion wasn't added",
        body: `We reviewed "${h.name}" and decided not to add it right now — it may be a duplicate or need more detail.`,
      });
    }
    setHoodSuggestions(prev=>prev.filter(x=>x.id!==h.id));
    ...
    <div style={{ minHeight:"100vh",background:T2.bg,paddingBottom:40 }}>
      <div style={{ background:T2.primary,padding:"16px 16px 0",color:"#fff" }}>
        ...
        <div style={{ display:"flex",gap:4,overflowX:"auto" }}>
          {[["reports","🚩 Reports"],["notices","📢 Notices"],["hoods","🏘️ Hoods"],["revenue","💰 Revenue"],["users","👤 Users"],["housing","🏠 Housing"]].map(([key,label])=>(
            <button key={key} onClick={()=>setTab(key)}
      ...
      <div style={{ padding:16 }}>
        ...
        {!loading && tab==="notices" && (
          noticeReports.length===0 ? <div style={{ textAlign:"center",padding:30,color:T2.muted }}>No reported notices. ✅</div> :
          noticeReports.map(r=>(
            <div key={r.id} style={{ background:"#fff",borderRadius:12,padding:14,marginBottom:10,border:`1px solid ${T2.border}` }}>
              {r.notice ? (
                <>
                  <div style={{ fontSize:11,color:T2.muted,textTransform:"capitalize" }}>{r.notice.category} notice · {r.notice.author_name} · {r.notice.phone}</div>
                  <div style={{ fontWeight:700,color:T2.text,fontSize:14,marginTop:4 }}>{r.notice.message}</div>
                </>
              ) : (
                <div style={{ fontSize:13,color:T2.muted,fontStyle:"italic" }}>Notice already removed.</div>
              )}
              <div style={{ fontSize:12,color:T2.coral,fontWeight:700,marginTop:8 }}>🚩 Reported by {r.reporter_phone}{r.reason ? ` · "${r.reason}"` : ""}</div>
              <div style={{ display:"flex",gap:8,marginTop:10 }}>
                <button onClick={()=>dismissNoticeReport(r.id)} style={{ flex:1,padding:9,background:"transparent",border:`1px solid ${T2.border}`,borderRadius:8,fontSize:12,fontWeight:700,cursor:"pointer" }}>Dismiss Report</button>
                {r.notice && (
                  <button onClick={()=>removeNotice(r.notice_id)} style={{ flex:1,padding:9,background:T2.coral,color:"#fff",border:"none",borderRadius:8,fontSize:12,fontWeight:700,cursor:"pointer" }}>Remove Notice</button>
                )}
              </div>
            </div>
          ))
        ...
        {!loading && tab==="hoods" && (
          hoodSuggestions.length===0 ? <div style={{ textAlign:"center",padding:30,color:T2.muted }}>No pending Hood suggestions. ✅</div> :
          hoodSuggestions.map(h=>(
            <div key={h.id} style={{ background:"#fff",borderRadius:12,padding:14,marginBottom:10,border:`1px solid ${T2.border}` }}>
              <div style={{ fontSize:11,color:T2.muted,textTransform:"capitalize" }}>{h.level}{h.parent ? ` · under ${h.parent.name}` : ""}</div>
              <div style={{ fontWeight:700,color:T2.text,fontSize:14,marginTop:4 }}>{h.name}</div>
              {h.county && <div style={{ fontSize:12,color:T2.muted,marginTop:2 }}>{h.county}</div>}
              <div style={{ fontSize:12,color:T2.muted,marginTop:4 }}>Suggested by {h.suggested_by_phone||"unknown"}</div>
              <div style={{ display:"flex",gap:8,marginTop:10 }}>
                <button onClick={()=>rejectHood(h)} style={{ flex:1,padding:9,background:"transparent",border:`1px solid ${T2.border}`,borderRadius:8,fontSize:12,fontWeight:700,cursor:"pointer" }}>Reject</button>
                <button onClick={()=>approveHood(h)} style={{ flex:1,padding:9,background:T2.primary,color:"#fff",border:"none",borderRadius:8,fontSize:12,fontWeight:700,cursor:"pointer" }}>✅ Approve</button>
              </div>
            </div>
          ))
        )}
...
function RequestBoard({ userName,phone,town,requests,setRequests,showToast }) {
  ...
  async function postRequest() {
    ...
    const { data } = await sb.from("requests").insert({ buyer_name:userName,buyer_phone:phone,item:form.item,budget:parseInt(form.budget),category:form.category,town:form.town||town,area:form.area,market_id:form.market_id||null,market_name:form.market_name||null }).select();
    if(data?.[0]) { setRequests(p=>[data[0],...p]); setShowForm(false); setForm({ item:"",budget:"",area:"",town:"",category:"mitumba",market_id:null,market_name:"" }); showToast("Request posted! Sellers can see it now."); }
  }
      ...
      <div>
        ...
        <div style={{ padding:16 }}>
          <div style={{ background:"#FFF8E7",borderRadius:16,padding:16,marginBottom:16,border:"1px solid #F5A623" }}>
            ...
            {req.market_id && <div style={{ marginTop:6,display:"inline-block",background:"#E8F5EE",color:"#145A36",borderRadius:20,padding:"3px 10px",fontSize:11,fontWeight:700 }}>🏬 Tagged to {req.market_name||"a market"}</div>}
          </div>
      ...
      {showForm && (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,51,102,0.6)",zIndex:150,display:"flex",alignItems:"flex-end",justifyContent:"center" }} onClick={()=>setShowForm(false)}>
          <div style={{ background:"#fff",borderRadius:"22px 22px 0 0",padding:"24px 20px 36px",width:"100%",maxWidth:430 }} onClick={e=>e.stopPropagation()}>
            ...
            <MarketPicker marketId={form.market_id} marketName={form.market_name||""} onPick={(id,name)=>setForm({...form,market_id:id,market_name:name})} />
