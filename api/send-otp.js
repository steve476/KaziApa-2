export default async function handler(req, res) {
  ...
  // Google Play reviewer bypass — reviewers outside Kenya can't receive a
  // real Africa's Talking SMS. This one fixed test number always gets a
  // fixed code instead of a random SMS-delivered one, given only to Google
  // in the Play Console "Sign in details" field. No real SMS is sent for it.
  const REVIEWER_PHONE = "+254700000000";
  const REVIEWER_OTP = "123456";
  const isReviewer = fp === REVIEWER_PHONE;

  const otp = isReviewer ? REVIEWER_OTP : Math.floor(100000 + Math.random() * 900000).toString();
  ...
  try {
    ...
    if (isReviewer) {
      // Skip real SMS entirely for the reviewer test number.
