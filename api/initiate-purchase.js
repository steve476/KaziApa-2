...
const PRICING = {
  ...
  contact_unlock: {
    standard: { amount: 50, days: 36500 }, // effectively permanent per-listing unlock for this tenant
  },
  nitume_unlock: {
    standard: { amount: 20, days: 36500 }, // one-time unlock to accept a Nitume job
  },
  rent_pro: {
    monthly: { amount: 350, days: 30 },
    yearly: { amount: 3500, days: 365 },
  },
};
...
export default async function handler(req, res) {
  ...
  if ((type === "boost" || type === "contact_unlock" || type === "nitume_unlock") && !listingId) {
  ...
  try {
    ...
    if (!saveRes.ok) {
      ...
      console.error("PURCHASES INSERT FAILED:", saveRes.status, err);
    ...
    console.error("INITIATE-PURCHASE CRASHED:", err.message, err.stack);
