...
export default async function handler(req, res) {
  ...
  try {
    ...
    switch (purchase.type) {
      ...
      case "verified_badge":
        await supabaseFetch(`users?id=eq.${purchase.user_id}`, {
          method: "PATCH",
          body: JSON.stringify({
            is_verified: true,
      ...
      case "chat_pass":
        if (purchase.tier === "topup10") {
          const userRes = await supabaseFetch(`users?id=eq.${purchase.user_id}&select=chat_starts_today`);
          ...
          await supabaseFetch(`users?id=eq.${purchase.user_id}`, {
            method: "PATCH",
            body: JSON.stringify({ chat_unlimited_until: purchase.expires_at }),
          });
        }
        break;
